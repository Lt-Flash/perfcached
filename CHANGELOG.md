...
## 0.4.0-rc44 — 2026-09-26

**A collection's sizing has a mode: `autoscale = auto | warn | off`
(S239).**  An operator's explicit resize was shrunk back a minute later
by the automatic shrink, and nothing could say "leave this one".  After
Ceph's `pg_autoscale_mode`:
- **`auto`** (the default, today's behaviour) grows and shrinks a table
  by its key count.
- **`warn`** changes nothing.  It says what auto would do, in the log
  (at once, then at most every ten minutes), on the status page and in
  `/metrics`.
- **`off`** changes nothing and says nothing.

Under `warn` and `off` growth is blocked too.  Keys past the slots go
to the overflow leg.  The status page shows the collection red, with
the size auto would grow it to (`off ↑ 2^N`), as soon as it has too
many keys for its buckets - `off` included, though it logs nothing.

Set it per collection (`[collection x] autoscale`), with a default in
`[daemon] autoscale`, or at runtime with the privileged `autoscale`
verb (`perfcli -E ... autoscale <col> <mode>`).  The verb's setting is
persisted and outranks the config.  The status page puts the mode beside
each collection, amber (`warn -> 2^N`) when warn would shrink it.  New
metrics: `perfcached_collection_autoscale_mode` and
`perfcached_collection_autoscale_target_buckets`.  New suite:
`autoscaletest`.

**After a crash, every key comes back with the value it was serving
(S240).**  Each worker thread queues its writes separately and the WAL
writes those queues out in turn, so two writes to one key from
different workers could land in the log in the opposite order to the
order they happened.  Replay applied the log in file order, so the key
came back with the older value.  Measured: 10,000-17,000 of 20,000 keys
written concurrently by eight connections came back wrong after
`kill -9`.  Replay now goes by each record's version (stamped under the
key's lock since S234): a write lands only over an older one, and a
delete removes only an older record and stops an older write later in
the log from bringing the key back.  Deletes now log their version,
carried in the record's value field so that an older build rolled back
onto this log still replays it (as plain deletes).  New suite
`walordertest`: the previous build failed it on 50,918 of 160,000 raced
keys, this one on none.

## 0.4.0-rc43 — 2026-09-26 (superseded by rc44)

*Superseded while its pipelines were still running.*

**A node the fleet refuses can no longer take its mastership (S241).**
A node whose config does not match the fleet's is refused membership,
but the maps it published as its own lone master were still obeyed.  A
refused node with a higher term made the real master step down, both
re-promoted in turn, and terms climbed without end: on the test fleet,
111 to 180 in five minutes, with members flapping between member and
backup.  A map, or a standby sync, is now accepted only from a member
the node admits or from the master it follows.  Ignored maps are
counted (`cluster.map.unadmitted`).  **The refusal now says what
differs (S242):** it prints this node's whole config (mode, eager, WAL,
replicas, placement, collections), and says so when mode and eager
match.  Before, a WAL mismatch showed only an unrelated placement
field.  New suite `refusedpeertest`: on the previous build the term
went 1 to 13 in 30 s.

## 0.4.0-rc42 — 2026-09-26 (RED - superseded by rc43)

*GitHub's `check-asan` failed the new `eagerparttest`: it preloaded its
network-cut shim alone, and an ASan daemon refuses to start unless its
runtime comes first in the preload list.  The suite now puts the runtime
first, as `healtest` does; the daemon is unchanged.*

**The status page no longer shows negative figures after a node
restarts (S243).**  Its 5-minute counter figures (REPLICATION's "pushed
on write", "migrated in" and the like) were the counter now minus the
oldest value the page had kept.  When the node restarted and its counters
began again at zero, that went negative, down to "-2 418 242", until the
page was reloaded.  A counter that goes backwards now restarts its
window.

**Known gap, now documented (S238): in eager mode, a delete made during
a partition can be undone when the partition heals.**  A new suite,
`eagerparttest`, splits a three-node eager fleet with writes on both
sides.  Both sides' writes survive the heal and agree everywhere, but
all 50 deletes made while it was split came back.  The other side's
repair sweep re-sends its copy after the 2 s tombstones have expired,
and a heal, unlike a restart, runs no reconcile.  The fix needs deletes
remembered for as long as a member may still hold the older copy, and
is being designed.  Until then, re-issue deletes that matter after a
split.

## 0.4.0-rc41 — 2026-09-26 (superseded by rc42)

*GitHub passed it; GitLab's pipeline was cancelled while its `check`
and `check-asan` were still running, when rc42 superseded it.*

**The status page's compass mark matches the anchor (S236):** its ring
and arrows are drawn in the page's text colour, as the anchor is, and
its ring at the anchor's line width.

**Writes scale with workers: three shared counters are gone from the
write path (S235, S231, S237).**  Every SET wrote three cache lines
that every worker also needed: the memory allocator's live-bytes
counter, a collection's write-stream statistics (kept beside the
fields every request reads to find its collection), and the version
clock, which shared a line with values read several times a request.
Each write therefore stalled the others, and more workers made it
worse.  The allocator and collection counters are now per-thread
stripes summed when read, and the clock has its own line.  CPU per
SET, measured with the load sent from two other hosts (µs/request):

| workers | before | after | change |
|---|---|---|---|
| 4 | 1.8 | 1.4-1.5 | -22% |
| 8 | 2.2-2.7 | 1.5 | -37% |
| 12 | 3.1-3.3 | 1.6 | -50% |

The collection statistics had also been losing counts: under eight
concurrent writers they undercounted by 0.4%, and the size histogram
disagreed with its own total.  They are now exact, and the new
`colstattest` checks it.

**A fleet stop stops the whole fleet after a partition, and its answer
says which peers really stopped (S233).**  RV-5's fault run cut one
node off from its peers.  The cut-off node promoted itself and recorded
a new term, but could not publish a map under it, and rejoined the
fleet on its old one.  It then compared the next fleet stop with the
term it had recorded, not the term of the fleet it had rejoined, took
the stop for a stale one, ignored it and kept serving.  A stop is now
judged against the term of the map the node follows.  A stop replayed
from a fleet that has since moved on still carries an older term, so
it is still refused.  Three more faults from the same run:
- **`stopped` now means said goodbye.**  The reply listed as `stopped`
  a peer that had merely left the live set: that one had promoted
  itself and was still serving.  A peer that leaves without a goodbye
  is now `lost`.
- **perfcli never sends a privileged verb twice.**  After its read
  timed out it reconnected and sent `fleetstop` again.
  `fleetstop`, `create`, `drop`, `rename`, `resize` and `restore` are
  not resent after a reconnect, and perfcli says the verb may already
  have run.
- **A one-shot `perfcli fleetstop` waits for the answer.**  The stop
  waits up to 30 s for its peers, the same as perfcli's read timeout.
  One-shot fleetstop now waits up to 125 s, and `fleetstop <ms>` sets
  the stop's own wait.

`stoptest` gains both cases, each run on a build with the fix reverted
as well.

## 0.4.0-rc40 — 2026-09-26 (superseded by rc41)

*GitHub passed it; GitLab's pipeline was cancelled while its `check`
and `check-asan` were still running, when rc41 superseded it.*

**The WAL sizing warning (rc37) now appears on a freshly booted host.**
Its ten-minute repeat limit was measured on a clock that counts from
boot, starting from zero.  So on any machine up less than ten minutes
the first warning was swallowed: a new VM, a container host just
started, every CI runner.  GitHub's `walsizetest` failed on it for
rc38 and rc39 while every long-running host passed.  The first warning
now always fires, and the limit applies to repeats.  Two cluster notices
limited the same way (the heartbeat watchdog's, 10 s, and the
wire-epoch one, 60 s) are fixed with it.

**A write's version is stamped where it lands, so two writes to one key
can no longer store in one order and replicate in the other (S234).**
A local write took its version while building the record, before it
locked the key.  So two workers writing one key could store the older
version over the newer: this node kept one value and every peer
refused it as older and kept the other, until the key was written
again or expired.  A delete had the same gap: its tombstone's version
was taken after the key was unlocked, so a set landing in between sat
below the tombstone on every peer and stayed here.  An increment of a
counter in a table's overflow leg also kept its old version while
replicating a new one.  Versions are now stamped under the key's lock,
never below the record they replace, and a tombstone carries the
version its delete was stamped with.  A side effect: an overwrite that
fits its record ticks the clock once instead of twice, halving the
clock's share of a SET (10-12% to 4-5% at 4 workers; SET 2.0-2.2 to
1.6 us/request there).  New unit test `verstamptest` holds the lock
itself and plays each race in a fixed order; it fails on the
previous table.

## 0.4.0-rc39 — 2026-09-26 (RED - superseded by rc40)

*GitHub's `check` failed `walsizetest` - as it had for rc38 - because
the sizing warning's repeat limit swallowed the first warning on a host
up less than ten minutes (fixed in rc40).  GitLab's pipeline was
cancelled when rc40 superseded it.*

*No change to the daemon.*  rc38's `check-asan` failed `healtest`: its
40,000-record fill, before the burst the suite is about, dropped 34
records and healed early.  The fill paced itself by round trips - two
writes in flight - but a write is acknowledged at the producer ring, so
round trips bound nothing when the WAL thread is slower than the client,
as it was under ASan on a busy runner.  The fill now puts a `sync`
barrier every 8 records (half the 16 KB ring), which bounds it by the
WAL thread itself; with the test device slowed during the fill, the old
fill dropped 4,816 of its first ~9,700
records and the new one none of 12,600.

## 0.4.0-rc38 — 2026-09-26 (RED - superseded by rc39)

*GitLab's `check-asan` failed `healtest` in its fill, a test that paced
itself by round trips (see rc39), and GitHub's `check` failed
`walsizetest` on a young runner (see rc40).*

**Delete tombstones can no longer be erased the moment they are
planted (RV-15).**  The negative cache that holds them was direct-
mapped: a delete whose key shared a slot with a live tombstone
overwrote it, however fresh - and a fresh tombstone is exactly the one
guarding its key against a copy of the deleted record still crossing on
the wire (S209), so losing it could bring a deleted key back.  Measured
at ~55k deletes/s on two nodes, 19-70% of tombstones were displaced
before their 2 s lifetime, any of them possibly within milliseconds of
planting.  It is now 4-way set-associative and evicts the entry closest
to expiry: under capacity, displacement fell from 18.6% to 1.1%; beyond
it (a sustained storm wants more live tombstones than slots) the oldest
go first, so a tombstone's shortest life is capacity / rate - ~1.2 s at
that rate - against a crossing window of milliseconds.  Same memory.
New: `cluster.neg_displaced`, `cluster.tomb_displaced`
(`perfcached_cluster_neg_displaced_total`,
`perfcached_cluster_tomb_displaced_total`) and a line on the page's
negative-cache card; `clloctest` proves a fresh tombstone survives three
colliding deletes behind it, which the direct-mapped table failed.

## 0.4.0-rc37 — 2026-09-26 (RED - superseded by rc38)

*GitHub's `check` failed `rdbkilltest` (1 acknowledged write of 42,000
missing after a kill -9): the suite killed the node the instant its last
write was answered, and an acknowledgement is given at the producer ring,
which a kill -9 takes before the WAL thread drains it - documented
semantics the test overreached.  rc38 syncs the WAL before the kill.
rc37 was live on the test fleet (245-247) only.*

**A snapshot killed mid-write no longer blocks every snapshot after it
(RV-14).**  Its temp file (`dump.rdb.tmp.<pid>`) was never removed - one
more table-sized file on the WAL's volume per crash - and because it is
opened exclusively under the pid, a daemon back with the SAME pid could
not write a snapshot again: every save failed, and since S229 every
heal with it.  In a container the daemon is pid 1, so one kill
mid-snapshot was enough.  Dead temp files are removed at start, and a
save that finds its own name taken removes it and goes on.  New suite
`rdbkilltest`: kill -9 mid-save twice, with the WAL ring wrapping while
the save runs - every acknowledged write is there after each restart,
including those written during the killed save - then a dead temp
planted under the daemon's own pid.  The build before kept 1, then 2,
dead files and failed the planted-name snapshot.

**A WAL too small for its snapshots says so (RV-13).**  If a snapshot
takes longer than the whole ring takes to fill at the current write
rate, no trigger can start one early enough: the node overruns and heals
again and again.  It now warns, at most every ten minutes, with both
figures and the three fixes, and `/stats` `wal.ring_fill_s` /
`perfcached_wal_ring_fill_seconds` give an alert the number to compare
with `perfcached_rdb_last_duration_seconds`.  Building it found a bug
between S224 and S229: a heal's snapshot is unthrottled (5 ms on tmpfs
where a budgeted one took 3 s), and it became "the last snapshot's
duration" - after a heal the rate trigger, this warning and RV-12's alert
all believed snapshots were free.  Only a snapshot written at the budget
sets that figure now.  New suite `walsizetest`.  `healtest` now forces
its losses with a slow-device shim (`syncfailshim` "walslow") instead of
racing the WAL pump, which under ASan on rc36's GitLab run it lost.

**Status page: the node controls sit on their own row, under the logo.**
In rc36 they sat right-aligned beside the identity line, and a second
after load - once the page filled in node, identity, role, version and
state - that row no longer fitted beside the larger marks, and the whole
group jumped to a second line.  They now always have a row of their own,
left-aligned under the anchor, so nothing on the first row can move them.

## 0.4.0-rc36 — 2026-09-26 (RED - superseded by rc37)

*GitLab's `check-asan` failed `healtest`: its burst raced the WAL pump to
force the drops it needs, and under ASan the pump kept up, so nothing
dropped and no heal could be shown - the test's fault, not the daemon's.
rc37 forces the loss with a slow-device shim.  rc36 was live on the test
fleet (245-247) only.*

**A node that loses WAL records heals itself instead of failing
(S229).**  A ring drop or a segment overrun used to mark the node FAILED
until a restart - and the dropped writes, acknowledged to their clients,
were gone after that restart: measured, 6,192 of a burst.  Every write
reaches the table before the WAL, so a lost record is missing from the
log, not from memory.  The node now goes HEALING: it refuses writes with
its own message, keeps serving reads, leaves client selection but stays
in the map (nothing re-shards, unlike FAILED), waits for its rings to
drain, takes an unthrottled snapshot, and returns to READY by itself
once a snapshot that began after the last loss is published - measured
at 100-400 ms.  After a kill -9 and a restart every acknowledged write
is there.  It still goes FAILED when it cannot heal: the device refused
a write, or the heal's snapshot failed.  New: the `healing` state
(travels to peers as state 6; an older peer reads it as not usable),
`perfcached_node_state{state=...}` - the node's state was never in
`/metrics` before - and `perfcached_wal_heals_total`,
`perfcached_wal_healing`, `perfcached_wal_last_heal_seconds`; `/stats`
`wal.heals`, `wal.healing`, `wal.last_heal_ms`.  The Grafana dashboard
gains a node-state timeline, a heals panel, a "healing now" tile and a
state column.  New suite `healtest`; `waloverruntest` now expects the
heal.

**Status page: bigger header marks, centred on what they sit beside
(S227).**  Both marks go from 38 to 64 px, so the anchor's and the
compass's cells read as squares rather than dots.  They also sat low:
each drawing's ink is centred one unit below its box's middle (the
anchor spans y 3-31 of 32, the compass ring is centred at 17), and the
all-caps wordmark's line box reserves descender room a capital never
uses.  Both viewBoxes now start one unit down, and the wordmark's line
box is trimmed to its cap height (`text-box: trim-both cap
alphabetic`; a browser without it keeps the old box).  `httptest` checks
both, red on the build before.

## 0.4.0-rc35 — 2026-09-26 (superseded by rc36)

*Superseded before its pipelines finished; rc36 carries it whole.  Live
on the test fleet (245-247) only.*

**The WAL thread keeps up with ~40% more writes (S226).**  The pump
copied every record out of the producer ring a byte at a time, with a
modulo per byte - ~60% of a saturated WAL thread.  Two `memcpy`s now.
Measured with 1 KB records and the WAL on tmpfs (so the pump, not the
disk, is the limit): the thread saturated at ~125-130k records/s and at
the next load step DROPPED records and FAILED the node; after, the same
load and 50% more land in full (167-181k records/s, zero dropped), at
4.6-5.1 us of WAL-thread CPU per record instead of 7.4.  `walringtest`
checks the copy against the old loop at every offset, length and wrap
split of six ring sizes (1.12M cases).  The README's claim that
`everysec` cannot drop is corrected: on a disk whose write-back held
~35 MB/s, the WAL fell behind at ~35k records/s of 1 KB.

**`members` takes half the time.**  The operator saw it at 44 us a call
on the status page.  Profiled saturated (three-node cluster, three
collections, 2.9 KB reply): the `snprintf` family was 31% of its worker
time - the 16-byte identity was formatted one hex byte per call (48 per
reply), each collection hash and each address through `snprintf` too -
and `strlen` of every string literal another 5%, because `pc_jw_lit`
measured its argument at run time.  Hex and addresses are now formatted
by hand, and `pc_jw_lit` is inline so a literal's length is folded at
compile time, which every JSON reply shares.  Mean 14.2 -> 7.4 us, p50 13
-> 6, p99 37 -> 25.  Checked byte for byte: a cluster mixing the old and
new builds reads every member's address, identity and collection hashes
identically from both.

## 0.4.0-rc34 — 2026-09-25 (RED - superseded by rc35)

*GitLab's `check-asan` failed `applythreadstest`: with one of four apply
threads frozen, the replica reported STALLED 133 ms after the writes
began - before that thread could have held a record for 2 s - and
counted two stalls.  GitHub's `check-asan` passed, as did every GitLab
ASan run of the arm since rc26, and it was not reproduced here under CPU
pressure; rc35 makes the arm print the replica's own stall lines when it
fails, so a recurrence says which thread stopped.  rc34 was live on the
test fleet (245-247) only.*

**GET ~33% faster through the RESP door, from three costs a profile
named.**  Measured off-box (the server on its own host, 4 cores; load
from two other hosts at once; pipeline 64, 64-byte values, 1M keys): GET
2.32-2.45M -> 3.06-3.17M requests/s over three rounds, server CPU per
request 1.62-1.72 -> 1.25-1.29 us.  SET varied between rounds and is not
claimed.  Redis 8.10.1 on the same cores: 0.74-0.77M SET/s and
0.90-0.96M GET/s (io-threads 4: 0.81-0.91M, 0.93-1.05M).  The three:
(1) every RESP command looked its name up FOUR times, a linear
case-folding scan each time - GET is the 24th row, ~96 compares per
request, 14% of a SET's and 18% of a GET's worker CPU; it is now one
hashed lookup.  (2) the daemon's seconds clock called `pthread_once` on
every read - 8% of a SET; now only its first read does.  (3) every reply
number went through `snprintf` - 2% of a GET; now formatted by hand, and
`jwtest` checks it against `%lld` at the edges and over 200,000 values.

**A KEYS walk takes its table handle as one pair.**  The walk read the
table's registry slot and then its publish generation, lock-free; a swap
between the two paired the OLD table with the NEW generation, and that
walk read the retired table to the end without seeing the swap (the same
shape S228 fixed for `dump`).  Now one read under the registry lock.
`keysretiretest` failed on rc33's GitHub `check` with no walk ending on
the swap; that was not reproduced in 16 runs here (including on one and
two cores), and this race is too narrow to explain it, so the suite now
reads the count after every walk has been answered and, on a zero,
prints the timeline - when the swap landed and which walks were in
flight - rather than retrying.  `outagetest` waits 46 s before its
witnessed kill (35 s failed rc33's `check-asan`: the grace a survivor
must clear is up to 40 s, the trap rc30 fell into).

## 0.4.0-rc33 — 2026-09-25 (RED - superseded by rc34)

*GitHub's `check` failed `keysretiretest` (no walk ended on the swap;
not reproduced here - see rc34) and its `check-asan` failed
`outagetest` (a 35 s wait under a grace of up to 40 s).  rc33 was live
on the test fleet (245-247) only.*

**A dump that crosses a table resize is redone, never lossy.**  `dump`
pages a collection by bucket cursor and resolved the collection's table
afresh on every call.  A resize - an operator's `resize`, or the
automatic shrink after mass deletes, on by default - swaps in a table of
another shape, and a cursor carried across made the walk repeat records
after a grow and silently MISS them after a shrink: measured on one node
with 3,000 records, a grow between two chunks gave 554 repeats and 6
misses, a shrink lost 2,200 records, and perfdump across a grow wrote
4,073 records for 3,000 keys and exited 0.  Every `dump` reply now names
the table it walked (`table`, `buckets`); a caller that passes `table`
back is refused once the collection has been swapped, and perfdump pins
each collection's walkers to one table and dumps a swapped collection
again (up to five times, then it fails).  In-place growth - the
maintenance thread splitting buckets - is unchanged: it can repeat a
record, never lose one, and perfload upserts by version.  New suite
`dumpresizetest` (red on the build without the fix).  Found while reading
why rc32's `check-asan` failed `loadtest`: that failure was the
documented at-least-once (a member still splitting from its push);
`loadtest` and `dumptest` now wait for the table to stand still before
asserting exactly-once.

**The snapshot alert S224 documented can now be written (RV-12).**  rc32
said to alert when `full_in_s` falls below twice the last snapshot's
duration, but that duration was only in `/stats`: `/metrics` had the WAL
side of the comparison and nothing to compare it with.  New series:
`perfcached_rdb_last_duration_seconds`, `perfcached_rdb_saves_total`,
`perfcached_rdb_last_success_age_seconds` (-1 before the first) and
`perfcached_rdb_in_progress`; `/stats` gains `rdb.last_age_s`.  The README
writes the rule out against those names and says what it did not: with a
WAL directory, `save = off` still takes the snapshots the WAL asks for
(the example config said it "disables snapshots"; corrected).
`snaptriggertest` evaluates the rule on scraped values - quiet on a fresh
node, firing under its load.

## 0.4.0-rc32 — 2026-09-25 (RED - superseded by rc33)

*GitLab's `check-asan` failed `loadtest`: an eager member dumped while it
was still splitting buckets from its push handed back 114 records twice.
That is the walk's documented at-least-once under growth, so the suite
was asserting more than the contract promises; reading why found a real
defect beside it (a dump across a table SWAP could lose records - rc33,
S228).  GitHub's `check` failed `snaptriggertest`, the suite rc32 added:
its driver wrote a 4 MB burst before reading any reply, and on the
runner's smaller socket buffers the two ends deadlocked (the node stops
reading while its replies go unread).  rc33 fills in batches.  rc32 was
live on the test fleet (245-247) only.*

**The WAL asks for a snapshot early enough - including on its first lap
(S224).**  A snapshot frees only the WAL segments below the point it
started at, so under heavy writes it has to start while the free space
still outlasts it.  The only trigger was a fixed "a quarter free", which
on a node whose snapshot takes longer than that last quarter takes to
fill was too late by construction: the ring overran and the node FAILED.
Measured on an 8 MB ring against a ~4 s snapshot: the quarter alone
overran and refused 10,020 of 18,000 writes; the new rate trigger - free
segments at the current fill rate against twice the last snapshot's
duration - kept up with four snapshots and refused none.
Building it found a worse gap: both triggers only ran once the ring had
wrapped, so a fresh node with `save = off` reached its first wrap with no
trigger having run, and overran.  The pressure is now evaluated at every
segment change.  `stats.wal` gains `fill_mb_s` and `full_in_s` (and
`/metrics` the matching gauges); alert when `full_in_s` falls below twice
`rdb.last_dur_ms`.  `save = 0 N` is still refused, and now says why: a
rule of changes alone would run snapshots back to back under load.
Measured with perf at 23,746 writes/s: the new checks cost too little to
register.  The same profile found the WAL thread copying records a byte
at a time - filed as S226.

**The two suites that made rc30 and rc31 red now wait for a witness.**
`sweepclocktest` and `tombstonetest` wait until the surviving peer has
been up 46 s before the kill: S223's 30 s grace, plus up to 10 s by which
the returning node's alive stamp can read early.

## 0.4.0-rc31 — 2026-09-25 (RED - superseded by rc32)

*Red by inheritance: it carries rc30's `sweepclocktest` failure (below),
so its pipelines were cancelled rather than run to the same result.  It
was live on the test fleet (245-247), where `tools/gate.sh health`
passed; it went nowhere else.*

**Status page: every collection's hit-rate bar starts in the same
place.**  The bar and its caption are right-aligned together, and the
caption was only as wide as its text, so a row reading "life 0.0%" drew
its bar one character to the right of rows reading "life 18.1%" or
"never read".  The caption now has a fixed width, sized to the longest
it can be ("life 100.0%").

**A clean stop takes a snapshot, and the whole fleet can be stopped with
one request (S222).**  A node's WAL holds only what that node wrote, so
until now a node stopped on purpose came back with its own share of an
eager fleet and nothing else: measured, a three-node fleet stopped with
SIGTERM and one node restarted alone served 100 of 300 keys.  A clean
stop now snapshots after every thread has finished - the snapshot walk
stores the copies a node holds for its peers too - and the same node
restarted alone serves all 300.  The save is unthrottled (the WAL it
normally yields to is done by then) and written to a temporary file and
renamed, so a stop cut short by the service manager loses nothing.  A
`kill -9` or a power cut takes no snapshot and recovers exactly as
before.
`fleetstop` asks every node in the fleet to stop that way, waits for
them to go, reports which did and which did not, and then stops the node
that was asked, last: `perfcli -E <enable secret> fleetstop`.  It needs
a privileged connection - stopping every node is the largest blast
radius there is, and the client secret is held by every application - and
a node ignores a stop from an older mastership term than its own, so a
captured request cannot stop a fleet that has since moved on.  A build
older than this ignores the request and is reported as still up.
The stop marker the design once called for was not built: its only job
was to tell a node whether the fleet had stopped together, and S223's
witness rule already answers that, for crashes as well.
`test/stoptest.sh` restarts a node alone after a clean stop, after a
fleet stop and after a kill, and repeats the first on a build without
the snapshot - which must leave that node with a third of the data.

## 0.4.0-rc30 — 2026-09-25 (RED - superseded by rc32)

*GitHub's `check` and `check-asan` failed in `sweepclocktest`: "the
reconcile pass probed 0 keys".  The daemon was right and the suite was
not.  Under S223 a returning node reconciles only if a peer was up at
least 30 s before it went down, and this suite killed its node while the
peer was about 20 s old, so the pass was correctly skipped.
`tombstonetest` had the same premise.  Both now wait for the peer to be
old enough; rc32 carries that.*

**A whole-fleet outage no longer makes an eager fleet delete its own
data (S223).**  Measured before the fix on three eager nodes with no
snapshot: killed and restarted together, 600 of 900 keys were gone;
restarted one by one, the second node back deleted all 300 of its own
durable records within 15 seconds.  An eager replica does not persist
the copies it receives, so after an outage each node holds only what it
wrote, and the reconcile pass - which drops a replayed key that no peer
still has, so that a key deleted while a node was away is not
resurrected - read every peer's truthful "I don't have it" as "it was
deleted".
The pass now needs a **witness**: a live peer that has been up since at
least 30 seconds before this node went down.  Only such a peer saw what
happened meanwhile, so only its "no" means "deleted".  A rejoin to a
fleet that stayed up finds one at once and behaves exactly as before; a
fleet that came back together - all at once, one by one, or in the
staggered order a power failure produces - finds none, skips the pass,
and deletes nothing.  Each peer's boot time already travels in its
heartbeat, and this node's time of death is the newest write in its
storage directory or a small stamp an eager node touches every ten
seconds, so **nothing on the wire changed** and a rolling upgrade is
the normal path.  `stats.cluster.reconcile_witness`,
`stats.cluster.reconcile_skipped` and
`perfcached_cluster_reconcile_skipped` say which way each node decided;
after a whole-fleet outage every node should read skipped.
The cost, stated: a node that dies within 30 seconds of every other
node's boot has no witness, so a key deleted while it was away stays.
That is a fleet that had only just formed; the alternative was deleting
its data.
The design that was filed for this - a purpose on the probe, a
"not authoritative" answer, and re-sharing before asking - was built
and measured wrong three ways; the last of them re-sent a deleted key
to the fleet before asking about it, which is the resurrection the pass
exists to prevent.  The design notes carry the whole account.
`test/outagetest.sh` kills and restarts a three-node fleet four ways and,
as a step of the same run, repeats two of them on a build with the
witness requirement removed - which must lose keys.

## 0.4.0-rc29 — 2026-09-24

**A cluster wire epoch, and mixed builds tested as mixed builds
(S221).**  Until now two daemons agreed to share a cluster if their
config digests and route algorithm matched - neither of which says
anything about the *meaning* of the frames they exchange.  An audit of
the releases in the fleet found what that costs: rc21 maps every probe
operation it does not recognise onto SET, so a DELETE forwarded from a
newer node executes there as a write.  Peer frames now carry
`PC_CL_EPOCH`, the number naming the contract the bytes belong to.  Two
builds with the same epoch federate whatever their versions; a peer with
another epoch is refused by **both** sides, the log names both numbers,
`stats.cluster.epoch_refused` and
`perfcached_cluster_epoch_refused_total` count it, and neither daemon is
otherwise affected.  The epoch is bumped only for a change an older
build would mis-execute rather than ignore, which is why a tail addition
does not bump it and a rolling upgrade remains the normal case; the
header in `src/cluster.h` carries the rule, and a release that does bump
it will say so here.  A build predating this field reads as epoch 0 and
is accepted while the epoch is 1, with a NOTICE saying the next one will
not accept it - so the upgrade that introduces the check needs no
outage.  A routing client reads the map and decides ownership itself, so
it is party to the same contract: libperfd compares the epoch the daemon
advertises and, on a mismatch, turns routing off and lets the daemon
forward - the fallback the route algorithm already had.  Plain RESP and
JSON clients are unaffected.
`test/epochtest.sh` is the first suite here to run **two builds against
each other**: it compiles three one-line variants of the tree (a
different version, a different epoch, and the epoch fields removed
entirely) and asserts that the version alone never blocks a join, that
the epoch alone always does, and - as its fail-first - that the build
without the check takes in the very node the checked build refuses.

## 0.4.0-rc28 — 2026-09-22

The apply-thread fix (S220), a warning for the one configuration that
cannot recover from a whole-fleet outage, and guidance on which mode to
run - written because measuring a power cut showed that the mode
believed to be the safest is the one that loses data when every node
dies together.  rc27 was green on both runners.

**An apply thread looks before it sleeps (S220).**  A consumer took the
mutex the instant its ring ran dry, so at about a microsecond a record
there was a sleep and a wake per record - and the wake was paid by the
receiving thread, the one that also has to drain the socket.  At
`apply_threads = 4` that cost the node more than the parallelism bought
it: 55% of the offered load applied and 27,716 datagrams dropped, with
one context switch per 13 records.  A consumer now re-reads its ring for
a few tens of microseconds before sleeping, so the common case needs no
syscall and the producer skips its signal too: **99% applied and 748
dropped** on the same load, one switch per 93 records.  Idle cost of the
looking, measured against a daemon with no apply threads: 0.7% of one
core for four threads.
The hypothesis was checked before the fix was written, as the filing
required - compiling the wake-up out cut CPU per record from 3.16 us to
1.43, which is over half the receiver's time, but made throughput worse
because the consumers then wait out their timer.  `apply_threads = 4` is
no longer warned against; what the README and config example say instead
is the thing that stays true, that each thread costs CPU per record
(0.95 us at one, 1.75 at two, 2.19 at four).

## 0.4.0-rc27 — 2026-09-22

Memory the arena gives back after a mass expiry (S190), a status page
whose alarms are visible and whose header lines up (S210, S208), the
doors under hostile input (RV-7 part 1), and two measurements that
closed questions rather than opening them (S217 step 2, S219).  rc26 was
green on both runners.

**`apply_threads`: use 2 at most, and the per-thread socket is not
coming (S217 step 2 measured, S220 filed).**  Step 2 - a socket per
apply thread, so the decrypt parallelises too - was argued from a
September profile where a record cost 8.5 us and decrypt was 30% of it.
Measured before building: a record now costs about 1 us, and handing it
to another thread costs about 0.7 us, so parallelising the decrypt
cannot pay for the distribution.  The premise is obsolete and step 2 is
refused rather than deferred.
The same measurement found that `apply_threads = 4` is worse than 1: the
receiver applies 47-83% of the offered load and drops 8,600-31,000
datagrams at the socket, against 0-337 at 2 threads.  The default is 1
and nothing in a fleet is affected, but the README and the config
example now say to use 2 at most and why.  Filed as S220 with the
hypothesis (one cond-signal per record, four rings deep, on the thread
that must also drain the socket) and the check that would confirm it.
No throughput number is claimed for 1 versus 2: in that rig the receiver
applies 96-99% of whatever the author offers, so those arms measure the
author, not the receiver.

**The doors under hostile input (RV-7, part 1).**  A new suite throws
about 1,070 malformed and random frames at the RESP and JSON doors -
truncated frames, lying lengths, negative and huge counts, a
10,000-command pipeline, a 100,000-argument multibulk, 200-deep JSON,
bad UTF-8, a PNG and a TLS hello, and 500 seeded random-byte frames at
each door - and asserts one property for all of them: the daemon
survives, and a good request on a fresh connection works afterwards,
checked every 25 cases and bisected so the case that broke it is named.
The size contract is pinned too: the largest value the arena allows
stores and reads back at full length, and one byte past the ceiling is
refused rather than truncated.  The seed is printed and is the second
argument, so any failure replays exactly.  Nothing was found - the
suite's worth is shown the other way, by removing the parser's
"the body is not all here yet" check in a sanitizer build, where the
corpus kills the daemon with a heap overflow and eleven assertions go
red.  It rides `check`, so `check-asan` runs it too.

**The replica apply resolves its collection once again (S219).**  S217
factored the per-record apply into a function an apply thread could call,
and that function must look the collection up itself - so the inline path,
which had already looked it up for its prefetch peek, scanned the
collection registry twice per applied record and copied the name twice.
The handle is passed through now, and the NUL-terminated copy turns out
to be needed only by the WAL, so it happens on the authored path instead
of every record.  **Tidying, not a speed-up:** this daemon's apply path
was profiled in September and `pc_store_find` did not reach the 1% cut,
so the redundant scan cost nothing worth measuring - the code is simply
smaller and does less.  No throughput claim is made or implied.

**A mass expiry no longer holds the memory it freed (S190).**  A chunk
retires only when every cell in it is free, so the handful of long-lived
records left behind by a mass expiry - spread over every chunk that held
the expired ones - pinned all of them: measured on the fleet, 57.5 MB
held for 2.2 MB live, four minutes after 62,000 records expired, and
indefinitely.  The arena now marks a chunk SPARSE on its reclaim tick
(at most an eighth live, quiet for the window) and the maintenance
thread moves those survivors into the densest chunk of their class, one
bounded slice a second, so the emptied chunks retire.  The move is the
overflow-leg drain's move applied between cells: the bucket lock, a copy
into the new cell, the slot swapped with the version bumped around it,
the old cell sent home - a reader sees the version move and retries, as
it already does for a replacing store.  It does nothing when no chunk is
sparse, which is the ordinary case.
New: `arena_sparse_chunks` and `arena_evacuated` in `stats`.
Gate: `evactest` - 60,000 records in 60 chunks, every 40th kept: the
chunks go 60 -> 4, the evacuator settles after 21 rounds and then moves
nothing (it does not shuffle records between sparse chunks for ever),
every survivor reads back, and with four reader threads hammering
throughout, 128,805 reads over 1,464 live moves lose nothing and tear
nothing.  Green under ASan+UBSan with the allocator's debug ledger
armed; red with the evacuator mutated to a no-op.

**A member in trouble blinks instead of glowing (S210).**  The eased
3.8 s swell read as ambience on a wall of nine cards.  Both alarm states
now switch hard - `steps()`, no easing - with the border, the tint and a
wide halo together, in two saturated tokens used for the alarm alone:
failed at 0.7 s, attention at 1 s, so the worse state is the faster one.
A departed member does not blink; it is information, not an alarm.  With
`prefers-reduced-motion` the bright ring is held, not flashed.

**The header row aligns and the marks are legible (S208).**  The header
laid out on the text baseline, which an inline SVG does not have, so the
anchor, the compass, the select and the button each settled where their
own box put them.  It aligns on centre now; the controls are one group
that right-aligns while it fits and wraps as a whole rather than shedding
one item at a time; both marks share one declared size at 38 px, with the
compass redrawn for it instead of scaled up.

## 0.4.0-rc26 — 2026-09-22

rc25 plus the cluster-thread work: a wedged cluster thread reports
itself and a wedged master is replaced (S216, S218), replica apply on
more than one thread (S217, `apply_threads`, default off), and clocks
on the two full-keyspace walks (S212 step 1).  rc25 was green on both
runners.

**The two full-keyspace walks carry a clock (S212, step 1).**  The eager
repair sweep walks the whole table every cycle and the post-restart
reconcile probes every replayed key at 64 a second; both are cheap at
today's keyspace and O(N) in it, and nothing said what either cost.
`stats.cluster`, `/metrics` and the replication card now carry
`repl_sweep_ms` / `_scanned` / `_sent` / `_cycles` (the last completed
repair cycle) and `reconcile_ms` / `reconcile_pending` (the last
completed reconcile pass, and probes issued but not yet answered).
Durations, not rates.  Measured by the gate: a 4.4 MB backfill on a 4 MB
budget is two slices a tick apart, 10.0 s; 1,100 replayed keys
reconcile in 19.1 s.  These are the numbers that say when step 2 (range
digests) is due; at the fleet's keyspace they say it is not.

**Replica apply on more than one thread (S217).**  `[cluster]
apply_threads = N` (default 1 = as before) keeps the receiving thread on
the socket and the decrypt and hands each replicated record to one of N
apply threads by a hash of its key, over a ring of that thread's own -
so every record of a key is applied by one thread in arrival order, and
a full ring makes the receiver wait, never drop.  A group that wants an
ack (a backfill, a migration) is acked after its LAST record is applied,
whichever threads applied them.  An apply thread that wedges with
records queued on it is reported STALLED like the receiver (S216), and
the other threads carry on.  The receiver still decrypts everything, so
expect about 3x at most; a per-thread socket is a later step.
`stats.cluster`: `apply_threads`, `apply_dispatched`, `apply_blocks`,
`apply_backlog`.  Not measured on the bench rig yet: the number is
owed before it is quoted.
Gate: `clapplytest` (the rings without a daemon: 60,010 records over
4 threads, every key on one thread in order, nothing lost across
thousands of ring turns, a slow consumer makes the producer wait, a
group acked once after its last record; it found a deadlock in the
first cut's wrap arithmetic and it is red under round-robin dispatch -
5,952 versions backwards) and `test/applythreadstest.sh` (a two-node
eager fleet, 12,000 interleaved writes to 40 hot keys read on the
replica throughout, set/del/set keys, a 60 KB value, the restart
backfill acked 80 of 80, `apply_threads = 1` unchanged, and one of
four threads frozen: STALLED in ~2.2 s naming the apply thread, 301 of
400 applied by the other three, READY again on release).

**A wedged master is replaced, and yields when it comes back (S218).**
S216 made a wedged node say stalled; a wedged MASTER was still the
master, because the heartbeat watchdog re-sent its MASTER_ALIVE as
faithfully as its ALIVE - members saw a live master, nobody elected, no
joiner was admitted and no map was issued.  Withholding the claim was not
enough on its own: the election defers to the highest LIVE address, a
stalled node's watchdog keeps it live, and when the wedged master held
the highest address every member deferred to it for ever.  Now a stalled
node's claim is not re-sent, and the election does not consider a
stalled peer; the members elect after MASTER_DEAD_MS, and the old master
steps down on its return when it hears the higher term (the existing
dual-claim rule).  `cluster.elections` and `cluster.demotions` are shown
in `stats` (counted since S26, shown nowhere).
Gate: `test/stallmastertest.sh` - the founder, holding the highest
address, frozen for 24 s in a three-node fleet: a new master in ~9.3 s
(2 s limit + 8 s MASTER_DEAD), both members agreeing, a fourth node
admitted while the old master is still frozen, no sample with two running
masters, the old master yielding on release with one demotion, one
master and the old master's key readable on all four.  On the S216-only
daemon: no new master in 22 s and the joiner admitted only at release.

**A node whose cluster thread has wedged says so (S216).**  One thread
applies every replicated record, answers every pull and runs the control
plane; a second one keeps the heartbeat going while the first is
heads-down, by re-sending the last frame it built.  That is right for a
long duty and is also exactly how a thread that blocked for good stayed
a healthy-looking member: the last frame said READY, and it was re-sent
for ever.  The cluster thread now stamps a progress clock; the heartbeat
thread reads it, and after `[cluster] apply_stall_ms` (default 10 s,
0 = off) without progress the node reports a new state, **stalled**, with
the reason - in its own stats, and in the heartbeat the surviving thread
sends, so peers list it as stalled, the map records it, and routing
clients stop selecting it.  It stays a member (nothing re-shards), it
refuses nothing, and it returns to READY by itself once the thread has
moved steadily for the same interval.  A whole-process pause (SIGSTOP, a
VM pause) is not read as a stall.  New: `cluster.stalled`,
`stalled_for_ms`, `apply_stalls`, `apply_stall_ms_max`;
`perfcached_cluster_thread_stalled`, `..._stalls_total`,
`..._stall_max_ms`.  A peer built before this reads the unknown state as
"not ready", the safe direction.
Found by the gate: **a node never showed another node's reason.**  S186
put a reason byte in the heartbeat and the words in one place; nothing
filled the byte on the way out or kept it on the way in, so `members`
and the fleet page gave every other node an empty reason.  Both ends are
wired.
Gate: `test/stalltest.sh` with `test/threadfreeze.c`, which starts the
daemon as its child and stops exactly ONE of its threads with ptrace (no
hook in the production binary); the kernel's own view of the two threads
is asserted before anything else is read.  13 checks; on rc25, with the
thread frozen for 14 s, the node stays `ready` everywhere (6 red).  The
pause arm was proven able to fail: with the guard disabled it goes red.
Not covered: a wedged MASTER is made visible but not replaced - that
needs the election to discount a stalled node, and is filed separately.

## 0.4.0-rc25 — 2026-09-22

rc24 plus one durability fix and the status-page work.  **rc24's verdict:
green on GitHub; red on the project's own runner in `check` and
`check-asan`, for a full disk and not for the code** (below) - rc24
stays as a record, and nothing in it is withdrawn.

**A sync the device refused no longer moves the durable watermark
(S215).**  From an outside read of the source: the WAL's `fdatasync` was
unchecked, and four call sites then advanced `synced_seq` regardless -
so on EIO or ENOSPC the `sync` verb answered `{"synced":true}`, the
metric and the status page agreed, and the node carried on as a healthy
member.  The snapshot's directory `fsync` was unchecked too (and a
failed `open` of the directory skipped it silently), and `safe_marker`
moved behind it - the value the WAL consults before it overwrites a
segment, so a rename that never reached the disk licensed the WAL to
destroy the only other copy of those records.
One function now syncs and, only if the device agrees, advances the
watermark.  The first refusal FAILS the node - a member still, reads
still served, writes refused, out of client selection until restarted -
and the sync is **never retried**: after a failed `fsync` Linux marks
the pages clean, so a retry can succeed having written nothing.  The
suite counts the calls: the old daemon asked the refusing device 21
times and reported success each time; the new one asks once.
`sync` answers the failure at once rather than at its timeout.  A
snapshot whose directory entry cannot be made durable is not published:
`safe_marker` and `saves` stay, the WAL keeps its segments, and the next
snapshot publishes normally.
Found while reading around it, fixed with it:
- a refused **append** or a failed **rotation** set `enabled = 0` and
  nothing else - the node kept acknowledging writes that went nowhere
  and `sync` said "persistence is not configured".  Same event, same
  answer now.
- the failure reason S186 defined for a WAL-failed node was never set;
  such a node said "failed without recording a reason".  It says which
  now: acknowledged writes dropped, or the device refused.
- a node still JOINING swallowed the WAL's failure for good (a leftover
  exemption from when shedding meant a goodbye), including the startup
  overrun that is delivered late on purpose.
- a daemon **without** `[cluster]` never installed the callback, so none
  of the WAL's failures refused a single write.
- a failed snapshot was retried every 250 ms - a full table walk each
  time - for as long as the device was broken; it backs off 10 s (an
  operator's `save` does not wait) and is counted.
- CONTROL's sync is checked, logged and counted; it does not fail the
  node, deliberately (it is rewritten whole each time, so the next sync
  covers all of it).
New: `wal.storage_failed`, `wal.ctrl_errors`, `rdb.safe_marker`,
`rdb.save_errors`; `perfcached_wal_storage_failed`,
`perfcached_wal_control_errors_total`,
`perfcached_rdb_save_errors_total`.  Gate: `test/syncfailtest.sh` with
`test/syncfailshim.c`, an `LD_PRELOAD` device that refuses by descriptor
class and is armed at runtime - six arms, 45 checks, 27 of them red on
rc24; green under ASan+UBSan.  It rides check-fast.

**rc24's GitLab verdict was red for a full disk, not for the code.**
`check` and `check-asan` failed in `statedirtest` cases 4-5 ("no
start"): a default-sized WAL could not be provisioned with 416 MB free.
`waldroptest` and `readygatetest` each left their 65 MB per-run tree in
`/var/tmp` - no exit trap - and 43 runs of each since 09-16 came to
5.4 GB on the runner.  GitHub ran the same tag green.  Both suites clean
up after themselves now (a tree the caller names is still kept).

**The commands card stays put, and memory + backpressure share a cell
(S214).**  The status page re-orders its detail cards by height on every
redraw, so a card whose row count follows the traffic - commands in the
last five minutes, the slow log, unknown commands - changed rank as rows
came and went and jumped about the page.  Those cards are always their
full height now (8, 6 and 8 rows), and a command name never wraps.
The order of the cells is decided once and kept: it is re-decided only
when a cell appears or goes, or the page changes width.
Short cards share grid cells, from a table the page composes itself
from: memory over backpressure, memory budget over cluster auth,
commands over the slow log, and six more - 21 cards in 11 cells, each
about as tall as durability-wal, so no row stands half empty.

## 0.4.0-rc24 — 2026-09-22

Four more items from the outside reviews.  One is a capability: a
forwarded request now tells the client it is behind - a hint on the
native doors, a genuine `-MOVED` on the RESP door for cluster-aware
clients (libperfd 0.2.12).  Three are hardening with no behaviour
change: the transport refuses at its own boundaries, the allocator's
invariants can fail in a debug build, and the metadata walk hands its
callback a struct.  rc23 was certified by GitHub (3/3); its GitLab
pipeline was still in its ladder when this was cut.

**A forwarded request tells the client it is behind (RV-10).**  A
request on the wrong node is forwarded and answered, so a stale client
map costs a hop and never correctness - but nothing said so.  Native
doors: a reply the daemon had to forward under shard or spread carries
`"moved"` with the stamp of the map it routed by, and libperfd 0.2.12
re-learns its members behind that call when it holds another stamp -
one request, where it waited for the next push or `refresh_ms`.  RESP:
`[listen] resp_redirect = moved` answers a key this node does not own
with a genuine `-MOVED <slot> <ip>:<port>` and no forward, which is the
only thing that makes a cluster-aware Redis client refresh its slot
table; `forward` stays the default.  Gates: `movedhinttest` (caught up
after 2 requests; never, without the hint) and `respmovedtest`.

**The allocator's invariants can fail now (RV-3).**  The lock-free free
list's no-ABA argument rests on "a cell is never pushed twice while it
is on the list", and nothing checked it.  A debug build
(`-DPCACHE_ARENA_DEBUG`; the sanitizer ladder uses it) keeps a byte per
cell saying where it is and checks every move - a double free, or a
second push home, aborts where it happens - and `pcache_arena_census()`
proves at a quiet moment that every cell is in exactly one place and the
live figure adds up.  A release build compiles none of it: the arena's
object is the same size to the byte.  `arenadbgtest`: a six-thread storm
with cells changing hands, then the three things that must fail.

**The transport refuses at its own boundaries (RV-2).**  A record one
byte over the Noise ceiling encrypted to 65,536 bytes - more than the
16-bit record length can carry - and decrypted again; nothing reached it
because every caller chunks, which is how the boundary came to belong to
the callers.  `cs_encrypt`/`cs_decrypt` refuse now, a refusal never moves
the nonce, and the handshake writers check the result (they fed it to
the hash as a length).  The nonce counter wrapped to 0 after 2^64-1 -
nonce reuse under one key, unreachable in practice - and now stops at
2^64-2 as Noise specifies: the connection is over, a reconnect re-keys.
Argon2id stays at the INTERACTIVE limits, deliberately: the PSK is
derived once per secret, not per connection, and the limits are part of
the wire contract.  New gates: `noiseboundtest` (6 of 10 red before),
and `plaintextdoortest` - no door off loopback reaches the plaintext
passthrough (mutation-checked).

**The metadata walk hands its callback a struct (RV-8).**  The callback
took expiry, write tick, flags and version as four positional
arguments, each of which converts into the next silently - the shape
behind S173, where a walk passed them in the record's order and
mis-reported every record it emitted for a year.  `struct
pcache_rec_meta` is public now and every walk consumer takes it; a
callback on the old shape does not compile.  No behaviour change;
`legmetatest` still catches a crossed field inside the walk (mutation
checked: 304 of 400 records wrong, red).

## 0.4.0-rc23 — 2026-09-21

Two items from the outside reviews, both daemon-side: a delete that a
balancer lands on a node without the key now finds the record where it
lives, and the arena's ceiling is a reservation rather than a check.
rc22 was certified on both runners (GitLab 5/5, GitHub 3/3).

**A delete that lands on a node without the key looks where the record
is (RV-11).**  A write taken by one node is acknowledged there while
its push to the peers batches for up to 3 ms; a delete of the same key
that a balancer lands on another node inside that window found nothing,
answered "absent" with no tombstone, and the push then put the record on
every member.  The client cannot carry the ordering across - the one
that does this here speaks Redis - so the node asks the fleet, the node
that HOLDS the record deletes it and tombstones under its own clock
(which has observed the record's version, so the tombstone is newer than
every copy, landed or in flight), and the client is told what really
happened.  Spread unicasts its best-ranked holder; store had the same
hole without the push.  A repeated miss costs nothing (negative cache);
`pull = 0` keeps a delete local, as it keeps a read.  Also: a probe that
became a forward now records the answering version, so a second holder's
yes is no longer read as a birth race.  Gate: `test/delprobetest.sh` -
6 of 8 red before (0 of 20 deletes true, the keys on every node at every
sample), 8/8 after.

**The arena's ceiling is a reservation, not a check (RV-1).**  A region
carve past the huge-page reservation tested `arena_cap_mb` with a
relaxed read outside the arena lock, allocated, and only then took the
lock to add its bytes - so two carves near the cap (a `create` on a
worker, the splitter on the maintenance thread) could both pass and
both land, past the figure an operator reads as a bound.  The carve now
takes its bytes under the lock before allocating and returns them if
the allocation fails, and compares exactly (the warm-slot credit is an
index preflight's, not a carve's).  `capracetest`: 8 threads at a
ceiling with room for one, 300 ceilings - 192-226 rounds over before
(worst by 650 KB), none after.

## 0.4.0-rc22 — 2026-09-21

One daemon fix, found on the fleet minutes after rc21 went live: a
device record alive on one node with a fresh TTL and gone from the
other two, because a TTL re-arm on a copy stayed on that node.  rc21
was certified by GitHub (3/3); its GitLab pipeline was cancelled in
favour of this tag on the one runner.

**A TTL re-arm is a write and takes the write's road (S213).**  Found
on the fleet minutes after rc21: a device record alive on ONE node with
a fresh TTL and gone from the other two.  `expire` touched whatever the
local table held - a passive copy was re-armed and left passive, so the
author's clock ran out while the copy lived on; that is the RGS
re-registration path (`EXISTS` yes, then only `EXPIRE`, through a
balancer), and the de-registration's `JSON.DEL` landing on a node
without the record deleted nothing - a ghost registration until the TTL
ran out.  It was also the one mutating verb without a mode branch: under
shard, proxy and spread a re-arm for a key held elsewhere answered
false.  Now a re-arm on a copy adopts it (the record becomes this
node's, in the WAL whole, pushed as authored), and a re-arm for a key
held elsewhere forwards like set/del/add do - shard to the owner, proxy
to the locator's holder or by probe, spread to the best-ranked holder.
`add/sub` clears the passive flag on the record it accumulates into.
Gate: `test/expiretest.sh`, five fleets, every node asserted: 7 of 10
red on rc21, 10/10 after.  DESIGN 12fg.

## 0.4.0-rc21 — 2026-09-21

One library fix and two filings.  A `jdel` or `jincr` right behind a
routed `jset` took a different road than the `jset` and missed it on an
eager fleet; libperfd 0.2.11 sends every keyed verb the same way.  The
daemon is unchanged from rc20 apart from its version.

**A `jdel` or `jincr` right behind a routed `jset` no longer misses it
(S211).**  With per-key routing on, every keyed verb went to the key's
owner over its own link except `jdel` and `jincr`, which went over the
primary link.  On an eager fleet the jset was applied at the owner and
its push to the primary batched for up to 3 ms, so a jdel arriving at
the primary meanwhile found nothing, deleted nothing, and the document
then landed on every member; a jincr answered "no such key".  Both now
route like the rest; `roundtrip_key`'s dead-standby fallback refuses to
replay a non-idempotent verb (it re-sent anything); `jset`/`jdel` are on
the replay list.  libperfd only - the daemon is unchanged.  Gate:
`test/routedpairtest.sh` (4 of 8 red before, 8/8 after), in check-fast.
The unrouted-client form of the window is filed as RV-11.

**libperfd 0.2.11.**  The S211 routing; `roundtrip_key`'s dead-standby
fallback refuses to replay a non-idempotent verb; `jset`/`jdel` on the
replay list beside `jget`; the keyed-verb rule written at
`roundtrip_key`.  No API change.

**Filed, not built.**  RV-11: a delete that reaches a different node
than the write it follows, for clients libperfd does not route - the
client-carried version hint.  S212: the sweeps at scale - the eager
repair is watermark-incremental on the wire, the post-restart reconcile
is the O(N) one at 64 probes/s; clocks first, select-only range digests
later.  DESIGN 12fe addendum, RV-11, 12ff.

## 0.4.0-rc20 — 2026-09-21

Two things that let data lie: a set and its delete crossing on the
wire so the key stayed on the peers, and a dashboard that would have run
whatever a key carried.  Both closed with tests that run in seconds, and
the tombstone fix proven on the fleet before and after.  The page also
wears the anchor now - favicon and header - and the compass where a
client finds a node.  rc19 was the first tag green on both runners since
rc7; this tree passed GitHub's full ladder before it was cut.

**A write and a delete microseconds apart no longer leave the key on
the peers.**  The write-path push batches for up to 3 ms while a
tombstone goes at once, so a set and its delete closer than the flush
reached the peers in the wrong order: the tombstone removed nothing, the
batched set landed after it, and the key resurrected on every peer for
good - the sweep repairs presence, not absence.  Found gating the
staging RGSs, whose de-registration is exactly set-then-delete.  A
tombstone now carries the delete's version (a fresh Lamport tick, newer
than the write it supersedes); the peer keeps it with the negative entry
and refuses any copy of the key that is not newer - on the datagram, the
bulk and the pull paths - and does not remove a record newer than a
late tombstone.  The frame grew by eight bytes at the tail, which an
older receiver ignores and an older sender leaves off (read as
unversioned).  `recv_tombstoned` counts each resurrection that did not
happen.  `test/deltombtest.sh` drives the pair through one connection
with no delay, both delete paths, and asserts on every member: 3 of 6 on
the unfixed daemon, 6 of 6 after.  The guard lasts `tombstone_ms`.

**What goes into the cache cannot run in the dashboard.**  Two layers.
Every cache-fed string the page renders - keys, values, client names,
command names, slow-log arguments - passes `esc()`, which now escapes
the single quote too.  And the page carries a Content-Security-Policy
whose `script-src` is the SHA-256 of its one inline script, computed
from the page itself at first use so it can never drift: an injected
script, inline or remote, does not run even if a sink is missed;
`connect-src 'self'` and `img-src data:` keep the page from talking to
anywhere but this node; `default-src 'none'` closes the rest.  Every
HTTP answer carries nosniff, no-referrer and no-framing headers.
`test/xsstest.sh` stores hostile keys, values, a client name and an
unknown command through the doors, RUNS the page and scans everything
it wrote into the DOM, and checks the policy's hash against the served
script; with `esc()` made the identity it reports 51 raw hits, so it
can fail.

**The compass rendered black and unsized - a CSS rule inside another.**
The `.brand` rule spans two source lines and the anchor's and compass's
rules had been inserted between them, inside the open brace.  Chrome's
recovery let the anchor through and dropped the compass.  The rules
follow the closing brace now, and httptest parses the served stylesheet
and refuses a rule that opens inside another (it named all ten).

**The compass marks the page's client side.**  The anchor's sibling -
four cells, lit north, a red needle - sits at the anchor's size before the node switch, in place of the word
"viewing" - the mark says it, and the word had pushed the row's last
item onto a second line
("viewing", which is literally go to another node) and in the clients
panel's header, where every row is something that found this node.
Inline at 18 px on the page's tokens, with one new token for the
needle's north red.  Not beside the anchor: one mark per surface.

**The anchor stands beside the wordmark in the page header**, thirty
pixels, inline so its inks follow the page's own tokens: ink for the
strokes, the dim ink for the three kept cells, the accent for the found
cell - the wordmark's colour, so the two read as one thing.

**The status page has a favicon: the anchor.**  `contrib/brand/` holds
the marks decided on 2026-09-21 - the four-cell anchor for the daemon,
its compass sibling for the client side, the lockup - and the anchor's
small cut is inlined into the page head as an SVG data URI: no second
route, no second request, light and dark from the SVG's own
`prefers-color-scheme`.  httptest decodes it and checks it is the
anchor.  1.7 KB on a 49 KB page.

## 0.4.0-rc19 — 2026-09-21

rc18 plus one test.  The daemon is rc18's apart from the version
string (`git diff v0.4.0-rc18 -- src lib` touches `src/version.h` and
nothing else); the cut exists so both runners can certify a tag that
rc17's GitHub `check` could not.

`failovertest` accepts both answers a fleet has for a state-less node
bouncing at the same address: a fresh id pushed to the client when the
join lands while the dead incumbent still reads "up", or the reserved
id handed back by address (S90) when it has lapsed.  It asserted only
the first, which GitLab's runner produces in 200 ms; GitHub's took the
second and the step failed rc17 there.  What it asserts now is the
property: one live entry at that port, serving.  No daemon change.

## 0.4.0-rc18 — 2026-09-21

A native-door `jdel` at the root that never removed the key - found
gating the staging switchover, which it held up - and the three items
the rc17 reviews put first: one bucket walk with a written contract,
a duplicate ack that credited a counter, and the design notes S195
to S200 written down.  proxytest is green on both runners again;
rc17's GitHub `check` failed a same-address-restart bound in
failovertest that passed on GitLab and under ASan (RV-9, filed).

**A native-door `jdel` at the root removes the key.**  It answered
-32022 "bad path" and left the key - on a fresh document, an edited
one, an implicit path and a plain value alike - because the native
door sent a root delete through `pc_jp_del`, which has always refused
the root ("deleting the root is verb del"), while the RESP door's
`JSON.DEL key` special-cased `$` into a key removal.  Found gating
`cachedb_perfd` for the staging RGSs, whose de-registration is
`JSON.DEL <key>`: on the native door the record survived and a
de-registered device kept reading as registered, with the driver
reporting success.  A root `jdel` is now the same call as the native
`del` - forwarding and parking on a proxy or shard collection included
- and `test/jdeltest.sh` runs the brief's matrix on both doors in one
daemon (3 of 10 before, 10 of 10 after).  A sub-path `jdel` is still
a document edit.

**A duplicate ack no longer credits `migrated_out`.**  A re-sent batch
whose first ack arrived after all retires no window slot - `clmig_ack`
says so - but the counter was credited regardless, so one transfer of
331 records read 314 on one runner and 330 on another depending on the
retransmits.  A late ack for a slot the deadline already wrote off as
lost credits nothing either: one event, one counter.

**One bucket walk.**  The slot loop existed three times - the two
metadata walks and the cursored scan - and S198's guard reached two of
them.  The third is the walk a collection drop removes through, where
S69 had met the same skip ("17,452 of 20,000 removed") and worked around
it with passes until nothing moves.  Every walker now goes through
`walk_bucket()`, the walk contract is written down in the header (a
callback may remove or replace the record it was handed, nothing else),
the leg node's offset-0 rule is a static assertion instead of a comment,
and a keys-only scan is keys-only in the overflow leg too - it handed
values over there regardless.  `test/scanremovetest.c` pins the drop's
shape: one pass, 400 of 400, table empty.

Correction to the rc17 note: spread's surplus reclaim collects
candidates inside the walk and removes them after it, so it never had
the S198 hole.  The proxy migration was the only in-walk remover.

## 0.4.0-rc17 — 2026-09-20

One walk and one page.  A table walk whose callback removed the record
it was handed skipped half of every full bucket, which is why rc14,
rc15 and rc16 were red on proxytest on both runners; and the status
page could not render since rc16 and blamed the daemon for it.  Both
are pinned by tests that run in milliseconds rather than minutes.

**A walk that removes as it goes skipped half of every full bucket.**
`pcache_ht_remove` compacts a bucket by moving its last record into the
freed slot, so a walk whose callback removes the record it was just
handed - a proxy migration stubs every victim from inside the walk -
left the slot with an unvisited record in it and moved on.  proxytest
on rc16 moved 236 of 400 small records off a 16-bucket table and never
the rest, on every runner, every time.  The two metadata walks now look
at a slot again when its cell changed under the callback;
`test/legwalkremovetest.c` pins it in milliseconds (352 of 400 before,
400 after).  Spread's surplus reclaim removes from inside the same walk
and had the same hole.

**The status page could not render at all.**  The five-minute helpers
went into `drawCmds`'s own scope while `wmax`/`wspan` - out at top level
- call them, so every poll threw `ReferenceError: wq is not defined` in
the row loop, before the table was written.  The page then blamed the
daemon for it: `get()` wrapped the whole callback in try/catch and
reported a failed RENDER as a failed fetch, so a node that had answered
perfectly was painted "this node did not answer" with a half-drawn page
behind it.  Only the JSON parse is guarded now; a render that fails says
so, by name, in the banner.

A verb is named one way everywhere: the verb, with its dialect beside
it.  The "commands by calls" card printed the composite name instead -
`json:get`, and `json.get` for a Redis name with a dot - which is what
the dialect field exists to stop, and its figures are now the same
five-minute window as the rest of the plane.

`test/pagejstest.sh` RUNS the page's script against the daemon's own
/stats under a stub DOM and asserts what a browser would see.  Every
page assertion before it was a grep for a substring, and a substring
cannot tell you a function is out of scope.

The commands table lists the verbs the last five minutes saw, and only
those: a verb falls out of the table as it falls out of the window,
and an empty table says "nothing called in the last 5 min" rather
than "no commands yet" on a daemon that has served millions.  The
window's slowest call now spans the same slots as its call count -
the max scanned one ring slot more than the count, so a verb last
used six minutes ago reported no calls beside a 250 us slowest call,
and sorted above every busy verb on it.  `test/winmaxtest.sh` waits
one roll and pins the two together.

A node restarting under a deploy drops one poll, and one miss painted
"this node did not answer" over a page whose cards were three seconds
old.  The banner waits for a second consecutive miss and says how long
the node has been silent.

## 0.4.0-rc16 — 2026-09-20

Two things the fleet showed under load: a bulk load stalled on memory
the daemon could have had ready, and a command's name could not say
which door it came from.  Verified on the fleet before tagging - a
62,000-record load at 98,000 writes/s with no write-path commits at
all, and the whole keyspace expiring on a 60 second TTL with the
expiry counted correctly.

The status page stopped lying about three things, and a mass expiry
stopped reporting nothing.  Every card figure is now the last five
minutes; a node that leaves keeps a card; every state says why; and a
collection keeps its counters across a resize.

### The status page shows the last five minutes, not the life of the process

Every figure on the cards was cumulative since the daemon started, so a
mean drifted toward the past, a p99 was diluted by every quiet second,
and one slow call during a bulk load owned the "slowest call" column
for good. The command table's figures are now a five-minute window kept
by the daemon, and the pull, forward, replication and collection
counters are differenced against the history the page already keeps.
Gauges - entries, buckets, load, the overflow leg, held bytes - stay
absolute, and `/metrics` keeps the cumulative counters Prometheus needs.

### The fleet view keeps a node that has left, and says why a node is not ready

A node that shuts down cleanly used to vanish from every other node's
fleet view, so a rolling restart looked like a cluster that had shrunk.
It now keeps a grey card saying when it left and how much longer its id
is reserved for it - the card goes when the reservation lapses, so a
decommissioned node is visible for as long as it still holds an id. And
every state that is not "ready" now carries a short reason on the card,
where before only one specific failure did.

### A pull miss is no longer flagged as a fault

Asking a peer for a key it does not have is an ordinary cache miss, and
the card had been highlighting it amber for the life of the process.
Timeouts, which are faults, keep their highlight.

### Bulk loads stop stalling on memory the daemon could have had ready

The arena keeps a window of memory committed ahead of where it is
allocating, so a write that crosses a boundary does not have to ask the
kernel for 2 MB itself. That window was refilled once a second, and a
bulk load crosses a boundary every 50 milliseconds - so twenty of those
crossings landed on the worker holding a client's write, each costing
milliseconds. The refill now runs ten times a second on the same
maintenance thread: measured over a 62,000-record load, twenty-one such
stalls became six, and the load's 99.9th percentile fell from 32 to
21 microseconds.

### Command rows say which door they came from

A command's name could not distinguish the native JSON door's `get`
from a RESP client sending `JSON.GET` - Redis command names may contain
dots. `/stats` now carries the dialect and the verb separately and the
status page shows "get (json)", while the joined name uses a colon so
that scrapers have an unambiguous key.

## 0.4.0-rc15 — 2026-09-20

Observability the operator asked for, and two counters that were lying.
Latency percentiles are estimates of the value rather than the power of
two above it; a collection's lifetime figures survive a resize, which is
why 62,000 keys could expire and the page report `expired` as 0; the
commands table folds to its slowest verb; and a node that marks itself
FAILED now says why, on every node's card.

### `/stats` stayed valid JSON when a client stores binary data

The slow log keeps the arguments of the commands it samples, and a
client row keeps the name a client announced - so storing a binary value
slow enough to be sampled put raw bytes into the JSON body and made the
whole document fail a strict parser, which is what a monitoring
integration is. Those fields now show an unrepresentable byte as \ufffd.
The paths that need the exact bytes, like key and value listings, keep
their base64 encoding.

### Four size classes where the records actually are

A record rounds up to a cell class, and the classes stepped by about 1.5
from 64 bytes up. Measured against a 61,944-record production keyspace
whose median record is 304 bytes, 72% of all rounding waste fell into
two classes - and because those classes pack their 256 KB chunks
densely, the waste was whole chunks rather than spare bytes. Adding
320, 448, 640 and 896 cuts what the arena holds for that keyspace from
51.5 MB to 47.5 MB, a 7.8% saving, with no change to the allocation
path. Denser ladders across the whole range measured worse, not better:
every class in use pins at least one chunk, so the extra floor exceeded
the saving.

## 0.4.0-rc14 — 2026-09-20

Memory and observability, on a fleet that has been quiet since rc13.
The size-class ladder gains four rungs where a measured production
keyspace actually sits, which takes 7.8% off what the arena holds for
it; `/stats` stays valid JSON when a client stores binary data; and
`growtest` stops failing on a tolerance that predated the arena's
commit-ahead window - a suite that only runs on tags, which is why it
went unseen through rc9 to rc13.

## 0.4.0-rc13 — 2026-09-20

rc12 closed one half of the replication loop and this closes the other:
a bulk transfer reported how many records it STORED, and a record the
peer already holds is refused as already-present, so a batch that
arrived perfectly read as a failed transfer and was offered again every
sweep.  Also: the status page's load column showed six times the real
load factor, and now carries the operator's thresholds.

### A cluster kept re-sending records its peers already had

A node reports back how many records of a bulk transfer it stored, and a
record the peer already holds is correctly refused as already-present -
so a transfer that arrived perfectly reported nothing stored, the sender
recorded it as unconfirmed, and offered the same records again on the
next sweep, indefinitely.  The reply now counts records the peer HOLDS
as a result of the transfer, which is what the sender acts on; the
statistics still count only what was new.

### The status page's load column read six times too high

A bucket holds six records, and the column divided by buckets alone, so
a collection at half its capacity displayed "300%".  It now shows the
real load factor and turns amber past 70% and red past 90%, where the
overflow leg starts costing every lookup that misses its bucket.

## 0.4.0-rc12 — 2026-09-20

**rc11 shipped a loop and this tag closes it.**  Oversized-value
replication (new in rc10, never released green) reported a busy
transfer queue through a counter shared by every peer, and then refused
to advance any peer's replication mark while it had moved.  The queue
held one batch and a sweep offers one per peer, so on a three-node
fleet the marks stopped advancing permanently and each node re-sent its
whole keyspace to both peers every sweep - measured at ~1,900 records a
second per node, all correctly refused as already-held.  Found on the
fleet within an hour of rc11 landing.

### Fixed: a cluster could re-send its whole keyspace for ever

A node that could not hand an oversized value to the bulk transfer
thread recorded the fact in a counter shared by every peer, and then
refused to advance ANY peer's replication mark while that counter had
moved.  Since the counter only ever climbs and the transfer queue held
one batch at a time - so a sweep with two peers always refused one - the
marks stopped advancing permanently, and each node re-sent its entire
keyspace to both peers every sweep, all of it correctly refused as
already-held.  Measured on a three-node fleet holding 63,969 records:
~1,900 records a second per node, indefinitely, from the moment a value
larger than a datagram was stored.

The transfer queue now holds a batch per peer, and a refused hand-off is
reported against the one peer it concerns, for the one cycle it
concerns.  Introduced earlier the same day with oversized-value
replication; no released version carried it.

## 0.4.0-rc11 — 2026-09-20

**rc10 was RED and is superseded by this tag.**  Its pipeline failed
`check` and `check-asan` on the same suite - `tailtest`, which measures
the arena mapping's residency against what the arena holds - and so, it
turned out, had rc9's, unread.  The defect is real and is fixed here:
the commit-ahead window S166 keeps above the chunk frontier was never
given back on a node that had gone quiet, so 10 MB stayed resident for
the life of the process.  This tag also makes the overflow leg's drain
thirty-five times faster by bounding it in time rather than in records.

### The overflow leg empties in seconds, not a minute

The drain's budget was a flat count per tick and was named for chains,
though it moves one record per step - so a bulk load's leg came back at
about 630 records a second, and the table did not widen until it had.
It is a time slice now, and a pass boundary: measured on 40,000 keys
loaded at 225,000/s, the leg settles in 1.5 s where it took over a
minute.

### The arena gives back the window it keeps ahead of itself

A node keeps a few groups committed above its allocation frontier so a
carve does not have to populate memory on the write path.  Nothing ever
returned them once the node went quiet, so an idle daemon held about
10 MB of resident memory it could not use and would not release.

## 0.4.0-rc10 — 2026-09-20 (RED - superseded by rc11)

**The overflow leg, from three sides.**  A key whose bucket is full
lives in a shared leg, and a production keyspace loaded into a
too-small table put 62% of its records there - permanently, because
nothing ever moved one back.  This tag adds the drain that does, fixes
a walk that reported every leg record's metadata rotated by one field
(which had the fleet re-sending 6.5 million copies nobody would take),
and lets a leg that stays occupied ask for a wider table.  It also
raises the largest value a collection will take from 64 KB to 256 KB,
which needed the log and the cluster's TCP plane to follow.

Measured on the test fleet and on 40,000-key loads; every item carries
its numbers below.

### A collection whose overflow leg stays busy now widens itself

A key whose bucket is full lives in a shared overflow leg, where every
lookup that misses the bucket walks a chain under one lock.  The
maintenance tick already puts back what it can, but a record whose
bucket is still full has nowhere to go - at the default load factor that
is about 8.5% of a bulk-loaded keyspace, paying a chain walk for ever.
That figure is now a signal: when a full pass over the leg cannot
re-home more than `leg_stuck_pct` of the entries (default 1%), the table
is split past `grow_at_pct` toward `grow_floor_pct` (default 50%), which
is the only thing that helps.  Measured on 40,000 keys loaded at
225,000/s into a deliberately small table: 8.5% of records left in the
leg before, 3.9% after, for about 295 KB more index.  A collection whose
leg is quiet grows exactly as it did, and `grow_floor_pct = 0` turns it
off entirely.  `/stats` gains `leg_stuck` and `leg_splits` per
collection.

### Values up to 256 KB, and they replicate

The largest value a collection would take was about 64 KB - the arena's
top size class.  Checked against a production keyspace of 61,944 keys,
19 values did not fit and the largest was 218,265 bytes, so the ceiling
was a product limit rather than a theoretical one.  It is now 262,080
bytes: one cell per 256 KB chunk, which is the largest a chunk can hold.

Two quieter limits moved with it, and both would have been silent.  The
write-ahead log kept a record cap of its own that sat between the old
ceiling and the new one, and a record above it is dropped AFTER the
client is told the write succeeded - a big value would have been
acknowledged and then absent after a restart.  And eager replication
skipped any value too large for a datagram, so on a cluster a big value
would have lived on the node that took the write and read as a miss
everywhere else; those copies now travel on the TCP plane the daemon
already used for moving records between nodes, arriving within one sweep
of the write rather than immediately.  `/stats` gains
`repl_bulk_lost`: unconfirmed oversized batches, which hold a sweep's
progress mark where it is so the next sweep offers them again.

### Records held in a collection's overflow leg were replicated over and over

A node kept sending peers records they already had, which the peers
refused every time: on the test fleet two of the three nodes had each
sent 6.58 million such records, every sweep, for as long as the leg was
occupied.  The walk that feeds replication reported a leg record's
version as its write tick, the tick's low byte as its flags and its
flags as its version - one call passing four values in the wrong order,
each converting silently - so the sweep saw a record that was newer than
anything the peer could have and sent it with a version of 2, which the
peer refused as older.  The same walk feeds the snapshot writer, the
collection resize and the bootstrap, all of which read those fields, so
a leg record could also be persisted or copied with the wrong version
and flags.  Records in buckets were never affected, and a table sized so
that its leg stays empty never reached the path - which is why it
survived since 0.2.0.

### Records that overflow a collection's buckets are put back

A key lives in its bucket or, when that bucket is full, in a shared
overflow leg where every lookup that misses the bucket has to find it
under one lock.  Until now nothing ever moved a record back: growing the
table made room in the buckets but never looked at the leg.  A bulk load
is the worst case - table growth runs once a second in bounded chunks,
so 62,000 keys written in under a second left 62% of the records in the
leg for good, and the collection stayed slow (KEYS 15.7 ms, GET 44 us,
against 6.8 ms and 2.6 us once sized correctly).  The maintenance tick
now walks the leg and returns every record whose bucket has room, and
`/stats` carries `leg_drained` beside the leg's size.  What remains in
the leg is what the load factor implies - a table at the default 75%
keeps about 7% of records there - which is a separate decision.

## 0.4.0-rc9 — 2026-09-20

rc8's contents with S168's index-zone window withdrawn.  rc8 was RED on
its snapshot runs - `regiontest` failed because the window re-committed
memory below the index frontier while the give-back returned a group
above it, so process RSS rose where the test requires it to fall - and
rc8 is superseded by this release.  The index zone commits its own
groups again, on the maintenance thread that splits the table, and the
cost is counted apart as `commits_index`.  Everything else in rc8 stands:
the arena still commits ahead of the WRITE path (S166), the `pin` and
`huge_pages` knobs, the slowest-calls table, the PHP client's removal.
libperfd is unchanged (0.2.10).  Everything under 0.4.0-rc8 below is in
rc9.

## 0.4.0-rc8 — 2026-09-20 (RED - superseded by rc9)

rc7 with the arena's growth taken off the paths that serve clients, two
new memory knobs, and the page's slowest-calls view made readable.

A write that grew the arena into a fresh 2 MB group populated that group
on the spot, on the worker holding the client's request - 1-4 ms on an
idle 16-vCPU host, seen as a 10.1 ms `SET` on a 4-vCPU test node.  The
maintenance tick now keeps memory committed ahead of both of the arena's
frontiers, so neither a client's write nor a table split pays it, and
`/stats` carries `commits_ahead` / `commits_inline` so the remaining
cases are countable.  Only nodes whose arena grows and shrinks were
affected: a pinned or fixed arena never re-faults.

`[memory] pin = require` refuses to start a node that cannot lock its
arena, naming the limit that refused - an unprivileged container's
`RLIMIT_MEMLOCK` beats the unit's `LimitMEMLOCK`.  `[memory] huge_pages
= off` forces the plain 4K tier, which previously needed a global sysfs
change.  `/stats` and `/metrics` gain `locked_bytes` and `pin_lost`.

The dashboard's slowest-calls card became a table under the collections
table: in a narrow card the figures squeezed the command name to
nothing.  The pure-PHP client is removed; it never rode libperfd.
libperfd is unchanged (0.2.10).  Everything under 0.4.0-rc7 below is in
rc8.

### `[memory] pin` and `[memory] huge_pages`

The arena is locked in RAM whenever it can be, and a failed lock was
only a warning - so a node could believe it was resident while it was
swappable.  `pin = require` refuses the start instead, naming
`RLIMIT_MEMLOCK` and the fact that an unprivileged container's limit
beats the unit's `LimitMEMLOCK`.  `pin = auto` (the default) keeps the
old behaviour.

`huge_pages = off` forces the plain 4K tier: no `MAP_HUGETLB`, no
`MADV_HUGEPAGE`, and THP disabled for the process, which previously
needed a global sysfs change or a wrapper.  Growing the arena then costs
many cheap faults instead of one 2 MB fault that may wait for
compaction.  `huge_pages = auto` (the default) probes as before.

`/stats` and `/metrics` gain `locked_bytes` - 0 means the arena is
swappable - and `pin_lost`, which is 1 when a group could not be locked
after a pinned start, a half-pinned state nothing reported before.

### The arena commits memory ahead of the write path, not during it

A write that grew the arena into a fresh 2 MB group populated that group
on the spot, on the worker holding the client's request: measured 1-4 ms
on an idle 16-vCPU host and seen as a 10.1 ms `SET` on a 4-vCPU test
node.  It showed only when the keyspace GREW, and only on nodes whose
arena is not pinned - a pinned arena stays resident, so it never
re-faults.  The maintenance tick now keeps 8 MB committed ahead of the
frontier and the give-back leaves that window alone, so the write path
finds the memory already there.  `/stats` and `/metrics` carry
`commits_ahead` and `commits_inline`: the second counts groups a write
still had to commit itself, which happens when growth outruns the 1 Hz
tick.  Nodes with a fixed, pinned arena are unaffected either way.

The index side of the arena is not windowed: a table split commits its
own groups, on the maintenance thread rather than on a client's write.
That cost is counted separately as `commits_index`, so the two can never
be confused.

### The slowest calls are a table under collections, with the command name

The slowest-calls card sat in the cluster-plane grid, whose columns are
as narrow as 184 px.  Its figures ("p99 128 us - max <= 512 us") took
the row's width and the command name, which clips with an ellipsis, was
squeezed to nothing: every row read as numbers with no verb attached.
It is now a table of its own under the collections table - command,
calls, mean, p50, p99, p99.9 and the slowest call - sorted with the
worst call first, and it scrolls sideways inside its own container on a
narrow screen rather than dropping the name.  Every command that has
been called is listed, not the top eight.  The "commands by calls" card
and the slow log's tail stay in the plane.

### Removed: the pure-PHP client

`lang/php/Perfcached.php` and its test are removed.  It spoke the JSON
dialect over its own Noise implementation - one connection, no member
list, no failover and no client-side routing - and had not been touched
since August; it never rode `libperfd`, so it could not gain what the
other clients get.  There is no PHP client in the tree for now; the
replacement over `libperfd` is planned but not started.  PHP
applications can speak the RESP door with any Redis client in the
meantime.
