...
The same window now covers the index side of the arena, where a table
split allocates from the other end.  Those commits were paid by the
maintenance thread, inside the slice the expiry sweep and the reclaim
tick wait behind, rather than by a client.
