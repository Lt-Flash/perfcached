...
P = R()
for pat in ("h[ae]llo", "h[^e]llo", "h\\*llo", "h?llo", "[a-c]x", "a*c"):
...
for i in range(200):
    try:
        if B.cmd("PUBLISH", "big", payload) != 1: break
...
time.sleep(0.5)
st = json.load(urllib.request.urlopen("http://127.0.0.1:18471/stats", timeout=5)).get("pubsub", {})
ok(st.get("slow_kills", 0) >= 1, "and counted as a slow kill (%r)" % st.get("slow_kills"))
