...
# await_members <n> <count> <secs>: poll node n's member count - a fixed
# sleep sampled once is what GitHub's sanitized runner fails first
await_members() {
	i=0
	while [ $i -lt $(( $3 * 2 )) ]; do
		[ "$(members "$1")" = "$2" ] && return 0
		sleep 0.5; i=$((i+1))
	done
	return 1
}
...
await_members 1 2 20; M=$(members 1)
...
await_members 1 3 20; M=$(members 1)
