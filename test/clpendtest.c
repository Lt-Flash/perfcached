...
int main(void)
{
	...
	/* ---- B6. S212: occupancy by kind ------------------------------- */
	clpend_init(&P, CAP);
	{
		struct pending *r1, *r2, *r3;
		int why;

		chk(clpend_used_kind(&P, PC_DONE_RECONCILE) == 0, "B6 no reconcile probes: 0");
		clpend_alloc(&P, PC_DONE_RECONCILE, 1, 0, -1, now, TMO, &r1, &why);
		clpend_alloc(&P, PC_DONE_RECONCILE, 1, 0, -1, now, TMO, &r2, &why);
		clpend_alloc(&P, PC_DONE_PULL, 1, 0, 0, now, TMO, &r3, &why);
		chk(clpend_used_kind(&P, PC_DONE_RECONCILE) == 2 &&
			clpend_used_kind(&P, PC_DONE_PULL) == 1 && clpend_used(&P) == 3,
			"B6 two reconcile probes and one pull: the gauge counts by kind");
		clpend_release(&P, r1);
		chk(clpend_used_kind(&P, PC_DONE_RECONCILE) == 1, "B6 a release takes one off its kind");
		clpend_release(&P, r1);
		chk(clpend_used_kind(&P, PC_DONE_RECONCILE) == 1, "B6 a second release of the same slot is a no-op for the kind too");
		clpend_expire(&P, now + TMO + 1);
		chk(clpend_used_kind(&P, PC_DONE_RECONCILE) == 0 &&
			clpend_used_kind(&P, PC_DONE_PULL) == 0,
			"B6 expiry releases by kind as well");
		chk(clpend_used_kind(&P, 99) == 0 && clpend_used_kind(&P, -1) == 0,
			"B6 a kind outside the table reads 0");
	}
	clpend_fini(&P);
	...
	clpend_init(&P, CAP);
	{
		...
		r = clpend_answer(&P, req, 1, 42, 0, 9999, &a);
		...
		r = clpend_answer(&P, req, 1, 9, 0, 9999, &a);
		...
		/* ---- H1c  TWO HOLDERS IS NOT AUTOMATICALLY A CONFLICT ----
		 *
		 * This is the bug that cost the 245-247 fleet 78% of its
		 * redundancy on 2026-09-14, upgrading 0.3.5 -> 0.3.7-rc4.
		 * A second positive used to mean "birth race" on its own.
		 * In EAGER mode two holders is the NORMAL state - every key
		 * is on every node by design - so after a simultaneous cold
		 * restart, where every node recovers the same keyspace from
		 * its own RDB, every reconcile probe drew two positives and
		 * all but one node shed its copy.  Measured: two of three
		 * nodes went 1988 -> ~425 records, and it did NOT heal,
		 * because the eager sweep re-sends only what a node
		 * AUTHORED.
		 *
		 * The version is what separates "the same record" from "the
		 * holders disagree".
		 */
		req = park(3, 0, 2, 1000, &sl);
		chk(clpend_answer(&P, req, 1, 42, 7000, 0, &a) == CLPEND_ANS_HIT,
			"H1c the first positive is still a hit");
		r = clpend_answer(&P, req, 1, 9, 7000, 0, &a);
		chk(r == CLPEND_ANS_SAME,
			"H1c a second holder at the SAME version is the same "
			"record, not a race - this is the normal eager state "
			"and demoting on it destroys redundancy on recovery");

		/* A real disagreement still resolves, and the NEWER write
		 * wins - a property of the DATA, where "lower node id" is a
		 * property of who happens to be asking.
		 *
		 * THE NEWER VERSION IS DELIBERATELY ON THE HIGHER NODE ID.
		 * With the newer version on the LOWER id both schemes pick
		 * the same winner and the assertion proves nothing: a
		 * mutation resolving purely by id passed this test until the
		 * ids were arranged to disagree with the versions.
		 */
		req = park(3, 0, 2, 1000, &sl);
		chk(clpend_answer(&P, req, 1, 9, 7000, 0, &a) == CLPEND_ANS_HIT,
			"H1c setup: node 9 (lower id) holds the OLDER version");
		r = clpend_answer(&P, req, 1, 42, 8000, 0, &a);
		chk(r == CLPEND_ANS_RACE,
			"H1c holders at DIFFERENT versions are still a race");
		chk(a.winner == 42 && a.loser == 9,
			"H1c the NEWER version wins even though its node id is "
			"HIGHER - resolving by id would pick 9 here");

		/* and the other way round, so the answer cannot depend on
		 * which holder happened to reply first */
		req = park(3, 0, 2, 1000, &sl);
		chk(clpend_answer(&P, req, 1, 42, 8000, 0, &a) == CLPEND_ANS_HIT,
			"H1c setup: the NEWER version answers first this time");
		r = clpend_answer(&P, req, 1, 9, 7000, 0, &a);
		chk(r == CLPEND_ANS_RACE, "H1c still a race the other way");
		chk(a.winner == 42 && a.loser == 9,
			"H1c and the newer version still wins, regardless of "
			"arrival order");

		/* a peer too old to report a version cannot prove the two
		 * copies match, so it must still resolve - by id, as before */
		req = park(3, 0, 2, 1000, &sl);
		chk(clpend_answer(&P, req, 1, 42, 0, 0, &a) == CLPEND_ANS_HIT,
			"H1c setup: a holder with no version");
		r = clpend_answer(&P, req, 1, 9, 0, 0, &a);
		chk(r == CLPEND_ANS_RACE,
			"H1c version 0 means UNKNOWN, not equal - it must not "
			"silently suppress a real conflict");
		chk(a.winner == 9 && a.loser == 42,
			"H1c and with no versions to compare it falls back to "
			"the lower id, as it always did");

		/* H2  expect 3, three negatives: WAIT, WAIT, miss */
		req = park(3, 0, 3, 1000, &sl);
		chk(clpend_answer(&P, req, 0, 1, 0, 0, &a) == CLPEND_ANS_WAIT,
			"H2 a negative with answers still due waits");
		chk(clpend_answer(&P, req, 0, 2, 0, 0, &a) == CLPEND_ANS_WAIT,
			"H2 and so does the second");
		r = clpend_answer(&P, req, 0, 3, 0, 0, &a);
		...
		chk(clpend_answer(&P, req, 1, 5, 0, 0, &a) == CLPEND_ANS_IGNORE,
		...
		chk(clpend_answer(&P, req, 1, 1, 0, 0, &a) == CLPEND_ANS_IGNORE,
		...
		memcpy(sl->key, "k1", 2);
		sl->klen = 2;
		r = clpend_answer(&P, req, 0, 1, 0, 0, &a);
		...
		memcpy(sl->key, "k", 1);
		sl->klen = 1;
		r = clpend_answer(&P, req, 1, 11, 0, 555555, &a);
