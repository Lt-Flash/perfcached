...
static const struct golden { const char *nm; size_t n;
	...
	/* S209: eight LE bytes of the delete's version after the key -
	 * the golden grew deliberately; an older receiver ignores the tail */
	{ "TOMBSTONE", 27, {
...
int main(void)
{
	 ...
	 * and decoded at the other. */
	{
		...
		/* F2. tombstone round trip - the key tail plus the delete's
		 * version (S209) */
		{
			uint64_t tv = 0;

			len = clfwd_tomb_build(buf, 0x52, &kw, 3785530ULL);
			chk(len == CLFWD_TOMB_HDR + 12 + 8, "F2 tombstone is the key tail plus 8");
			chk(clfwd_tomb_parse(buf, len, &kr, &tv) == 1, "F2 parses");
			chk(kr.collen == 6 && !memcmp(kr.col, "orders", 6), "F2 collection");
			chk(kr.klen == 6 && !memcmp(kr.key, "cust:7", 6), "F2 key");
			chk(tv == 3785530ULL, "F2 the delete's version survives");
			/* an OLDER sender's frame - no version tail - parses with
			 * ver 0, so a mixed fleet keeps deleting during a roll */
			tv = 7;
			chk(clfwd_tomb_parse(buf, len - 8, &kr, &tv) == 1 && tv == 0,
				"F2 an unversioned tombstone parses with version 0");
			chk(kr.klen == 6 && !memcmp(kr.key, "cust:7", 6),
				"F2 and keeps its key");
			/* a version tail cut short is not a version */
			tv = 7;
			chk(clfwd_tomb_parse(buf, len - 3, &kr, &tv) == 1 && tv == 0,
				"F2 a partial version tail reads as unversioned");
		}

		/* F3. the two are NOT interchangeable - parsing one as the
		 * other must not quietly succeed with shifted fields */
		len = clfwd_tomb_build(buf, 0x52, &kw, 1);
		...
		len = clfwd_tomb_build(buf, 0x52, &kw, 1);
		chk(clfwd_tomb_parse(buf, CLFWD_TOMB_HDR - 1, &kr, &tv0) == 0,
			"F4 a tombstone shorter than its header is refused");
		chk(clfwd_tomb_parse(buf, CLFWD_TOMB_HDR + 3, &kr, &tv0) == 0,
		 ...
		 * overflow ASan can see */
		{
			...
			if (t2) {
				...
				chk(clfwd_tomb_parse(t2, CLFWD_TOMB_HDR - 1, &kr, &tv0) == 0,
		...
		kw.col = ""; kw.collen = 0;
		len = clfwd_tomb_build(buf, 0x52, &kw, 1);
		chk(clfwd_tomb_parse(buf, len, &kr, &tv0) == 1, "F6 an empty collection");
	...
	/* ---- the golden wire vectors -------------------------------- */
	{
		...
		}
		{
			...
			check_golden(g, clfwd_tomb_build(g, 22, &k, 0x0102030405060708ULL), "TOMBSTONE");
		}
		{
			struct clfwd_json j;
			memset(&j, 0, sizeof j);
