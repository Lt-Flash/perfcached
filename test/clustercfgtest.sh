...
collections = c" \
	"[collection c]
buckets_log2 = 10" "store|eager|proxy|shard"
badconf "eager key retired" "mode = store
eager = 1
collections = c" \
	"[collection c]
buckets_log2 = 10" "eager is now a mode"
badconf "collection eager key retired" "mode = store
collections = c" \
	"[collection c]
buckets_log2 = 10
eager = 1" "eager is now a mode"
badconf "collection eager contradicts cluster" "mode = store
collections = c" \
	"[collection c]
buckets_log2 = 10
mode = eager" "contradicts"

# ---- S127: mode = spread and its copy factor -------------------------
# Every rule here exists to refuse a config that would LOOK like it works
# and quietly place records where the rest of the fleet is not looking.
badconf "spread without replicas" "mode = spread
collections = c" \
	"[collection c]
buckets_log2 = 10" "needs replicas"
badconf "replicas without spread" "mode = store
replicas = 3
collections = c" \
	"[collection c]
buckets_log2 = 10" "only meaningful with mode = spread"
badconf "replicas = 1 is not an alias for shard" "mode = spread
replicas = 1
collections = c" \
	"[collection c]
buckets_log2 = 10" "REFUSED rather than aliased"
badconf "replicas above the member ceiling" "mode = spread
replicas = 258
collections = c" \
	"[collection c]
buckets_log2 = 10" "not an integer in .1..257."
badconf "spread is not a per-collection mode" "mode = store
collections = c" \
	"[collection c]
buckets_log2 = 10
mode = spread" "not a per-collection setting"

# and the config that SHOULD work
conf "$D/spreadok.conf" 17301 1 "mode = spread
replicas = 2
collections = c" "[collection c]
buckets_log2 = 10"
"$BIN" -C -f "$D/spreadok.conf" >/dev/null 2>&1 \
	&& ok || bad "mode = spread with replicas = 2 rejected"

# the effective config must SAY spread, or an operator cannot confirm it
"$BIN" -C -f "$D/spreadok.conf" 2>&1 | grep -qi "spread" \
	&& ok || bad "-C output never mentions spread"

# mode = eager parses, and is the spelling that replaced the flag
conf "$D/eagerok.conf" 17301 1 "mode = eager
collections = c" "[collection c]
buckets_log2 = 10"
"$BIN" -C -f "$D/eagerok.conf" >/dev/null 2>&1 \
	&& ok || bad "mode = eager rejected"
...
collections = c" "mode = shard
collections = c"
pair "eager differs"  "mode = eager
collections = c" "mode = store
collections = c"
pair "collection SET differs" "mode = store
collections = c" "mode = store
collections = c, other"
# S127: THE COPY FACTOR IS INTERCHANGE STATE.  Two members running
# different K place the same record on different nodes and each reads
# the other as missing it - the exact failure the digest exists to
# refuse, and the reason `replicas` is folded into it.
pair "spread K=2 vs K=3" "mode = spread
replicas = 2
collections = c" "mode = spread
replicas = 3
collections = c"
pair "spread vs eager" "mode = spread
replicas = 2
collections = c" "mode = eager
collections = c"
