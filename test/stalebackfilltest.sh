...
#  1. records on L; X restarts empty; L backfills it and logs the completion
#  2. once X reports itself established (cold window closed), Y drops the
#     flag it never used, and says so
...
XE=$(q "1711$X" entries)
[ "$XE" = "0" ] && ok "X came back empty" || bad "X did not come back empty (entries='$XE')"
...
i=0
while [ $i -lt 40 ]; do
	grep -q "backfilled node $IX after its restart" "$D/n$L.log" && break
	sleep 1; i=$((i+1))
done
grep -q "backfilled node $IX after its restart" "$D/n$L.log" && ok "L logged the completion (after ${i}s)" \
	|| bad "L never logged 'backfilled node $IX': $(grep -E 'backfill|cold' "$D/n$L.log" | tail -1)"
...
# ---- 2. X established: Y drops the flag it never used ------------------
# X reports COLD for 30 s after ready, then ESTABLISHED; Y should drop the
# stale flag at the next tick and say so.
i=0
while [ $i -lt 60 ]; do
