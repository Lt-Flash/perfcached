...
# name (json.get, bin.get; RESP keeps its bare Redis name), and the
...
check(jg.get("calls") == 40 and len(jg.get("hist", [])) == 17 and sum(jg.get("hist", [])) == 40,
      "/stats commands: json.get calls=40 with 17 buckets summing to the calls (%r)" % (jg.get("calls"),))
...
check(len(bk) == 17 and les[0] == "0.000001" and les[15] == "0.032768" and les[16] == "+Inf",
      "/metrics histogram: 16 log2 bounds from 1 us to 32.768 ms and +Inf (%d rows)" % len(bk))
