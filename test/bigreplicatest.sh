...
start() { # start <id>
	mkdir -p "$D/wal$1"
	: > "$D/n$1.log"               # the ready line must be THIS start's
...
# THREE nodes, not two: the defect this pins needs a tick in which the
# sweep offers a bulk batch to more than one peer (S178).
node 1 17696 17697
node 2 17698 17699
node 3 17700 17701
start 1
...
while time.time() < end:
    if all(len(stats(p)["cluster"].get("peers") or []) == 2
           for p in (17697, 17699, 17701)):
        break
    time.sleep(0.5)
peers = len(stats(17697)["cluster"].get("peers") or [])
(ok if peers == 2 else bad)("the three nodes see each other (%d peers)" % peers)

# and both must be READY: a node still bootstrapping refuses writes
# outright ("LOADING"), which is not what this test is about
end = time.time() + 60
while time.time() < end:
    if all(stats(p)["state"] == "ready" for p in (17697, 17699, 17701)):
        break
    time.sleep(0.5)
st = tuple(stats(p)["state"] for p in (17697, 17699, 17701))
(ok if st == ("ready",) * 3 else bad)("all three nodes are ready %s" % (st,))
...
# a keyspace with some body to it: a pinned sweep mark re-sends
# everything above it every cycle, and three records cannot show that
for i in range(3000):
    call(N1, "SET", "br:k%d" % i, b"x" * 200)
end = time.time() + 60
while time.time() < end:
    if all(stats(p)["collections"][0]["entries"] >= 3000
           for p in (17699, 17701)):
        break
    time.sleep(1)
ent = [stats(p)["collections"][0]["entries"] for p in (17697, 17699, 17701)]
(ok if min(ent) >= 3000 else bad)(
    "3,000 more records reach both peers (%s)" % ent)
...
# the bulk plane carried it, to BOTH peers - one offer per peer in the
# same tick is the case the one-slot handoff could not serve
b_in = [stats(p)["cluster"].get("bulk_in") or 0 for p in (17699, 17701)]
(ok if min(b_in) >= 1 else bad)(
    "the copy arrived over the bulk plane at both peers (bulk_in=%s)" % b_in)
N3 = conn(17700)
g3 = call(N3, "GET", "br:big")
(ok if isinstance(g3, bytes) and g3 == v else bad)(
    "the third node has it too, whole (%s)"
    % (len(g3) if isinstance(g3, bytes) else g3))
...
(ok if lost == 0 else bad)(
    "no oversized batch went unconfirmed - with one slot in the handoff "
    "the second peer's batch is refused every tick (%d)" % lost)

# S178: and the sweep must SETTLE.  A refused offer used to be reported
# through a fleet-wide counter that only climbs, compared at the cycle
# boundary, so one refusal pinned every mark on the node and the sweep
# re-sent the whole keyspace to every peer for ever - measured live at
# ~1,900 records/s a node, all refused as older.  Three nodes and one
# oversized value is the smallest shape that reproduces it.
r0 = [stats(p)["cluster"].get("repl_out", 0) for p in (17697, 17699, 17701)]
time.sleep(30)
r1 = [stats(p)["cluster"].get("repl_out", 0) for p in (17697, 17699, 17701)]
moved = [b - a for a, b in zip(r0, r1)]
(ok if max(moved) <= 2 else bad)(
    "the sweep settles once every copy has landed: %s record(s) sent in "
    "the last 30 s, across three nodes" % moved)

print("bigreplicatest: %d passed, %d failed" % (pass_n, fail_n))
open("/tmp/.bigreplica_stage1", "w").write("%d %d" % (pass_n, fail_n))
sys.exit(1 if fail_n else 0)
PY_EOF
rc=$?
[ $rc = 0 ] || exit $rc

# ---- S179: a node that comes BACK re-offers what its peers already have
# Its sweep marks are zeroed on a rejoin, so it sends its whole recovered
# keyspace - the big value included - to peers that hold every record
# already.  The bulk receiver used to answer with the number it STORED,
# and a batch that was entirely refused as older therefore read as a
# failed transfer: the sender retried it every sweep, for ever.  This is
# the fleet's own shape after a deploy.
...
while time.time() < end:
    ...
    time.sleep(1)
st = tuple(stats(p)["state"] for p in (17697, 17699, 17701))
(ok if st == ("ready",) * 3 else bad)("the restarted node rejoined %s" % (st,))
e = stats(17697)["collections"][0]["entries"]
(ok if e >= 3000 else bad)("and came back with its records (%d)" % e)

# give it several sweeps to re-offer everything it recovered
time.sleep(40)
r0 = [stats(p)["cluster"].get("repl_out", 0) for p in (17697, 17699, 17701)]
l0 = [stats(p)["cluster"].get("repl_bulk_lost", 0) for p in (17697, 17699, 17701)]
time.sleep(30)
r1 = [stats(p)["cluster"].get("repl_out", 0) for p in (17697, 17699, 17701)]
l1 = [stats(p)["cluster"].get("repl_bulk_lost", 0) for p in (17697, 17699, 17701)]
(ok if max(l1) == 0 else bad)(
    "an oversized batch the peers ALREADY have is not reported unconfirmed "
    "- absolute, because one unconfirmed batch pins the mark and the whole "
    "keyspace goes again (repl_bulk_lost %s -> %s)" % (l0, l1))
(ok if max(b - a for a, b in zip(r0, r1)) <= 2 else bad)(
    "and the sweep settles after the rejoin: %s record(s) in 30 s"
    % [b - a for a, b in zip(r0, r1)])

print("bigreplicatest rejoin: %d passed, %d failed" % (pass_n, fail_n))
sys.exit(1 if fail_n else 0)
PYEOF
rc=$?
exit $rc
