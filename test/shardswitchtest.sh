...
trap 'pkill -9 -f "/var/tmp/[p]csw" 2>/dev/null; rm -rf "$D"' EXIT TERM INT
pass=0 fail=0
...
i=0
while [ $i -lt 40 ]; do
	SURV=$(for n in 1 2; do pgrep -f "$D/n$n.conf" >/dev/null && echo $n; done | head -1)
...
[ "$(get $SURV afterloss)" = "stillhere" ] \
	...
	|| bad "a write after the loss was not readable ($(get 2 afterloss))"
