...
print("REASON_AFTER", call(f, "stats").get("result", {}).get("state_reason", ""))
...
# S215: S186 defined the reason and nothing ever set it, so a node failed
# by its WAL said "failed without recording a reason"
case "$(grep '^REASON_AFTER ' "$D/out")" in
*"dropped acknowledged writes"*)	ok "and says why: $(sed -n 's/^REASON_AFTER //p' "$D/out" | cut -c1-60)..." ;;
*)		bad "FAILED, and the reason reads: '$(sed -n 's/^REASON_AFTER //p' "$D/out")'" ;;
esac
