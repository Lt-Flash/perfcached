...
# S111: the close line counts a JSON connection's requests (it counted
# RESP commands only, so every driver link closed with "0 requests")
python3 - <<'EOF'
import json, socket
s = socket.create_connection(("127.0.0.1", 16479), timeout=2)
f = s.makefile("rwb")
for i in range(3):
    f.write(('{"jsonrpc":"2.0","id":%d,"method":"ping"}\n' % (i + 1)).encode()); f.flush(); f.readline()
s.close()
EOF
sleep 1
if grep -q "disconnected (peer closed) after [0-9]*s, 3 requests" "$D/log"; then ok
else bad "S111: no close line with 3 requests: $(grep -a 'disconnected' "$D/log" | tail -1 | cut -c1-120)"; fi
