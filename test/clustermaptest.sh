...
if what == "keyslot":
    ...
    r, _ = parse(call(host, port, [b"INFO", sec.encode()] if sec else [b"INFO"]))
    sys.stdout.write(r.decode() if isinstance(r, bytes) else str(r))
elif what == "solo":
...
slotstat() { # slotstat <n> <field> - stats.resp.<field> over the native door
	"$CLI" -h 127.0.0.7$1 -p 1745$1 -q -j '{"method":"stats"}' \
		| python3 -c 'import json,sys; print(json.load(sys.stdin)["resp"]["'"$2"'"])'
}
fleet() { # fleet <count> - bring up N nodes and assert the whole surface
	...
	# ---- S74: the advertisement agrees with the map --------------------
	inf=$(python3 "$PY" 127.0.0.71 17461 info server)
	echo "$inf" | grep -q "redis_mode:cluster" && ok "INFO server says redis_mode:cluster" \
		|| bad "S74: INFO server: $(echo "$inf" | grep redis_mode)"
	inf=$(python3 "$PY" 127.0.0.71 17461 info cluster)
	echo "$inf" | grep -q "cluster_enabled:1" && ok "INFO cluster exists and says enabled" \
		|| bad "S74: INFO cluster: $(echo "$inf" | head -2 | tr '\n' ' ')"
	python3 "$PY" 127.0.0.71 17461 info | grep -q "^# Cluster" && ok "bare INFO carries the Cluster section" \
		|| bad "S74: bare INFO has no Cluster section"
	# ---- S75: built once per membership, served from the cache after ----
	b0=$(slotstat 1 slots_builds); h0=$(slotstat 1 slots_hits)
	python3 "$PY" 127.0.0.71 17461 cover >/dev/null 2>&1
	python3 "$PY" 127.0.0.71 17461 cover >/dev/null 2>&1
	b1=$(slotstat 1 slots_builds); h1=$(slotstat 1 slots_hits)
	[ "${b1:-x}" = "${b0:-y}" ] && [ "$((h1-h0))" -ge 4 ] \
		&& ok "SLOTS and SHARDS served from the cache (builds $b1, hits +$((h1-h0)))" \
		|| bad "S75: builds $b0->$b1, hits $h0->$h1 across four map fetches"
	if [ "$N" = 4 ]; then
		# a node dies: the fleet notices within PEER_UP_MS and the next
		# fetch is a rebuild that no longer lists its door
		s_t0=$(date +%s%N)
		python3 "$PY" 127.0.0.71 17461 cover >/dev/null 2>&1
		echo "  (measure) one cover fetch from the cache, 4 nodes: $(( ($(date +%s%N) - s_t0) / 1000000 )) ms"
		kill -9 $P4 2>/dev/null; P4=
		sleep 12
		cv=$(python3 "$PY" 127.0.0.71 17461 cover 2>/dev/null)
		echo "$cv" | grep '^PORTS ' | grep -q 17464 && bad "S75: a dead node's door is still in the map: $(echo "$cv" | grep '^PORTS ')" \
			|| ok "the map dropped the dead node's door"
		[ "$(slotstat 1 slots_builds)" -gt "$b1" ] && ok "a membership change rebuilt the map" \
			|| bad "S75: builds did not advance after a node died"
	fi
	f_n=1
	while [ $f_n -le "$N" ]; do
...
# S74: and the same node calls itself standalone, in both places
python3 "$PY" 127.0.0.79 17469 info server | grep -q "redis_mode:standalone" \
	&& ok "a clusterless node says redis_mode:standalone" || bad "S74: clusterless INFO server"
python3 "$PY" 127.0.0.79 17469 info cluster | grep -q "cluster_enabled:0" \
	&& ok "a clusterless node says cluster_enabled:0" || bad "S74: clusterless INFO cluster"
