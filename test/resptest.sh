...
#  - S86: RedisJSON's JSON.SET/GET/DEL/NUMINCRBY/ARRAPPEND/DEBUG over the
#    native path operations, both path spellings, EX, NX/XX, the probe.
...
# JSON.SET/GET/DEL/NUMINCRBY/ARRAPPEND mapped onto the native path ops;
# a `$` path answers v2 style (an array of matches), a legacy `.path`
# the bare value; EX on JSON.SET; NX/XX answer nil; the probe OpenSIPS
# sends at connect (JSON.DEBUG HELP) is answered like RedisJSON's.
OJ=$(python3 "$RC" 17091 <<EOF
JSON.SET${US}doc${US}\$${US}{"status":"Unreachable","n":1,"arr":[1]}
JSON.GET${US}doc
JSON.GET${US}doc${US}\$.status
JSON.GET${US}doc${US}.status
JSON.GET${US}doc${US}status
JSON.SET${US}doc${US}\$.status${US}"Reachable"
JSON.GET${US}doc${US}.status
JSON.SET${US}doc${US}\$.status${US}"x"${US}NX
JSON.SET${US}doc${US}\$.missing${US}1${US}XX
JSON.NUMINCRBY${US}doc${US}\$.n${US}4
JSON.NUMINCRBY${US}doc${US}.n${US}1
JSON.ARRAPPEND${US}doc${US}\$.arr${US}2
JSON.ARRAPPEND${US}doc${US}.arr${US}3
JSON.GET${US}doc${US}\$.arr
JSON.GET${US}doc${US}\$.nope
JSON.GET${US}doc${US}.nope
JSON.GET${US}k1
JSON.SET${US}nokey${US}\$.a${US}1
JSON.SET${US}bad${US}\$${US}notjson
JSON.DEL${US}doc${US}\$.n
JSON.DEL${US}doc${US}\$.n
JSON.DEBUG${US}HELP
JSON.DEBUG${US}MEMORY${US}doc
JSON.SET${US}d2${US}\$${US}{"a":1}${US}EX${US}300
TTL${US}d2
JSON.SET${US}d2${US}\$.a${US}2
TTL${US}d2
JSON.GET${US}d2${US}.a
JSON.DEL${US}doc
JSON.GET${US}doc
...
jexpect() { # jexpect <lineno> <exact>
	L=$(printf '%s\n' "$OJ" | sed -n "${1}p")
	[ "$L" = "$2" ] && ok || bad "S86 line $1: got '$L' want '$2'"
}
jexpect 1 "OK"
jexpect 2 'bulk:{"status":"Unreachable","n":1,"arr":[1]}'
jexpect 3 'bulk:["Unreachable"]'
jexpect 4 'bulk:"Unreachable"'
jexpect 5 'bulk:"Unreachable"'
jexpect 6 "OK"
jexpect 7 'bulk:"Reachable"'
jexpect 8 "nil"
jexpect 9 "nil"
jexpect 10 "bulk:[5]"
jexpect 11 "bulk:6"
jexpect 12 "arr:[int:2]"
jexpect 13 "int:3"
jexpect 14 "bulk:[[1,2,3]]"
jexpect 15 "bulk:[]"
L=$(printf '%s\n' "$OJ" | sed -n 16p); case "$L" in ERR*path*) ok;; *) bad "S86 line 16: legacy path miss must be an error, got '$L'";; esac
L=$(printf '%s\n' "$OJ" | sed -n 17p); case "$L" in WRONGTYPE*) ok;; *) bad "S86 line 17: a plain value must answer WRONGTYPE, got '$L'";; esac
L=$(printf '%s\n' "$OJ" | sed -n 18p); case "$L" in ERR*) ok;; *) bad "S86 line 18: a path on a missing key must be an error, got '$L'";; esac
L=$(printf '%s\n' "$OJ" | sed -n 19p); case "$L" in ERR*) ok;; *) bad "S86 line 19: a non-JSON document must be refused, got '$L'";; esac
jexpect 20 "int:1"
jexpect 21 "int:0"
L=$(printf '%s\n' "$OJ" | sed -n 22p); case "$L" in "arr:[bulk:JSON.DEBUG MEMORY"*) ok;; *) bad "S86 line 22: JSON.DEBUG HELP must answer like RedisJSON, got '$L'";; esac
L=$(printf '%s\n' "$OJ" | sed -n 23p); case "$L" in int:[1-9]*) ok;; *) bad "S86 line 23: JSON.DEBUG MEMORY must be a size, got '$L'";; esac
jexpect 24 "OK"
L=$(printf '%s\n' "$OJ" | sed -n 25p); case "$L" in int:29[0-9]|int:300) ok;; *) bad "S86 line 25: EX on JSON.SET did not set the TTL, got '$L'";; esac
jexpect 26 "OK"
L=$(printf '%s\n' "$OJ" | sed -n 27p); case "$L" in int:29[0-9]|int:300) ok;; *) bad "S86 line 27: a field update did not preserve the TTL, got '$L'";; esac
jexpect 28 "bulk:2"
jexpect 29 "int:1"
jexpect 30 "nil"
jexpect 31 "int:0"
