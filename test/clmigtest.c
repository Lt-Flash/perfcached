...
int main(void)
{
	 ...
	 * differently - which is why both are pinned here. */
	{
		...
		chk(clmig_group_hdr(y, sizeof y, 15, 0xdeadbeef, 4097, 0) ==
		...
		/* the write-path push seals a group in one go and writes the
		 * final count straight in - same layout, one spelling */
		memset(x, 0xAA, sizeof x); memset(y, 0xAA, sizeof y);
		x[0] = 18; pc_p32(x + 1, 0); pc_p16(x + 5, 4097); pc_p16(x + 7, 7);
		chk(clmig_group_hdr(y, sizeof y, 18, 0, 4097, 7) == CLMIG_GHDR &&
			memcmp(x, y, CLMIG_GHDR) == 0,
			"G3b a sealed group carries its count in the header");
		chk(clmig_group_count(y) == 7, "G3c and the accessor reads it");
