...
int main(void)
{
	...
	/* ---- 12. "not in the top K" vs "not in this map" ----------- */
	{
		uint16_t known, absent = 60000;

		mkmap(&M, 5, PC_CLMAP_W_UNSET);
		known = M.node[4].node_id;

		CHK(pc_clplace_has(&M, known),
			"a node the map lists reads as absent");
		CHK(!pc_clplace_has(&M, absent),
			"a node the map has never heard of reads as present");
		CHK(!pc_clplace_has(NULL, known),
			"a NULL map claims to know a node");

		/* the distinction that matters: in_set() answers 0 for BOTH,
		 * and a caller that DELETES on that 0 removes a node's own
		 * records whenever the published map predates its id.  Find
		 * a key this node does not hold at K=1, so both calls are 0
		 * for genuinely different reasons. */
		{
			int i2, found = 0;

			for (i2 = 0; i2 < 500; i2++) {
				uint64_t kh = kh_of(i2);

				if (pc_clplace_in_set(&M, kh, known, 1))
					continue;
				found = 1;
				CHK(!pc_clplace_in_set(&M, kh, known, 1) &&
				    !pc_clplace_in_set(&M, kh, absent, 1),
					"in_set does not answer 0 for both cases");
				CHK(pc_clplace_has(&M, known) !=
				    pc_clplace_has(&M, absent),
					"has() cannot tell 'not a holder' from "
					"'not in the map' - the ambiguity that "
					"cost a returning node 39 of 58 records");
				break;
			}
			CHK(found, "no key found that this node does not hold "
				"at K=1 - the case above went untested");
		}

		/* a node the map does not list is in NO set, at any k - the
		 * lookup failing must never read as membership */
		CHK(!pc_clplace_in_set(&M, kh_of(3), absent, 5),
			"an unknown node was placed in the set at k = every "
			"node");
	}
