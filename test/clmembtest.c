...
int main(void)
{
	...
	for (i = 0; i < sizeof D / sizeof D[0]; i++) {
		...
		ref_alive(a, 8, 65535, 11, 22, 33, 3, 1, D[i], 6379, ident,
			...
			123456, 77, 10, 20, 30, 17, 17600, D[(i + 3) % 6], 16, 19, COLS);
		{
	...
	ref_alive(a, 8, 65535, 11, 22, 33, 3, 1, 0x0123456789abcdefULL, 6379,
		...
		8080, 123456, 77, 10, 20, 30, 17, 17600, 0xa5a5a5a55a5a5a5aULL, 16, 19, COLS);
	for (n = 0; n <= CLMEMB_ALIVE_LEN; n++) {
		...
		CHK(!!(A.have & CLMEMB_A_PSRELAY) == (n >= CLMEMB_ALIVE_LEN), w);
		if (n >= CLMEMB_ALIVE_LEN) { uint64_t pi; memcpy(&pi, a + 94 + 56 * 16, 8);
	...
	/* ---- S163: a shorter collection block is complete at ITS length ---- */
	{
		...
		CHK(len == 92 + 56 * 2 + 10, "ALIVE with 2 collections is 214 bytes");
		...
		CHK(clmemb_alive_parse(b, len - 1, &A) && (A.have & CLMEMB_A_COLS) &&
			!(A.have & CLMEMB_A_PSRELAY),
			"one byte short: the relay fields are absent, the block intact");
		CHK(clmemb_alive_parse(b, len - 11, &A) && !(A.have & CLMEMB_A_COLS) &&
			!(A.have & CLMEMB_A_PSRELAY),
			"eleven short: the block is absent, not partial, and nothing after it is read");
