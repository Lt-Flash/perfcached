	...
	int nreason)
{
	...
	msg[102 + 56 * ncols] = (unsigned char)nreason;   /* S186, last */
}
...
int main(void)
{
	...
	ref_alive(a, 8, 65535, 11, 22, 33, 3, 1, 0x0123456789abcdefULL, 6379,
		...
		8080, 123456, 77, 10, 20, 30, 17, 17600, 0xa5a5a5a55a5a5a5aULL, 16, 19, COLS, 1);
	for (n = 0; n <= CLMEMB_ALIVE_LEN; n++) {
		...
		/* S186: the reason byte is the LAST field, so the relay
		 * fields are complete one byte before the full frame */
		CHK(!!(A.have & CLMEMB_A_PSRELAY) == (n >= CLMEMB_ALIVE_LEN - 1), w);
		CHK(!!(A.have & CLMEMB_A_REASON) == (n >= CLMEMB_ALIVE_LEN), w);
		CHK((n >= CLMEMB_ALIVE_LEN) ? A.nreason == 1 : A.nreason == 0, w);
		if (n >= CLMEMB_ALIVE_LEN - 1) { uint64_t pi; memcpy(&pi, a + 94 + 56 * 16, 8);
	...
	/* ---- S163: a shorter collection block is complete at ITS length ---- */
	{
		...
		CHK(len == 92 + 56 * 2 + 11, "ALIVE with 2 collections is 215 bytes");
		...
		/* S186: the reason is the LAST field, so it is the first to go
		 * missing - and a frame without it must read as "no reason
		 * given", never as a reason the peer did not send */
		CHK((A.have & CLMEMB_A_REASON) && A.nreason == 1,
			"the state reason rides at the tail and parses");
		CHK(clmemb_alive_parse(b, len - 1, &A) && (A.have & CLMEMB_A_PSRELAY) &&
			!(A.have & CLMEMB_A_REASON) && A.nreason == 0,
			"one byte short: no reason is invented, the relay fields intact");
		CHK(clmemb_alive_parse(b, len - 2, &A) && (A.have & CLMEMB_A_COLS) &&
			!(A.have & CLMEMB_A_PSRELAY) && !(A.have & CLMEMB_A_REASON),
			"two short: the relay fields are absent, the block intact");
		CHK(clmemb_alive_parse(b, len - 12, &A) && !(A.have & CLMEMB_A_COLS) &&
			!(A.have & CLMEMB_A_PSRELAY),
			"twelve short: the block is absent, not partial, and nothing after it is read");
		/* an S163-era frame - the block, nothing after - leaves the relay
		 * port absent rather than reading the block's bytes as one */
		CHK(clmemb_alive_parse(b, len - 10, &A) && (A.have & CLMEMB_A_COLS) &&
