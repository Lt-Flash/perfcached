...
static void ref_malive(unsigned char *msg, unsigned char type, int node,
	...
	int mode, int eager, uint64_t cd, uint32_t term, int epoch)
{
	msg[0] = type;
	...
	memcpy(msg + 27, &cd, 8);
	p32(msg + 35, term);
	msg[39] = (unsigned char)epoch;                 /* S221 */
}
...
static void ref_alive(unsigned char *msg, unsigned char type, int node,
	...
	uint32_t clo, uint32_t clr, uint32_t clb, uint32_t clj, uint32_t cln,
	int psport, uint64_t psint, int ncols, int ntot, const uint64_t *cols,
	int nreason, int epoch)
{
	int i;
	msg[0] = type;
	p16(msg + 1, (uint16_t)node);
	...
	p32(msg + 82, clj);
	p32(msg + 86, cln);
	msg[90] = (unsigned char)ncols;                 /* S163 */
	msg[91] = (unsigned char)ntot;
	for (i = 0; i < ncols * 7; i++)
		memcpy(msg + 92 + 8 * i, &cols[i], 8);
	p16(msg + 92 + 56 * ncols, (uint16_t)psport);  /* PS11: after the block */
	memcpy(msg + 94 + 56 * ncols, &psint, 8);
	msg[102 + 56 * ncols] = (unsigned char)nreason;   /* S186 */
	msg[103 + 56 * ncols] = (unsigned char)epoch;     /* S221, last */
}
...
static void ref_joinreq(unsigned char *msg, unsigned char type, uint64_t tok,
	...
	int proposed, uint32_t incarn, int wal, int epoch)
{
	msg[0] = type;
	memcpy(msg + 1, &tok, 8);
	...
	msg[42] = (unsigned char)epoch;              /* S221, last */
}
...
int main(void)
{
	...
	unsigned char ident[16], a[CLMEMB_ALIVE_LEN + 8], b[CLMEMB_ALIVE_LEN + 8];
	uint64_t COLS[16 * 7];
	{
		int ci;
		for (ci = 0; ci < 16 * 7; ci++)
			COLS[ci] = 0x0100000000000000ULL * (uint64_t)(ci / 7 + 1) + (uint64_t)ci * 7919u;
	}
	...
	for (i = 0; i < sizeof D / sizeof D[0]; i++) {
		...
		ref_alive(a, 8, 65535, 11, 22, 33, 3, 1, D[i], 6379, ident,
			...
			123456, 77, 10, 20, 30, 17, 17600, D[(i + 3) % 6], 16, 19, COLS, (i % 5) + 1,
			(i % 3) + 1);
		{
			...
			in.uptime_s = 123456;
			in.cl_open = 77; in.cl_resp = 10; in.cl_bin = 20;
			in.cl_json = 30; in.cl_nresp = 17;
			in.ps_port = 17600; in.ps_interest = D[(i + 3) % 6];
			in.nreason = (i % 5) + 1;      /* S186 */
			in.epoch = (i % 3) + 1;        /* S221 */
			in.ncols = 16; in.ncols_total = 19;
			for (k = 0; k < 16; k++) {
				in.cols[k].hash = COLS[k * 7]; in.cols[k].entries = COLS[k * 7 + 1];
				in.cols[k].hits = COLS[k * 7 + 2]; in.cols[k].misses = COLS[k * 7 + 3];
				in.cols[k].stores = COLS[k * 7 + 4]; in.cols[k].removes = COLS[k * 7 + 5];
				in.cols[k].expired = COLS[k * 7 + 6];
			}
			CHK(clmemb_alive_write(b, sizeof b, 8, &in) == CLMEMB_ALIVE_LEN,
				"alive length");
		}
		CHK(memcmp(a, b, CLMEMB_ALIVE_LEN) == 0, "ALIVE bytes");
		/* S216: the watchdog's two-byte patch is exactly what a fresh
		 * encode with that state and reason would have produced - at
		 * the full length and with no collections at all */
		{
			struct clmemb_alive in, out;
			unsigned char p1[CLMEMB_ALIVE_LEN], p2[CLMEMB_ALIVE_LEN];
			size_t n1, n2;
			int nc;

			for (nc = 0; nc <= 16; nc += 16) {
				memset(&in, 0, sizeof in);
				in.node = 300 + i; in.nstate = 2; in.nreason = 0;
				in.ident = ident; in.lamport = D[i]; in.ncols = nc;
				in.ncols_total = nc; in.uptime_s = 7 + (unsigned)i;
				n1 = clmemb_alive_write(p1, sizeof p1, 8, &in);
				in.nstate = 5; in.nreason = 7;
				n2 = clmemb_alive_write(p2, sizeof p2, 8, &in);
				CHK(n1 == n2 && n1 > 0 &&
					clmemb_alive_patch_state(p1, n1, 5, 7) == 0 &&
					memcmp(p1, p2, n1) == 0,
					"patched ALIVE == a fresh encode");
				memset(&out, 0, sizeof out);
				CHK(clmemb_alive_parse(p1, n1, &out) &&
					out.nstate == 5 && out.nreason == 7,
					"patched ALIVE parses with the new state");
			}
			CHK(clmemb_alive_patch_state(p1, 60, 5, 7) == -1,
				"a frame too short to hold both is refused");
		}
	}
	...
	ref_alive(a, 8, 65535, 11, 22, 33, 3, 1, 0x0123456789abcdefULL, 6379,
		...
		8080, 123456, 77, 10, 20, 30, 17, 17600, 0xa5a5a5a55a5a5a5aULL, 16, 19, COLS, 1, 2);
	for (n = 0; n <= CLMEMB_ALIVE_LEN; n++) {
		...
		CHK(!!(A.have & CLMEMB_A_CLIENTS) == (n >= 90), w);
		if (n >= 90) CHK(A.cl_open == g32(a + 70) &&
			A.cl_resp == g32(a + 74) && A.cl_bin == g32(a + 78) &&
			A.cl_json == g32(a + 82) && A.cl_nresp == g32(a + 86), w);
		CHK(!!(A.have & CLMEMB_A_COLS) == (n >= 92 + 56 * 16), w);
		/* the tail is [relay fields][nreason][epoch], so the relay
		 * fields are complete TWO bytes before the full frame */
		CHK(!!(A.have & CLMEMB_A_PSRELAY) == (n >= CLMEMB_ALIVE_LEN - 2), w);
		/* S221 appended the epoch behind the reason, so the reason now
		 * appears one byte earlier than the full frame */
		CHK(!!(A.have & CLMEMB_A_REASON) == (n >= CLMEMB_ALIVE_LEN - 1), w);
		CHK((n >= CLMEMB_ALIVE_LEN - 1) ? A.nreason == 1 : A.nreason == 0, w);
		CHK(!!(A.have & CLMEMB_A_EPOCH) == (n >= CLMEMB_ALIVE_LEN), w);
		CHK((n >= CLMEMB_ALIVE_LEN) ? A.epoch == 2 : A.epoch == 0, w);
		if (n >= CLMEMB_ALIVE_LEN - 2) { uint64_t pi; memcpy(&pi, a + 94 + 56 * 16, 8);
			CHK(A.ps_port == g16(a + 92 + 56 * 16) && A.ps_interest == pi, w); }
		if (n >= 92 + 56 * 16) {
			uint64_t v;
			memcpy(&v, a + 92 + 8 * (5 * 7 + 3), 8);
			CHK(A.ncols == 16 && A.ncols_total == 19 &&
				A.cols[0].hash == COLS[0] && A.cols[5].misses == v &&
				A.cols[15].expired == COLS[15 * 7 + 6], w);
		}
	}
	/* ---- S163: a shorter collection block is complete at ITS length ---- */
	{
		struct clmemb_alive in = { 0 };
		size_t len;
		in.node = 7; in.ident = ident; in.ncols = 2; in.ncols_total = 2;
		in.ps_port = 17600;
		in.cols[0].hash = 0x1111; in.cols[0].hits = 5;
		in.cols[1].hash = 0x2222; in.cols[1].expired = 9;
		in.nreason = 1;                       /* S186: PC_NRE_WAL_DROP */
		len = clmemb_alive_write(b, sizeof b, 8, &in);
		CHK(len == 92 + 56 * 2 + 12, "ALIVE with 2 collections is 216 bytes");
		CHK(clmemb_alive_parse(b, len, &A) && (A.have & CLMEMB_A_COLS) &&
			A.ncols == 2 && A.cols[1].hash == 0x2222 && A.cols[1].expired == 9 &&
			A.cols[0].hits == 5 && (A.have & CLMEMB_A_PSRELAY) && A.ps_port == 17600,
			"and parses whole at that length, relay port included");
		/* S186 then S221: the tail is [nreason][epoch], and each is the
		 * first to go missing as the frame shortens - a frame without
		 * one must read as "not sent", never as a value invented here */
		CHK((A.have & CLMEMB_A_REASON) && A.nreason == 1 &&
			(A.have & CLMEMB_A_EPOCH),
			"the state reason and the wire epoch ride at the tail and parse");
		CHK(clmemb_alive_parse(b, len - 1, &A) && (A.have & CLMEMB_A_REASON) &&
			!(A.have & CLMEMB_A_EPOCH) && A.epoch == 0,
			"one byte short: no EPOCH is invented - a pre-S221 peer reads as epoch 0");
		CHK(clmemb_alive_parse(b, len - 2, &A) && (A.have & CLMEMB_A_PSRELAY) &&
			!(A.have & CLMEMB_A_REASON) && A.nreason == 0,
			"two short: no reason is invented, the relay fields intact");
		CHK(clmemb_alive_parse(b, len - 3, &A) && (A.have & CLMEMB_A_COLS) &&
			!(A.have & CLMEMB_A_PSRELAY) && !(A.have & CLMEMB_A_REASON),
			"three short: the relay fields are absent, the block intact");
		CHK(clmemb_alive_parse(b, len - 13, &A) && !(A.have & CLMEMB_A_COLS) &&
			!(A.have & CLMEMB_A_PSRELAY),
			"thirteen short: the block is absent, not partial, and nothing after it is read");
		/* an S163-era frame - the block, nothing after - leaves the relay
		 * port absent rather than reading the block's bytes as one */
		CHK(clmemb_alive_parse(b, len - 11, &A) && (A.have & CLMEMB_A_COLS) &&
			!(A.have & CLMEMB_A_PSRELAY) && A.ps_port == 0,
			"a frame from before the relay fields: no relay port is invented");
	}

	ref_malive(b, 9, 4097, 3, 0x0123456789abcdefULL, 1234, 5678, 91011,
		2, 1, 0xfedcba9876543210ULL, 0xdeadbeef, 2);
	for (n = 0; n <= CLMEMB_MALIVE_LEN; n++) {
		...
		/* S221: the epoch is the last byte, and a frame from a build
		 * that predates it reads 0 - which is the grandfather case */
		CHK(!!(M.have & CLMEMB_M_EPOCH) == (n >= 40), w);
		CHK(M.epoch == (n >= 40 ? b[39] : 0), w);
	}
