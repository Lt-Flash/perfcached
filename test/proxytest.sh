...
f = s.makefile("rwb")
# oversized proxy births need fleet-confirmed absence first (the
# get-miss->set pattern): a blind oversized SET through a non-holder
# is refused, because the forward plane cannot carry it and a local
# store could fork a remote holder
f.write(json.dumps({"jsonrpc":"2.0","id":1,"method":"get","params":
    {"col":"px","key":"bigkey"}}).encode()+b"\n")
f.flush()
json.loads(f.readline())
f.write(json.dumps({"jsonrpc":"2.0","id":2,"method":"set","params":
...
# the gather proof is a RATIO: gathered small records amortize to
# many records per datagram, ungathered runs at ~1/each.  (The old
# fixed margin MD <= MO-350 assumed ~465 records migrate; a
# condition-wait sampling at 427 failed it by ONE datagram - the
# margin tested the sample point, not the gathering.)
[ "$MD" -gt 0 ] && [ $((MO / MD)) -ge 3 ] && ok \
...
# ---- oversized proxy writes: refuse, never fork (S54 open defect) ------
# The forward plane carries at most PC_MAX_FWD_VAL bytes.  An oversized
# SET through a NON-holder must be refused (a local store could fork the
# remote holder); with a locator naming the holder OR with the holder
# unknown, the answer is "value too large".  Only a fleet-confirmed-
# absent key (GET miss stamps the negative cache) may be BORN locally.
OVR=$(python3 - <<'EOF'
import json, socket
def rpc(f, i, method, params):
    f.write(json.dumps({"jsonrpc":"2.0","id":i,"method":method,
        "params":params}).encode()+b"\n")
    f.flush()
    return json.loads(f.readline())
f2 = socket.create_connection(("127.0.0.1", 17012), timeout=6).makefile("rwb")
# a key node2 does NOT hold: list node2's local keys, pick a pk absent
loc = rpc(f2, 1, "keys", {"col":"px","match":"pk*"})["result"]["keys"]
remote = next("pk%03d" % i for i in range(120) if "pk%03d" % i not in loc)
# learn the locator (read-through), then try the oversized overwrite
rpc(f2, 2, "get", {"col":"px","key":remote})
r1 = rpc(f2, 3, "set", {"col":"px","key":remote,"value":"X"*60000})
e1 = "err" if "error" in r1 else "STORED"
# holder unknown, no confirmation: bare oversized SET of a fresh key
r2 = rpc(f2, 4, "set", {"col":"px","key":"ovnew1","value":"X"*60000})
e2 = "err" if "error" in r2 else "STORED"
# fleet-confirmed absent: GET miss first, then the oversized birth
rpc(f2, 5, "get", {"col":"px","key":"ovnew2"})
r3 = rpc(f2, 6, "set", {"col":"px","key":"ovnew2","value":"X"*60000})
e3 = "stored" if r3.get("result", {}).get("stored") else "REFUSED"
# the born key lives HERE (node2 is its holder)
here = "local" if "ovnew2" in rpc(f2, 7, "keys",
    {"col":"px","match":"ovnew2"})["result"]["keys"] else "ABSENT"
# net-zero for the later entry counts
rpc(f2, 8, "del", {"col":"px","key":"ovnew2"})
print(e1, e2, e3, here)
EOF
)
set -- ${OVR:-pyfail pyfail pyfail pyfail}
[ "$1" = "err" ] && ok || bad "oversized SET through a non-holder was NOT refused ($1) - fork window"
[ "$2" = "err" ] && ok || bad "oversized SET with unknown holder was NOT refused ($2)"
[ "$3" = "stored" ] && ok || bad "confirmed-absent oversized birth refused ($3)"
[ "$4" = "local" ] && ok || bad "oversized birth did not land on the birthing node ($4)"
