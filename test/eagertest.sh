...
#
# The window opens on THE RESTARTED NODE's completion, from a line written
# after its restart, on whichever survivor was the sender - never on node
# 1's record count, which a second sender's copies can satisfy while the
# first sender's cycle still has a slice to collect.  S82 documents two
# senders during the holdoff, so one full sweep period is then allowed
# for a second one to finish; after that EVERY node's counter must hold.
ID1=$(cstat 17071 node)
after() { tail -n +"$(($2 + 1))" "$D/n$1.log"; }   # after <n> <pos>: lines since the restart
i=0
while [ $i -lt 60 ]; do
	{ after 2 "$POS2"; after 3 "$POS3"; } | grep -q "backfilled node $ID1 after its restart" && break
	sleep 1; i=$((i+1))
done
SENDERS=""
after 2 "$POS2" | grep -q "backfilled node $ID1 after its restart" && SENDERS="$SENDERS node2"
after 3 "$POS3" | grep -q "backfilled node $ID1 after its restart" && SENDERS="$SENDERS node3"
[ -n "$SENDERS" ] && ok \
	|| bad "no survivor reported node $ID1's backfill complete in ${i}s: $( { after 2 "$POS2"; after 3 "$POS3"; } | grep -E 'backfill|cold' | tail -2 | tr '\n' '|')"
echo "backfill of node $ID1 completed by:$SENDERS (after ${i}s)"
sleep 11                       # one sweep period: a second sender's tail lands
