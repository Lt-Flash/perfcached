...
# A pass over the leg is 16,384 chains at PC_DRAIN_CHAINS a tick, so a
# record whose bucket was full when the cursor went by is not looked at
# again for about sixteen seconds - and the widening that gives it room
# happens meanwhile.  So "stopped moving" has to mean "did not move
# across a whole pass", not "did not move in the last two seconds".
end = time.time() + 150
last = leg0
still = 0
while time.time() < end:
    time.sleep(3)
    now = col().get("overflow", 0)
    still = still + 1 if now == last else 0
    last = now
    if still >= 7:                 # 21 s > one full pass
        break
c = col()
