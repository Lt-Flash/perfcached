...
# PS7: publishers on several workers at once.  The relay used to stamp one
# global counter from whichever worker published, so two workers sent N+1
# before N and node 1 counted a loss that never happened.  Every message must
# arrive, and node 1 must report nothing lost and nothing duplicated.
import threading
S1.cmd("UNSUBSCRIBE", "x"); S2.cmd("UNSUBSCRIBE", "x")
b1, b2 = stats(18481), stats(18482)
PER, THREADS = 1500, 8
def burst(tn):
    c = R(17482)
    frame = b"".join(("*3\r\n$7\r\nPUBLISH\r\n$4\r\nconc\r\n$6\r\nt%02dm%02d\r\n" % (tn, i % 100)).encode() for i in range(PER))
    c.s.sendall(frame)
    for _ in range(PER): c.read()
th = [threading.Thread(target=burst, args=(i,)) for i in range(THREADS)]
[t.start() for t in th]; [t.join() for t in th]
time.sleep(1.0)
a1, a2 = stats(18481), stats(18482)
sent = a2.get("relay_sent", 0) - b2.get("relay_sent", 0)
recv = a1.get("relay_recv", 0) - b1.get("relay_recv", 0)
lost = a1.get("relay_lost", 0) - b1.get("relay_lost", 0)
dups = a1.get("relay_duplicates", 0) - b1.get("relay_duplicates", 0)
ok(sent == PER * THREADS and recv == sent, "8 concurrent publishers on node 2: %d relayed, %d received on node 1" % (sent, recv))
ok(lost == 0, "and node 1 reports NO relay loss for a run where nothing was lost (got %r)" % lost)
ok(dups == 0 and "relay_duplicates" in a1, "and no duplicate (got %r)" % a1.get("relay_duplicates"))
