...
# group that has two cards on this plane is ONE cell, a trio is one cell,
# and no card was lost or doubled by the regrouping
...
cells = re.findall(r'<div class="stk">(.*?)</div></div></div>', plane, re.S)
sizes = sorted(c.count("<h3>") for c in cells)
(ok if len(cells) >= 8 and sizes and sizes[0] >= 2 and sizes[-1] == 3 else bad)(
    "the short cards share cells: %d stacks, of %s cards" % (len(cells), sizes))
