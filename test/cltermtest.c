...
int main(void)
{
	...
	/* ---- 9. S106: the legacy eight-byte form is read once and
	 * rewritten as text, keeping the term ---------------------- */
	{
		char p[512], line[64] = "";
		FILE *f;
		/* 'PCTM' little-endian, then term 7 little-endian */
		static const unsigned char legacy[8] =
			{ 'P', 'C', 'T', 'M', 7, 0, 0, 0 };

		snprintf(p, sizeof p, "%s/node-term", dir);
		f = fopen(p, "wb");
		if (f) { fwrite(legacy, 1, 8, f); fclose(f); }
		CHK(pc_term_init(dir) == 0 && pc_term_current() == 7,
			"the legacy eight-byte term was not read (term %u)",
			pc_term_current());
		f = fopen(p, "r");
		if (f) { if (!fgets(line, sizeof line, f)) line[0] = 0; fclose(f); }
		CHK(strcmp(line, "perfcached-node-term 1\n") == 0,
			"the legacy term file was not rewritten as text "
			"(first line '%s')", line);
		CHK(pc_term_init(dir) == 0 && pc_term_current() == 7,
			"the rewritten text term does not read back as 7 (%u)",
			pc_term_current());
	}

	/* ---- 10. S106: a text term whose check does not match refuses,
	 * and so does a fourth line ---------------------------------- */
	{
		char p[512];
		FILE *f;
		...
		f = fopen(p, "w");
		if (f) {
			fputs("perfcached-node-term 1\nterm 7\n"
				"check 0000000000000000\n", f);
			fclose(f);
		}
		CHK(pc_term_init(dir) == -1,
			"a term file with a wrong check value was accepted");
		f = fopen(p, "w");
		if (f) {
			fputs("perfcached-node-term 1\nterm 7\n"
				"check 0000000000000000\nextra\n", f);
		...
		CHK(pc_term_init(dir) == -1,
			"a term file with a trailing line was accepted");
	}
