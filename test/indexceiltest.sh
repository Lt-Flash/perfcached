...
# S129: this suite resizes, which is privileged
enable = idx-enable-secret
[listen]
plaintext = loopback
...
# S129: create/resize are privileged, and this helper dials once per
# invocation - raise it on THIS link before anything else.
call("enable", secret="idx-enable-secret")
def say(r):
    if "error" in r:
...
#    have taken.  2^20 is 68.8 MB of index against a 64 MB ceiling.
...
[ "$R" = OK ] && ok "a 2^16 create (5 MB of index) is accepted" \
...
R=$(drive resize c 14)
[ "$R" = OK ] && ok "a 2^14 resize (2 MB of index) is accepted" \
...
if [ $rc -eq 124 ]; then
	...
	ok "a 2^22 collection (272.8 MB of index) on a $MAXMB MB arena refuses to start (rc=$rc)"
else
