...
EOF
	...
	# THE PREMISE, CHECKED: an ack is at the producer ring, not the file
	# (src/wal.h), and kill -9 takes whatever is still in a ring.  A fixed
	# sleep assumed the pump had drained; under ASAN on a two-core runner
	# it had not, and 95 of 600 acked keys were "lost" by a test that had
	# never waited for them to be logged.  Wait on the mark the daemon
	# publishes for exactly this - wal.staged (S145) - bounded, and say so
	# if it never drains rather than blame recovery for it.
	w=0
	while [ $w -lt 100 ]; do
		st=$(python3 - <<'PYS'
import json, socket
s = socket.create_connection(("127.0.0.1", 16487), timeout=5); f = s.makefile("rwb")
f.write(b'{"jsonrpc":"2.0","id":1,"method":"stats"}\n'); f.flush()
w = json.loads(f.readline())["result"]["wal"]
print(w.get("staged", (w["last_seq"] - w["synced_seq"])))
PYS
)
		[ "${st:-1}" = 0 ] && break
		sleep 0.1; w=$((w+1))
	done
	[ "${st:-1}" = 0 ] && echo "  gen $gen: wal drained (staged=0 after $((w/10)).$((w%10))s)" \
		|| bad "gen $gen: the wal never drained in 10s (staged=$st) - the storm cannot assert no-loss"
	hardkill
done
newlog; start
python3 - <<'EOF'
