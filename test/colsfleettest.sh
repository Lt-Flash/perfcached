...
set -- $(state 1); [ "$1" = ready ] && [ "$2" = 2 ] && ok "three nodes formed a store-mode fleet" || { bad "fleet never formed: $(state 1)"; exit 1; }
...
check(all(r.get("result", {}).get("found") is True for r in r2), "node 2 answers k1 through a pull from node 1")
