...
# column to show.  S164: /stats rows carry the daemon's fleet block, and
# under store (a pulled copy is kept) its entries are the fullest member's
# as a lower bound, not the copies summed.
# Fail-first: v0.4.0-rc1's ALIVE carries no collection block and its
# /stats rows carry no hash.
# The read through node 2 once took node 3's late "joined" notification for
# its reply (GitHub master, rc3 and rc4; 1 of 60 on two pinned CPUs), and an
# 80 ms pull timed out on those two CPUs (2 of 60) - hence the id-matching
# reader and pull_timeout_ms = 1000: this test counts, it does not time.
...
def own_misses(): return sum(col(h).get("misses_client", 0) for h in (H1, H2, H3))
check(wait(lambda: sum(c.get("misses", 0) for c in fleet_b(H3).values()) == own_misses(), 6),
      "fleet misses equal the members' own client misses (%r vs %r)" % (sum(c.get("misses", 0) for c in fleet_b(H3).values()), own_misses()))
check(set(fb[list(fb)[0]].keys()) == {"hash", "entries", "hits", "misses", "stores", "removes", "expired"}, "seven fields a collection")
fl3 = col(H3).get("fleet") or {}
check(fl3.get("basis") == "at_least" and fl3.get("members") == 3 and fl3.get("reporting") == 3
      and fl3.get("entries") == max(c.get("entries", 0) for c in fb.values())
      and fl3.get("copies") == sum(c.get("entries", 0) for c in fb.values()),
      "store: node 3's fleet block reports the fullest member's entries as a lower bound, the copies beside (%r)" % fl3)
check(fl3.get("hits") == want_hits, "and its fleet hits are the members' client hits summed (%r)" % fl3.get("hits"))
...
check("fleet figures first" in page and "function fv(" in page and "copies across" in page, "the page renders fleet figures first with this node beside")
PY
