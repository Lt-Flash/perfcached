...
int main(void)
{
	 ...
	 * happens to do rather than what it must do. */
	{
		...
		in.mode = 0; in.eager = 0; in.wal = 0; in.replicas = 0;
		...
		/* S127: K decides WHICH nodes hold a key, so two members
		 * running different K place the same record differently and
		 * each reads the other as missing it.  It must move the
		 * digest, and each K must be distinct from every other. */
		in.route_algo = "hrw-slot16k-v1"; in.n_names = 1;
		in.replicas = 2;
		chk(clinit_config_digest(&in) != d0,
			"G5 replicas changes it");
		{
			uint64_t k2 = clinit_config_digest(&in);

			in.replicas = 3;
			chk(clinit_config_digest(&in) != k2,
				"G5 K=3 and K=2 are different digests - a fleet "
				"cannot half-agree about the copy factor");
		}
		in.replicas = 0;
		...
		in.replicas = 0;
		chk(clinit_config_digest(&in) == d0,
			"G6 replicas=0 folds nothing either, so every non-spread "
			"fleet keeps the digest it had before S127");
