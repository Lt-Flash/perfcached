...
lock = threading.Lock()
# when each walk was sent and answered, per connection - so a walk IN
# FLIGHT at the swap can be named, and a zero below explained rather than
# retried.  GitHub's check on rc33 read zero with 5790 walks through the
# resize, which 14 runs here never did; the timeline is what the next one
# will show.
spans = [[], [], []]
def walker(ix):
    try:
...
# the swap's moment: the bucket count stats reports moves from 2^18 to
# 2^16 when the registry publishes the new table
swap_at = [None]
def swapwatch():
    while not stop and swap_at[0] is None:
        try:
            cs = native("stats").get("collections", [])
            b = [x.get("buckets") for x in cs if x.get("name") == "0"]
            if b and b[0] == 1 << 16:
                swap_at[0] = time.time()
        except Exception:
            pass
        time.sleep(0.02)
ths = [threading.Thread(target=walker, args=(i,)) for i in range(3)]
for t in ths: t.start()
sw = threading.Thread(target=swapwatch); sw.start()
time.sleep(0.5)
...
for t in ths: t.join()
sw.join()
# read the count AFTER every walk has been answered: a walk in flight at
# the swap has, by then, taken the step that ends it on the swap
r_after = retire()
...
k = r_after.get("keys_ended_on_swap", 0)
if k >= 1:
    ok("%d walk(s) ended on the swap instead of reading the old table on" % k)
else:
    bad("no walk ended on the swap (keys_ended_on_swap=%r, %r at the retire poll) - the handle is not checked" % (k, r.get("keys_ended_on_swap")))
    # the timeline, so a zero is explained, not retried
    sa = swap_at[0]
    print("    swap seen at %s" % ("%.3fs after the resize verb" % (sa - t0) if sa else "NEVER (the bucket count never read 2^16 while walking)"))
    for i, sp in enumerate(spans):
        inflight = [x for x in sp if sa and x[0] < sa <= x[1]]
        before = [x for x in sp if sa and x[1] < sa]
        print("    conn %d: %d walks, %d answered before the swap, %d in flight at it%s" % (
            i, len(sp), len(before), len(inflight),
            (" (walk %.1f ms long)" % ((inflight[0][1] - inflight[0][0]) * 1000)) if inflight else ""))
