...
print("ok" if "align-items:center" in hdr and "baseline" not in hdr and same and px and 36<=int(px.group(1))<=40
      and "margin-left:auto" in ctl and "<div class=ctl>" in h and "(this node)" not in h else
      "hdr=%r same=%s px=%s ctl=%r"%(hdr[:60],same,px.group(1) if px else None,ctl[:40]))
')
[ "$S208" = ok ] && ok "S208: the header aligns on centre, both marks share --markpx (36-40 px), the controls are one right-aligned group, the option no longer says (this node)" \
	|| bad "S208: $S208"
