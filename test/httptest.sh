...
echo "$PG" | grep -qF "function sizePlane(" && echo "$PG" | grep -qF "sizePlane();trend();}" \
	&& ok "the plane's cards are sorted by their height on every redraw" \
	|| bad "the plane's cards are not sorted by height"
# the slow log and slowest-calls cards clip a long row (a key has nowhere
# to wrap, and the time was pushed off the card), and the slowest card
# reads each verb's histogram for its WORST bucket - a mean over 64,000
# SETs showed 7.6 us beside a slow log holding one of 10.1 ms
echo "$PG" | grep -qF ".pc.clip .kv>span:first-child{min-width:0;overflow:hidden;text-overflow:ellipsis" \
	&& echo "$PG" | grep -qF "card('slow log \\u00b7 tail'" && echo "$PG" | grep -qF "[['empty','']],'clip')" \
	&& echo "$PG" | grep -qF "function wmax(" && echo "$PG" | grep -qF "' \\u00b7 max '+wmax(x)" \
	&& ok "the slow log and slowest-calls rows clip, and the slowest card shows each verb's worst call" \
	|| bad "a slow log row can push its time off the card, or the slowest card has no max"
