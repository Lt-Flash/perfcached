	...
	for (;;) {
		...
		if (j->hi < sh->nb[j->col])
			snprintf(params, sizeof params,
				"{\"col\":\"%s\",\"cursor\":%u,\"count\":%d,\"end\":%u,\"table\":%llu}",
				sh->cols[j->col], cursor, count, j->hi, sh->tbl[j->col]);
		else
			snprintf(params, sizeof params,
				"{\"col\":\"%s\",\"cursor\":%u,\"count\":%d,\"table\":%llu}",
				sh->cols[j->col], cursor, count, sh->tbl[j->col]);
...
static int probe_table(struct shared *sh, int c)
{
	...
	snprintf(params, sizeof params, "{\"col\":\"%s\",\"cursor\":0,\"count\":1}", sh->cols[c]);
	res = perfd_command(p, "dump", params);
	toks = malloc((size_t)MAXTOK * sizeof *toks);
	if (res && toks && (ntok = pc_json_parse(res, strlen(res), toks, MAXTOK)) > 0) {
		tt = pc_json_get(res, toks, ntok, 0, "table");
		tb = pc_json_get(res, toks, ntok, 0, "buckets");
		if (tt >= 0 && tb >= 0) {
			sh->tbl[c] = strtoull(res + toks[tt].start, NULL, 10);
			sh->nb[c] = (unsigned int)strtoul(res + toks[tb].start, NULL, 10);
			rc = 0;
		} else
