...
#define P_KEY()   jw_kv_str(&w, "key", argv[2])
	...
	if (!strcmp(m, "ping") || !strcmp(m, "save") ||
	    ...
	    !strcmp(m, "fleetstop")) {       /* S222: needs privilege */
		return 0;                      /* no params */
	  ...
	  "  fleetstop - stop EVERY node cleanly (each snapshots on the\n"
	  "      way down); needs -E.  This node goes last, after it has\n"
	  "      reported which peers stopped and which did not\n"
