...
FILES="lib/perfd.c lib/perfd.h lib/perfd_push.h src/json.c src/json.h src/pc_noise.c src/pc_noise.h src/pc_slot.h src/pc_mix.h"
...
# the licence and the third-party notice travel with the code (S84):
# an export without them is MIT in name only
for d in lib/LICENSE lib/NOTICE; do
	[ -f "$SRC/$d" ] || { echo "sync-libperfd: $d missing - NOT exporting without it" >&2; exit 1; }
	cp "$SRC/$d" "$DST/lib/"
done
