...
# RUNTIME: podman by default, but this host has nerdctl and the rest of
# the tooling (bench/containers-up.sh, bench/containerlib.sh) already
# picks whichever is present.  A release tool that runs on one developer
# box is a release tool that gets bypassed.
RT=${RUNTIME:-}
[ -n "$RT" ] || for c in podman nerdctl docker; do
	command -v "$c" >/dev/null 2>&1 && { RT=$c; break; }
done
[ -n "$RT" ] || { echo "no container runtime (podman, nerdctl or docker)"; exit 1; }
echo "runtime: $RT"
mkdir -p "$OUT"
...
echo "$DISTROS" | while IFS= read -r line; do
	...
	echo "=== $name ($img) ==="
	if $RT run --rm --platform "$PLATFORM" -v "$work":/src -w /src "$img" sh -ec "
		$deps
