...
point() { # point <host> <port> <clients> <pipeline>
	redis-benchmark -h "$1" -p "$2" -t set,get -n "$(reqs "$4")" -c "$3" -P "$4" \
...
point_get() { # point_get <host> <port> <clients> <pipeline>
	redis-benchmark -h "$1" -p "$2" -t get -n "$(reqs "$4")" -c "$3" -P "$4" \
		-r "$KEYSPACE" -d "$VAL" --csv 2>/dev/null \
	| awk -F',' '
		function unq(s) { gsub(/"/, "", s); return s }
...
printf '# build=%s date=%s host=%s cpus=%s workers=%s keys=%s reqs=%sxP nmax=%s val=%s reps=%s\n' \
	"$BINREV" "$(date -u +%Y-%m-%dT%H:%MZ)" "$(hostname)" "$NCPU" \
	"$WORKERS" "$KEYSPACE" "$N" "$N_MAX" "$VAL" "$REPS" > "$D/results.tsv"
