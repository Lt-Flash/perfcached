...
#   this line once promised predates n_for sizing the cells)
...
printf '# build=%s date=%s host=%s runtime=%s cpus=%s workers=%s arena_mb=%s keys=%s nbase=%s val=%s reps=%s mtu=%s cpuset_srv=%s cpuset_cli=%s mems_srv=%s dialects=%s nat_secs=%s nat_lat=%s cli_threads=%s n_max=%s base=%s redis_img=%s route=%s\n' \
