...
probe_build() {
	...
	printf 'FROM docker.io/library/alpine:3.24\nRUN true\n' \
		> "$pd/Containerfile"
...
# Debian-based (glibc), NOT redis:8-alpine.  Two reasons, and the second
...
# THIS IMAGE, so an alpine tag means the resp arms are driven by a
...
# Container base.  DEBIAN (glibc) is the default because Alpine's musl
# allocator serialises libperfd's per-reply malloc on the JSON path: a
# musl-built client peaks at TWO threads and then goes BACKWARDS -
...
# printed a measurement of the allocator.
#
# The alpine file stays: musl is a supported build target and the
# portability check wants it.  It is just not what we benchmark on.
#
...
# alpine's busybox provides it - so switching REDIS_IMG to the Debian
# build silently turned this into an empty string and printed `mtu=?`
# into the provenance line.  /sys/class/net exists in every image.
