...
# three perfcached:alpine nodes (automatic membership over multicast on
# the bridge) + one redis:8-alpine, all on a user-defined bridge
# network.  Works with docker OR containerd/nerdctl - set RUNTIME, or
# it picks whichever is installed (nerdctl preferred).
#
...
# one node, fleet capacity = the sum of arenas).  With `wal`, every
...
echo "runtime: $RT   arena: ${ARENA}MB   durability: $WAL"
...
EOF
	...
	$RT run -d --name pcnode$i --network pcnet --ip 10.99.0.1$i \
		...
		$VOL perfcached:alpine -f /etc/perfcached.conf -D >/dev/null
done
...
if [ "$WAL" = wal ]; then
	...
	$RT run -d --name pcredis-aof --network pcnet --ip 10.99.0.21 \
		...
		redis:8-alpine --appendonly yes >/dev/null
fi
