...
**Pub/sub is the exception:** redis-benchmark cannot subscribe, so the
pub/sub comparison uses `bench/rpsbench.c`, one RESP client driving both
servers the same way (below).
...
## Pub/sub through a Redis client — the readings (rpscmp, 2026-09-17)

`bench/rpscmp.sh` puts Redis and perfcached in containers pinned to the
same eight cores of one 16-vCPU host and drives both from the other
eight with one client, `bench/rpsbench.c`: publishers and subscribers
over RESP on loopback, 64-byte payloads that carry a send time and a
sequence, so every subscriber measures latency and counts gaps.  Redis
8.10.1 runs with one and four I/O threads, perfcached with one and four
workers.  Four cells: one subscriber paced to 100k publishes/s and flat
out, 64 subscribers paced to 20k and flat out.  Rows, two repetitions
each: `results/rpscmp-e9b7fdb.tsv`; the build before push batching,
one repetition: `results/rpscmp-08b8d57.tsv`.

    IMG_PC=localhost/perfcached:<rev> ARMS="redis-io1 redis-io4 pc-w4 pc-w1" \
        bench/rpscmp.sh 2 8

| cell | Redis, 1 I/O thread | Redis, 4 I/O threads | perfcached, 1 worker | perfcached, 4 workers | 4 workers before (08b8d57) |
|---|---|---|---|---|---|
| 1 sub, flat out: publishes/s | 572-574k | 813-837k | 533-554k | 924-984k | 812k accepted, 97k delivered |
| 1 sub, 100k/s: p99 | 432-472 µs | 236-252 µs | 408-488 µs | 328-360 µs | 369 ms |
| 64 subs, 20k/s: p50 | 1.38-1.66 ms | 1.82-2.18 ms | 1.38-1.41 ms | 0.36-0.38 ms | 0.29 s at 7.3k/s |
| 64 subs, flat out: deliveries/s | 3.49-3.70M | 3.39-4.05M | 4.77M | 12.9-13.0M | 439k |

**Nothing was lost on the current build** - every message published
was read, in order, in every cell.  **The build before lost most of what
it accepted:** a worker wrote each delivered message the moment it was
staged, one write(2) a message, so the worker owning a subscriber
delivered ~100k messages a second and the 16 MB queue in front of it
dropped the rest (6,364,222 counted in the flat-out cell).  Staging the
frames and writing each connection once a turn is the change between
the two columns (see the changelog).

**Per core, one worker and Redis's one thread are close:** 3-7% behind
with one subscriber, 29-37% ahead at 64.  **Across cores perfcached
scales and Redis mostly does not:** four workers deliver 12.9M a second
to 64 subscribers against 3.4-4.0M, because the fan-out itself runs on
every worker; Redis's I/O threads speed up the one-subscriber cells and
leave fan-out on its main thread - and they spin, three cores busy at
100k/s paced.

What these numbers are not:
- **Loopback.**  Batched output rides a 64 KB MTU here; see the next
  section before carrying any of it to a network.
- **One node.**  A publish relayed across a fleet (PS3, PS11) is not in
  these cells, and neither is Redis Cluster.
- **Client-bound checked, not client-free.**  The driver's busiest
  thread stayed at or below 0.72 of a core in every cell (0.86 on the
  old build's fan-out); its latency
  is per read(2) batch, so messages in one batch share an arrival time.

## Pub/sub across two nodes, and through Noise — the readings (2026-09-17)

**Across the fleet.**  `bench/rpsrelay.sh` runs publishers against node
A and subscribers against node B, from a driver on a third host, so every
message crosses perfcached's relay - or, in the other arm, a two-master
Redis Cluster's bus - before it is delivered, and latency is read on one
clock.  Both servers pinned to eight cores on each node, perfcached with
four workers relaying everything, Redis 8.10.1 with one I/O thread, 64 B
payloads, jumbo frames end to end.  Rows, two repetitions:
`results/rpsrelay-7253754.tsv`.

    A_HOST=... B_HOST=... DRIVER=... IFACE_B=eth0 IFACE_DRIVER=eth0 \
    DRIVER_RPS=ff bench/rpsrelay.sh 2 8

**Look at the driver's receive path before any fan-out latency.**  The
driver here had one receive queue and no RPS, so all of its receive
softirq ran on one CPU; four perfcached workers write 2.4-3.5x the packets
one Redis thread does for the same bytes (209-276k against 65-98k a second
in the 64-subscriber cells), that CPU saturated, and 64 subscribers at 20k
publishes a second read p50 9.7 ms - 1.06 ms once RPS spread it.  That was
the driver's figure.  The rows above were taken with `DRIVER_RPS=ff`, and
`driver_softirq_cores` is in every one.

| cell | perfcached relay | Redis Cluster |
|---|---|---|
| 1 subscriber, 20k/s | p50 480-504 µs, p99 784-816 µs, nothing lost | p50 640 µs, p99 1.01 ms, nothing lost |
| 64 subscribers, 20k/s | p50 1.12-1.38 ms, p99 14.8-20.0 ms; relay lost 0.09-1.63% | p50 1.25-1.47 ms, p99 2.4-2.8 ms, nothing lost |
| 1 subscriber, flat out | 184-193k delivered/s, p50 0.74-0.85 ms; relay lost 1.9-3.6% | 124-143k accepted, 56-66k delivered/s, p50 2.69 s; a subscriber closed at its 32 MB output limit |
| 64 subscribers, flat out | 6.22-6.26M deliveries/s, p50 3.1-6.3 ms; relay lost 0.34-5.2% | 153-164k accepted, 0.74-1.88M deliveries/s, p50 7.6-8.4 s; output buffers 26-33 MB |

**Flat out the two fail in opposite directions.**  The bus is TCP: it
never drops a publish, so when B cannot keep up the publishes wait - for
seconds - and the subscriber's output buffer grows until Redis closes it.
The relay is sealed UDP, at-most-once by design: the node keeps delivering
at milliseconds and the datagrams its receive path cannot take are lost,
and counted - `relay_lost` on B, which the driver's loss matched exactly
in every cell.  A publisher on A cannot be paused for a subscriber on B,
so the queue-cap pause that keeps a single node lossless does not reach
across the relay.

**Loss at 20k/s with 64 subscribers is intermittent, and not new.**  Six
alternating runs of that cell alone: 889e0f6 lost datagrams in 1 of 3
(460 messages), 7253754 in 2 of 3 (290 and 124); p99 ranged 3.6-24 ms
across both.  B is busy writing 260-275k packets a second in that cell
and its relay receive threads lose the race now and then.

**Through Noise, on the native door.**  `bench/psnoise.sh` runs the
native JSON dialect through libperfd (`bench/psbench.c`) against one
daemon, plaintext (`plaintext = loopback`) and sealed (`plaintext =
never`), on one host.  Rows, two repetitions: `results/psnoise-7253754.tsv`.

| cell | plaintext | Noise | Noise / plaintext |
|---|---|---|---|
| 1 subscriber, flat out: deliveries/s | 612-645k | 451-515k | ~0.8x |
| 64 subscribers, 20k/s: server cores for 1.28M deliveries/s | 2.90-2.93 | 3.42-3.50 | +17-20% CPU |
| 8 subscribers, 1 KB, flat out: deliveries/s | 525-605k | 238-280k | ~0.5x |

Each subscriber's connection has its own key, so every delivery is
sealed again; the larger the payload, the more of the cost that is.
**Sixty-four subscribers flat out is a client ceiling here, not a server
figure:** plaintext showed a 70-78% shortfall against what was published,
with 5-17 subscribers closed as slow, and the subscriber process at 5.5
cores - one host's client cores could not read what the daemon wrote.
The table leaves those rows out; the TSV marks them with
`shortfall_pct` and `slow_kills`.

The same cells on 889e0f6, before notifications were built once per
message, took 3.73-3.74 server cores for the same 1.28M plaintext
deliveries a second - 2.90-2.93 now - and delivered 206-213k a second
of 1 KB payloads to 8 subscribers, against 525-605k now.
