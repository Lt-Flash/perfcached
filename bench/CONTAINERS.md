...
Redis 8 — building perfcached as a Debian 13 (**glibc**) image and
running it under **containerd (nerdctl)** or **docker**.  Everything a
third party needs is in this directory: the Containerfile, the
bring-up script, the measurement battery, and the client tools baked
into the image.

> **glibc only.**  There is one Containerfile and one image.  A musl
> variant existed until 2026-09-02 and was deleted, not demoted: its
> allocator makes the daemon behave differently enough that anything
> measured or asserted on it is partly measuring the allocator.  See
> the table below.
...
## Topology
    ...
    │  │  automatic membership - no ids, no peer lists      │   redis:8         │
    │  └────────────────────────────────────────────────────┘                   │
...
**Why glibc and nothing else**, at +48.5 MB (53.5 -> 102 MB): musl's
...
The musl Containerfile has been **deleted**. Keeping it as an opt-in
knob was the mistake: `bench/containers-up.sh` reached for it for every
fleet it ever built, and nobody noticed for a month. Portability is
covered by [tools/matrix.sh](../tools/matrix.sh), which cross-builds
four architectures on `debian:trixie-slim` — a compile check that never
produces a runnable fleet.
...
## 3. Pull Redis 8
    ...
    nerdctl pull redis:8                      # or docker pull
...
> **RETIRED — these are musl numbers, and nothing below should be
> quoted.**  The 2026-08-26 pass ran a musl image (deleted 2026-09-02),
> whose allocator serialises libperfd's per-reply malloc: a musl-built
> client peaks at two threads and then goes backwards, 20.6x behind
> glibc at eight.  Whatever these tables measured, part of it was the
> allocator — on BOTH sides, since the Redis image was musl too, so
> even the comparison is not a clean one.  The topology is
> `perfcached:debian` + `redis:8` now and the matrix owes a fresh pass.
> The rows stay because a board is append-only — not because they stand.

Measured **2026-08-26** on a 16-vCPU build host: a musl image of this
repo (since deleted), nerdctl/containerd on a CNI bridge, against a
**musl Redis 8.10.1 (jemalloc)** — matrix passes across the
...
Running the daemon on a different libc for the first time surfaced two
real portability bugs and two performance cliffs — all fixed in this
repo, all invisible on glibc: 128 KB default thread stacks vs ~130 KB of
