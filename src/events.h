 ...
 * Defined in config.c, which links standalone in wipetest. */
...
/* PS8: the same kinds published as Redis keyspace notifications - on
 * __keyspace@<collection>__:<key> (payload: the event) and
 * __keyevent@<collection>__:<event> (payload: the key) - to THIS node's
 * subscribers only, never relayed: each node emits for what it applies,
 * as Redis Cluster's primaries do.  Off unless a mask names it. */
void pc_ev_set_notify_default(unsigned int mask);
void pc_ev_configure_notify(int slot, unsigned int mask);   /* PC_EV_INHERIT = default */
/* a record applied on this node from a peer - a forwarded write at its
 * holder, an eager replica - is a store here, and notifies as one */
...
extern unsigned int pc_ev_notify_any;   /* PS8: any collection notifying */
