		...
		const struct pc_psk_ctx *psk, struct pc_conn **list)
{
	...
	for (;;) {
		...
		if (fd < 0)
			return;
		if (resp_only && !pc_resp_peer_allowed(&peer, plen)) {
 ...
 * under musl, where each 1MB allocation is an mmap+munmap+fault
 * cycle: the Alpine container bench measured a ~50x throughput loss
 * before the scratch became thread-owned.  Workers live as long as
...
int pc_conn_event(struct pc_conn *c, uint32_t events)
{
	...
	if (c->close_after && !c->out_len && c->wire_off >= c->wire_len)
		goto drop;                     /* QUIT: reply flushed, close */
	return 0;
drop:
	conn_destroy(c);
	return -1;
