		...
		size_t *argl, int nargs)
{
	...
	if (es.start) {
		...
		c->enum_ht = es.ht;
		c->enum_col = pc_store_slot_of(es.ht);
		c->enum_pub = c->enum_col >= 0 ? pc_store_pub_gen(c->enum_col) : 0;
...
void pc_proto_pull_complete(const struct pc_pull_done *d)
{
	...
	if (d->kind == PC_DONE_PULL && d->found) {
		...
		if (ht && pc_store_shard_enabled(ht)) {
			 ...
			 * owner */
		} else if (ht && pc_cluster_replicas() &&
		...
		const char *chan, size_t clen, const char *data, size_t dlen)
{
	...
	size_t n = 1 + 2 + plen + 2 + clen + 4 + dlen;
		...
		uint64_t *seq, const char **err)
{
	...
	if (n != 2 * PFP_COOKIE || sodium_hex2bin(ck, sizeof ck, cookie_hex, n,
		...
		const char *resp_frame, size_t flen)
{
	 ...
	 * (decision 4, DESIGN 12eb). */
	{
		...
		pc_jw_lit(&w, "}}");
		rc = w.overflow ? PC_PS_PUSH_NOMEM : pc_conn_notify(c, w.buf, w.len);
		free(buf);
		return rc;
	}
}
