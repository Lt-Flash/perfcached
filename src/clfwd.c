...
{
	size_t len = 1 + key_tail_size(k) + 8;
	struct pc_wcur c;

	pc_wcur_init(&c, msg, len);
	pc_w8(&c, type);
	key_tail_build(&c, k);
	pc_w64(&c, ver);                   /* S209: after the key, see clfwd.h */
	return c.ok ? len : 0;
}
		...
		uint64_t *ver)
{
	...
	*ver = 0;
	...
	if (!pc_rskip(&c, 1))
		return 0;
	if (!key_tail_parse(&c, pt, n, CLFWD_TOMB_HDR, k))
		return 0;
	tail = CLFWD_TOMB_HDR + k->collen + k->klen;
	if (n >= tail + 8)
		*ver = pc_g64(pt + tail);        /* a versioned sender */
	return 1;
}
...
{
	struct clfwd_op f = {
		.req = req, .node = (unsigned int)node,
