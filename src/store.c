...
} colst[PC_MAX_COLLECTIONS][COLST_STRIPES] __attribute__((aligned(64)));
		...
		const struct pcache_rec_meta *mt, void *p)
{
	...
	if (pcache_ht_store_ver(c->to, key, val, mt->expires,
	        (unsigned char)(mt->rflags & PCACHE_F_PASSIVE), mt->ver) == 0)
		c->moved++;
	return 0;
}
