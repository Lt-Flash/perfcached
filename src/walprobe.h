...
struct pc_wal_probe {
	int valid;
	long long sync_bytes;          /* phase B volume - the evidence
					* behind the ceiling below, and the
					* reason it is only a ceiling */
...
struct pc_wal_policy {
	...
	/* A CEILING derived from a burst, never a sustained rate.  Phase B
	 * writes at most 16 MB (4000 x 4K) and stops after ~1.2s, so on any
	 * storage with a write-back cache in front of it - which every
	 * Proxmox guest running cache=writeback has - the burst never
	 * reaches the device.  Measured 2026-09-03: probe 139us p50 and
	 * ~3467/s where sustained fio saw 3.7ms and 236/s.  Treat as an
	 * upper bound; see DESIGN section 12am. */
	long long max_durable_wps;
