...
void pc_stats_json(struct pc_jw *out, const char *only_col)
{
	...
	for (i = 0; i < pc_store_count(); i++) {
		 ...
		 * growth is blocked */
		pc_jw_lit(out, ",\"autoscale\":\"");
		pc_jw_lit(out, pc_autoscale_name(pc_store_autoscale(i)));
		pc_jw_lit(out, "\",\"autoscale_target_log2\":");
		pc_jw_i64(out, pc_store_autoscale_target(i));
	...
	pc_jw_lit(out, "],\"cluster\":");
	{
		...
		if (!cs.enabled) {
			...
			pc_jw_lit(out, ",\"unadmitted\":");   /* S241 */
			pc_jw_i64(out, (long long)cs.map_unadmitted);
			...
			pc_jw_lit(out, ",\"neg_displaced\":");   /* RV-15 */
			pc_jw_i64(out, (long long)cs.neg_displaced);
			pc_jw_lit(out, ",\"tomb_displaced\":");
			pc_jw_i64(out, (long long)cs.tomb_displaced);
	...
	pc_jw_lit(out, ",\"wal\":");
	{
		...
		if (!ws.enabled) {
			...
			pc_jw_lit(out, ",\"heals\":");          /* S229 */
			pc_jw_i64(out, (long long)ws.heals);
			pc_jw_lit(out, ws.healing ? ",\"healing\":true" : ",\"healing\":false");
			pc_jw_lit(out, ",\"last_heal_ms\":");
			pc_jw_i64(out, ws.last_heal_ms);
			...
			pc_jw_i64(out, ws.free_segments);
			{
				...
				char fb[128];      /* RV-13: three figures now */
				int fl = snprintf(fb, sizeof fb,
					",\"fill_mb_s\":%.1f,\"full_in_s\":%.1f,\"ring_fill_s\":%.1f",
					ws.fill_mb_s, ws.full_in_s, ws.ring_fill_s);
...
#define NEEDKEY() do { \
	...
	if (pc_json_streq(line, m, "autoscale")) {
		...
		if (nl <= 0)
			ERR(E_PARAMS, "missing col");
		nm[nl] = '\0';
		ml = get_str(line, toks, ntok, params_tok, "mode", NULL, md,
			sizeof md - 1);
		if (ml <= 0)
			ERR(E_PARAMS, "missing mode (auto|warn|off)");
		md[ml] = '\0';
		if ((mode = pc_autoscale_parse(md)) < 0)
			ERR(E_PARAMS, "mode is auto, warn or off");
		...
		pc_jw_str(out, nm, (size_t)nl);
		pc_jw_lit(out, ",\"autoscale\":\"");
		pc_jw_lit(out, pc_autoscale_name(mode));
		pc_jw_lit(out, "\"}");
		return 0;
	}
