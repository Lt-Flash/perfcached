...
#define PC_DUMP_TAIL_LIMIT 12288           /* S112: walked records per tail call - ~3.3 MB of 272 B records, under JW_HEAP_MAX */
		...
		size_t vlen, unsigned int exp, unsigned long long ver)
{
	...
	if (!pc_cluster_enabled() || !pc_store_eager_enabled(ht))
		return;
	cn = col_name_of(ht);
	pc_repl_push(cn, strlen(cn), k->s, (size_t)k->len, val, vlen, exp, ver);
}

/* after a touch: bytes, expiry, flags and version out of ONE read, so
 * the copy pairs the right value with the right version - and a
 * passive record is never re-sent */
static void eager_push_current(pcache_htable_t *ht, const str *k)
{
	str val;
	unsigned int exp = 0;
	unsigned char fl = 0;
	...
	if (!pc_cluster_enabled() || !pc_store_eager_enabled(ht))
		return;
	if (pcache_ht_fetch_full(ht, k, &val, &exp, &fl, &ver) != 0 ||
	        (fl & PCACHE_F_PASSIVE))
		return;
	eager_push(ht, k, val.s, (size_t)val.len, exp, ver);
}
...
static int op_expire(pcache_htable_t *ht, str *k, long long ttl)
{
	...
	if (rc == 1) {
		pc_wal_touch(col_name_of(ht), k->s, k->len, ttl_to_abs(ttl),
			pcache_last_ver);
		eager_push_current(ht, k);     /* the new TTL travels too */
	}
	return rc == 1 ? PC_OP_OK : PC_OP_ABSENT;
}
...
struct dump_keys {
	...
	int full;
	/* the tail phase: the last bucket and the overflow leg come out of
	 * one scan call as one unit, unbounded by the bucket budget, so the
	 * verb re-walks that unit skipping what earlier calls emitted and
	 * stopping after `limit` walked records; `walked` counts every
	 * record the walk offered, before the slot filter, so every
	 * connection's skip means the same thing */
	unsigned long skip, limit, walked;
	int stopped;
};
		...
		void *p)
{
	...
	(void)val;
	if (dk->limit) {
		if (dk->walked++ < dk->skip)
			return 0;
		if (dk->walked > dk->skip + dk->limit) {
			dk->stopped = 1;
			return -1;             /* the rest waits for the next call */
		}
	}
	if (exp && exp <= dk->now)
...
void pc_stats_json(struct pc_jw *out, const char *only_col)
{
	...
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_resp_reqs));
	pc_jw_lit(out, ",\"slots_hits\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_resp_slots_hits));
	pc_jw_lit(out, ",\"slots_builds\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_resp_slots_builds));
	pc_jw_lit(out, "}");
	...
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_nat_conns));
	pc_jw_lit(out, ",\"json\":{\"conns\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_nat_text_conns));
	pc_jw_lit(out, ",\"requests\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_nat_text_reqs));
	pc_jw_lit(out, "},\"binary\":{\"conns\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_nat_bin_conns));
	pc_jw_lit(out, ",\"requests\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_nat_bin_reqs));
	pc_jw_lit(out, "},\"resp\":{\"conns\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_nat_resp_conns));
	pc_jw_lit(out, ",\"requests\":");
	pc_jw_i64(out, (long long)PC_RESP_READ(pc_nat_resp_reqs));
	pc_jw_lit(out, "}}");
...
#define NEEDKEY() do { \
	...
	if (pc_json_streq(line, m, "collections")) {
		...
		for (i = 0; i < pc_store_count(); i++) {
			...
			pc_jw_str(out, pc_store_name(i), strlen(pc_store_name(i)));
			pc_jw_lit(out, ",\"mode\":\"");
			pc_jw_lit(out, pc_store_shard_enabled(cht) ? "shard"
	...
	/* ---- S112: dump {col, cursor?, count?, slot_lo?, slot_hi?} ----------
	...
	if (pc_json_streq(line, m, "dump")) {
		...
		long long tlim = PC_DUMP_TAIL_LIMIT;
		unsigned int cursor;
		size_t off, mark;
		int n = 0;
		...
		/* tail_limit: walked records per tail call, the reply's budget
		 * by default; a caller may lower it (a test exercising the
		 * chunking on a small table, a tool pacing itself) */
		get_int(line, toks, ntok, params_tok, "tail_limit", &tlim);
		if (tlim < 1 || tlim > 65536)
			ERR(E_PARAMS, "tail_limit out of range");
		...
		if (!dk.buf)
			ERR(-32603, "out of memory");
		/* Two phases in one cursor space.  Below nb - 1 the chunk is
		 * `count` buckets and never reaches the last one; from nb - 1
		 * on it is the TAIL - the last bucket plus the overflow leg,
		 * which the scan drains whole in its final call (a table at
		 * load factor four keeps some percent of its records there,
		 * unbounded in one reply) - re-walked with a skip of what
		 * earlier calls emitted and a limit of 4 x count walked
		 * records; tail cursors are nb + skip.  Duplicates cannot
		 * arise: bucket chunks stop short of the tail, and the tail's
		 * order is stable while the table is quiet. */
		{
			...
			if (cursor >= nb - 1) {
				tail = 1;
				skip = cursor >= nb ? (unsigned long)cursor - nb : 0;
			}
			if (endb >= 0 && (unsigned long long)endb < (unsigned long long)nb - 1 &&
			    !tail && cursor >= (unsigned int)endb) {
				free(dk.buf);
				ERR(E_PARAMS, "cursor at or past end");
			}
			if (tail) {
				unsigned int tc = nb - 1;

				dk.skip = skip;
				/* the tail's limit is the reply's budget in walked
				 * records, never a function of count: the leg is
				 * re-walked from its start on every call, so the
				 * cost is (leg^2 / limit) - 256-bucket chunks on a
				 * million-record table measured ten seconds, the
				 * same walk at this limit under two */
				dk.limit = (unsigned long)tlim;
				rc = pcache_ht_scan_ex(ht, &tc, 1, PCACHE_SCAN_NOVAL,
					dump_key_cb, &dk);
				if (dk.full) {
					free(dk.buf);
				...
				if (rc < 0 && !dk.stopped) {
					free(dk.buf);
					ERR(-32603, "walk failed");
				}
				/* stopped = more of the tail remains; else it is done */
				cursor = dk.stopped ? (unsigned int)(nb + skip + dk.limit) : 0;
				/* a walker that owns only part of the table never
				 * has a tail; reached only via cursor >= nb - 1 */
			} else {
				unsigned int room = nb - 1 - cursor;   /* buckets before the tail */
				unsigned int take = (unsigned int)count < room ? (unsigned int)count : room;
				unsigned int stop = nb - 1;            /* where this walker's range ends */

				if (endb >= 0 && (unsigned long long)endb < (unsigned long long)nb - 1) {
					stop = (unsigned int)endb;
					if (take > stop - cursor)
						take = stop - cursor;
				}
				...
				if (stop < nb - 1) {
					/* a bounded range: done at its end, no tail */
					if (cursor == 0 || cursor >= stop)
						cursor = 0;
				} else if (cursor == 0 || cursor > nb - 1)
					cursor = nb - 1;       /* next call starts the tail */
			}
		}
	 ...
	 * collection argument is a glob, the answer is grouped by collection,
	 * and the limit applies across the WHOLE result (default 1000), or
	 * one large collection starves the rest and the reply silently lies
	 * about the others.  Enumeration is the most expensive thing this
	 * daemon does, so this is a bounded convenience over the same walk
	 * scan uses, never a new unbounded one. */
	if (pc_json_streq(line, m, "keys")) {
		...
		if (tc < 0 || memchr(cg, '*', cgl) || memchr(cg, '?', cgl) ||
			...
			int i, first = 1, matched = 0, stopped = 0;
			...
			pc_jw_lit(out, "{\"collections\":{");
			for (i = 0; i < pc_store_count() && !stopped; i++) {
				...
				pc_jw_lit(out, "]");
				limit -= sc.emitted;   /* the cap is across the whole result */
				if (sc.stopped || limit <= 0)
					stopped = 1;
			}
			pc_jw_lit(out, "},\"truncated\":");
			pc_jw_lit(out, stopped ? "true" : "false");
		...
		int nm, int shards)
{
	...
	int i;
#define CLMAP_MIX(_x) do { unsigned long long _v = (unsigned long long)(_x); \
	int _b; for (_b = 0; _b < 8; _b++) { \
		h ^= (_v >> (_b * 8)) & 0xff; h *= 1099511628211ULL; } } while (0)
	CLMAP_MIX(nm);
	CLMAP_MIX(shards);
	for (i = 0; i < nm; i++) {
		...
		struct pc_enum_start *es, void *obs)
{
	...
	if (resp_is(cmd, cl, "HELLO")) {
		...
		resp_arr(out, 14);
		resp_bulk_lit(out, "server");
		resp_bulk_lit(out, "perfcached");
		resp_bulk_lit(out, "version");
		resp_bulk(out, PC_VERSION, strlen(PC_VERSION));
		resp_bulk_lit(out, "proto");
		resp_int(out, 2);
		resp_bulk_lit(out, "id");
		resp_int(out, 0);
		resp_bulk_lit(out, "mode");
		resp_bulk_lit(out, "standalone");
		resp_bulk_lit(out, "role");
		resp_bulk_lit(out, "master");
		resp_bulk_lit(out, "modules");
		resp_arr(out, 0);
		return 0;
	}
	...
	if (resp_is(cmd, cl, "CLUSTER")) {
		...
		if (resp_is(sub, subl, "SLOTS") || resp_is(sub, subl, "SHARDS")) {
			...
			if (resp_clmap_cached(is_shards, key, out)) {
			...
			if (pc_jw_init_heap(&tw, (size_t)nr * 104 +
			        (size_t)nm * 256 + 64) != 0) {
				free(ownv);
				resp_err(out, "could not build the slot map");
				return 0;
			}
			if (!is_shards) {
				resp_arr(&tw, nr);
			...
			if (tw.overflow) {
				...
				if (pc_jw_init_heap(out, tw.len + 16) != 0)
					resp_err(out, "could not build the "
						"slot map");
				else
					pc_jw_raw(out, tw.buf, tw.len);
			}
			pc_jw_free(&tw);
			return 0;
		}
	...
	if (resp_is(cmd, cl, "CLIENT")) {
		if (nargs == 3 && resp_is(argv[1], argl[1], "SETNAME")) {
			...
			if (b.overflow)
		        ...
		        "GETNAME")) {
			resp_bulk_lit(out, "");   /* name echo: minimal */
		} else {
...
#undef INFO_WANT
	 ...
	 * answering "no such collection"). */
	{
		static const char *const dc[] = { "GET", "SET", "SETEX",
			...
			"JSON.SET", "JSON.GET", "JSON.DEL", "JSON.NUMINCRBY",
			"JSON.ARRAPPEND", "JSON.DEBUG" };
		size_t i;
		int known = 0;
	...
	*collenp = *cur_collen;
	...
	if (resp_is(cmd, cl, "JSON.DEBUG")) {
		if (nargs >= 2 && resp_is(argv[1], argl[1], "HELP")) {
			resp_arr(out, 2);
			resp_bulk_lit(out, "JSON.DEBUG MEMORY <key> [path] - reports "
				"memory usage");
			resp_bulk_lit(out, "JSON.DEBUG HELP - this message");
			return 0;
		}
		if (nargs >= 3 && resp_is(argv[1], argl[1], "MEMORY")) {
			char *frag = NULL;
			const char *fmsg = NULL;
			long long nv = 0;
			...
			rc2 = pc_json_rmw(ht, argv[2], (int)argl[2], PC_JOP_GET,
			...
			if (rc2 == 0 && jplen == 1 &&
				...
				free(frag);
				resp_err_code(out, "WRONGTYPE", "the value at this key "
					"is not a JSON document");
				return 0;
			}
			resp_int(out, rc2 == 1 ? 0 : fraglen);
			free(frag);
			return 0;
		}
		resp_err(out, "JSON.DEBUG: use HELP or MEMORY <key> [path]");
		return 0;
	}
	if (resp_is(cmd, cl, "JSON.SET") || resp_is(cmd, cl, "JSON.GET") ||
	        resp_is(cmd, cl, "JSON.DEL") ||
	        resp_is(cmd, cl, "JSON.NUMINCRBY") ||
	        resp_is(cmd, cl, "JSON.ARRAPPEND")) {
		...
		if (resp_is(cmd, cl, "JSON.SET"))
			op = PC_JOP_SET;
		else if (resp_is(cmd, cl, "JSON.GET"))
			op = PC_JOP_GET;
		else if (resp_is(cmd, cl, "JSON.DEL"))
			op = PC_JOP_DEL;
		else if (resp_is(cmd, cl, "JSON.NUMINCRBY"))
			op = PC_JOP_INCR;
		else
			op = PC_JOP_APPEND;
		if (nargs < 2 || (op == PC_JOP_SET && nargs < 4) ||
		        (op == PC_JOP_INCR && nargs != 4) ||
		        (op == PC_JOP_APPEND && nargs < 4)) {
			resp_err(out, "wrong number of arguments for this JSON command");
			return 0;
		}
		...
		*klenp = argl[1];
		...
		if (nargs >= 3 && !(op == PC_JOP_GET && nargs == 2)) {
			const char *pp = argv[2];
			size_t pl = argl[2];

			if (pl == 0 || pl + 2 >= sizeof jpath) {
				resp_err(out, "bad path");
				return 0;
			}
			if (pp[0] == '$') {
				v2 = 1;
		...
		if (op == PC_JOP_SET) {
			...
			svlen = argl[3];
			for (i = 4; i < nargs; i++) {
				if (resp_is(argv[i], argl[i], "NX")) {
					nx = 1;
				} else if (resp_is(argv[i], argl[i], "XX")) {
					xx = 1;
				} else if (resp_is(argv[i], argl[i], "EX") &&
				        i + 1 < nargs) {
					if (resp_ll(argv[i + 1], argl[i + 1], &jttl) < 0 ||
					        jttl <= 0) {
						resp_err(out, "invalid expire time in "
							"'json.set' command");
						return 0;
					}
					have_ttl = 1;
					...
					resp_err(out, "unsupported JSON.SET option");
					return 0;
				}
			}
		} else if (op == PC_JOP_INCR) {
			if (resp_ll(argv[3], argl[3], &jby) < 0) {
				resp_err(out, "value is not an integer (JSON.NUMINCRBY "
		...
		if (pc_cluster_enabled() &&
		        ((pc_store_proxy_enabled(ht) &&
		          pcache_ht_probe(ht, &k, NULL, NULL, NULL) != 0) ||
		         (pc_store_shard_enabled(ht) &&
		          pc_shard_owner(col_name_of(ht), strlen(col_name_of(ht)),
		              k.s, (size_t)k.len) != 0))) {
			resp_err(out, "JSON on a key another node holds is not "
		...
		if (op == PC_JOP_APPEND) {
			...
			for (i = 3; i < nargs; i++) {
				rc2 = pc_json_rmw(ht, k.s, k.len, PC_JOP_APPEND,
					jpath, jplen, argv[i], (int)argl[i], 0, 0, 0,
		...
		rc2 = pc_json_rmw(ht, k.s, k.len, op, jpath, jplen, sval,
		...
		if (rc2 < 0) {
			...
			if (fmsg == jp_strerror(PC_JP_E_DOC))
				resp_err_code(out, "WRONGTYPE", "the value at this "
		...
		default:                       /* GET */
			if (rc2 == 1) {
				...
				if (rootpath)
					...
					resp_err(out, "path does not exist");
			} else if (rootpath &&
			        !pc_jp_valid_value(frag, (size_t)fraglen)) {
				...
				pc_jw_lit(out, "$");
				pc_jw_i64(out, (long long)fraglen + 2);
				pc_jw_lit(out, "\r\n[");
				pc_jw_raw(out, frag, (size_t)fraglen);
				pc_jw_lit(out, "]\r\n");
			} else
				resp_bulk(out, frag, (size_t)fraglen);
			free(frag);
		}
		return 0;
	}
