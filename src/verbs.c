 ...
 * malloc'd VAL_WIRE_MAX buffer per request - cheap under glibc,
 * costly under musl (the Alpine bench; see proto.c's jw scratch for
		...
		unsigned int *park, int probed)
{
	 ...
	 * (or placement keeps the new key local) */
		...
		if (!target && !probed &&
		        !pc_neg_hit(cn, strlen(cn), k->s, (size_t)k->len)) {
			/* holder unknown and not recently-confirmed-absent:
			 * probe the fleet BEFORE placement can fork an
			 * existing key (the locator is a cache; an evicted
			 * entry must not turn a re-write into a second
			 * holder).  The get-miss->set pattern skips this: the
			 * get already stamped the negative cache. */
			unsigned int req = pc_probe_fwd_begin(0, cn,
				strlen(cn), k->s, (size_t)k->len, v->s,
				(size_t)v->len, ttl > 0 ? (unsigned int)ttl : 0,
				0);
			...
			/* no peers / oversized: place as before */
		}
		if (!target)
		...
		/* placement chose LOCAL: fall through to the local store */
	}
