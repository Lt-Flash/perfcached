...
#define NEEDKEY() do { \
	...
	if (pc_json_streq(line, m, "create")) {
		...
		if (rc2 == -3)
			ERR(-32000, "the table could not be created - the arena "
				"has no room for its index");
		if (rc2 != 0)
		...
		pc_store_persist();
		pc_cluster_col_announce(nm, (size_t)nl, (int)bl, 0);
	...
	if (pc_json_streq(line, m, "drop")) {
		...
		pc_wal_col_drop(nm);
		pc_store_persist();
		pc_cluster_col_announce(nm, (size_t)nl, 0, 1);
	...
	if (pc_json_streq(line, m, "resize")) {
		...
		if (rc2 == -4)
			ERR(-32000, "the new table could not be created - the "
				"arena has no room for its index");
		if (rc2 == -5)
			ERR(E_PARAMS, "the collection is already that size");
		pc_cluster_col_announce(nm, (size_t)nl, (int)bl, 3);   /* S69: resize */
		...
		struct pc_enum_start *es, void *obs)
{
	...
	if (resp_is(cmd, cl, "SELECT")) {
		...
		{
			char nm[64];
			size_t nl = resp_map_index(argv[1], argl[1], nm,
				sizeof nm);   /* S69 */
			...
			if (!pc_store_find(nm, nl)) {
				resp_err(out, "DB index is out of range (no "
