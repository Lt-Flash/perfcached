 ...
 * directory exists but the term file there does not validate, which is
 * worth refusing startup for: a node that silently forgets its term can
 * reissue one.  S106: the file is three lines of text (a version line,
 * `term <n>`, `check <16 hex>`); the legacy eight-byte form is read
 * once and rewritten. */
/* S80: the term file's name, so the cluster can carry it from a WAL
 * directory into a newly introduced state_dir; the file itself stays
 * this module's to read and write */
#define PC_TERM_FILE "node-term"
