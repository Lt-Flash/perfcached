...
#define NEG_WAYS      4          /* RV-15: set-associative, oldest-expiry out */
...
struct clloc {
	/* S209: a negative entry carries the VERSION of the delete that
	 * planted it (0 from an older sender), so a copy of the record that
	 * arrives after the tombstone can be told from a genuinely newer
	 * write.  The write-path push batches for up to REPL_FLUSH_MS while
	 * a tombstone goes at once, so the two cross on the wire whenever a
	 * set and its delete are closer than the flush. */
	struct { uint64_t h; long long exp_ms; uint64_t ver; } neg[NEG_SLOTS];
	struct { uint64_t h; uint16_t node; } loc[LOC_SLOTS];
	unsigned long long neg_hits, loc_hits, loc_clears;
	/* RV-15: the table is direct-mapped, so a set on a slot holding a
	 * LIVE entry for another key displaces it.  neg_displaced counts
	 * every such overwrite; tomb_displaced the dangerous subset - a live
	 * VERSIONED tombstone (S209), the key's only protection against a
	 * copy of the deleted record still crossing on the wire */
	unsigned long long neg_displaced, tomb_displaced;
};
...
/* S209: plant with the delete's version; clloc_neg_set keeps 0 */
void clloc_neg_set_ver(struct clloc *l, const char *col, size_t cn,
		const char *key, size_t kn, long long now, int ms, uint64_t ver);
/* 1 when a LIVE negative entry exists for the key; *ver is what the
 * delete carried (0 = unversioned).  Does not count as a hit. */
...
unsigned long long clloc_neg_displaced(const struct clloc *l);   /* RV-15 */
unsigned long long clloc_tomb_displaced(const struct clloc *l);
