...
const char pc_status_page[] =
	...
	"<th class=n title=\"entries x mean stored size: an estimate from the write stream, not a walk\">&asymp; held</th>\n"
	"<th title=\"value sizes stored, log2 classes from 64 B to 256 KB+\">sizes</th></tr></thead>\n"
	...
	"var HP=(location.port||\"8080\"),last=0,H={},HMAX=1200,POLL=3000,P=null;\n"
	...
	"function spark(a){var w=240,h=80,n=a.length,i,lo,hi,p=[],x,y,g=\"\";\n"
	...
	"var d=p.join(\" \");\n"
	/* S91: quarter lines across the plotted band.  The footer states min
	 * and max, so a line at each quarter is readable without text inside
	 * the svg - which preserveAspectRatio=none would stretch. */
	...
	"'</span><span>'+(a.length>1?dur((a.length-1)*POLL/1000)+' ago \\u2192 now':'first sample')+\n"
	...
	"(x.stored_n?sz((x.entries||0)*(x.stored_bytes/x.stored_n)):'<span class=dim>\u2014</span>')+\n"
	"'</td><td>'+hist(x.size_hist)+'</td></tr>';}\n"
	"document.getElementById(\"cols\").innerHTML=o||\n"
	"'<tr><td colspan=11 class=dim>no collections</td></tr>';\n"
