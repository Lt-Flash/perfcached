...
const char pc_status_page[] =
	...
	"lf=t?100*x.hits/t:null,pv=PC[x.name],rt=null,wn=-1,\n"
	"ld=x.buckets?100*x.entries/x.buckets:0;\n"
	"if(pv){var dh=(x.hits||0)-pv.h,dm=(x.misses||0)-pv.m;\n"
	"if(dh>=0&&dm>=0){wn=dh+dm;if(wn>0)rt=100*dh/wn;}}\n"
	"NC[x.name]={h:x.hits||0,m:x.misses||0};\n"
	"var k=rt===null?\"\":(rt<25?\"crit\":rt<60?\"low\":\"\");\n"
	...
	"'<td><div class=hit>'+(x.reach===undefined?'<span class=dim>\u2014</span>':\n"
	"'<span class=hv><span class=n>'+nf(x.reach)+'</span><span class=hl>'+\n"
	"(x.entries?(100*x.reach/x.entries).toFixed(x.reach*100<x.entries?2:1)+'% of held':'of 0 held')+\n"
