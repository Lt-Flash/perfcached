...
const char pc_status_page[] =
	 ...
	 * the title says so. */
	"function cmdcards(s){var cm=(s.commands||[]).filter(function(x){return wcalls(x)>0;}),sl=(s.slowlog||[]).slice(0,6),now=Date.now()/1000,a;if(!cm.length&&!sl.length)return'';\n"
	"a=cm.sort(function(x,y){return wcalls(y)-wcalls(x);}).slice(0,8).map(function(x){return [vname(x),nf(wcalls(x))+' \\u00b7 '+lat1(wmean(x))];});\n"
	"return card('commands \\u00b7 by calls, 5 min',a.length?a:[['nothing in the last 5 min','']])+\n"
