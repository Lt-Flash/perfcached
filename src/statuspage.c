...
const char pc_status_page[] =
	...
	"var RANK={failed:0,draining:1,recovering:1,starting:1,ready:3,gone:4};\n"
	...
	"function drawCmds(s){var cm=(s.commands||[]).filter(function(x){return wcalls(x)>0;}),o='',i,x;\n"
	...
	"function slow(x){var m=wq(x,'max_us');return m>0?m:worst(x);}\n"
	"cm.sort(function(x,y){return slow(y)-slow(x)||wq(y,'p99_us')-wq(x,'p99_us')||wq(y,'usec')/wcalls(y)-wq(x,'usec')/wcalls(x);});\n"
	"for(i=0;i<cm.length;i++){x=cm[i];\n"
	"o+='<tr'+(i?' class=cx':'')+'><td>'+esc(x.verb||x.name)+(x.dialect?' <span class=dim>('+esc(x.dialect)+')</span>':'')+'</td><td class=n>'+nf(wcalls(x))+'</td><td class=n>'+lat1(wq(x,'usec')/wcalls(x))+\n"
