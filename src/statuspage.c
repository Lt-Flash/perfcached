...
const char pc_status_page[] =
	...
	"function lat(u){u=+u;return !(u>=0)?'\\u2014':u<1000?nf(Math.round(u))+' us':u<1e5?(u/1000).toFixed(1)+' ms':u<1e6?Math.round(u/1000)+' ms':(u/1e6).toFixed(1)+' s';}\n"
	...
	"function cmdcards(s){var cm=(s.commands||[]).slice(),sl=(s.slowlog||[]).slice(0,6),now=Date.now()/1000,a,b;if(!cm.length&&!sl.length)return'';\n"
	...
	"b=cm.filter(function(x){return x.calls>0;}).sort(function(x,y){return worst(y)-worst(x)||y.p99_us-x.p99_us||y.usec/y.calls-x.usec/x.calls;}).slice(0,8).map(function(x){\n"
	"return ['<span title=\"'+esc(x.name)+' \\u00b7 '+nf(x.calls)+(x.calls==1?' call':' calls')+' \\u00b7 mean '+(x.usec/x.calls).toFixed(1)+' us \\u00b7 p50 '+lat(x.p50_us)+' \\u00b7 p99 '+lat(x.p99_us)+\n"
	"' \\u00b7 p99.9 '+lat(x.p999_us)+' \\u00b7 slowest call '+wspan(x)+'\">'+esc(x.name)+'</span>','p99 '+lat(x.p99_us)+' \\u00b7 max '+wmax(x)];});\n"
	"return card('commands \\u00b7 by calls',a.length?a:[['none yet','']])+card('commands \\u00b7 slowest calls',b.length?b:[['none yet','']],'clip')+\n"
