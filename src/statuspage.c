...
const char pc_status_page[] =
	 ...
	 * metric cards already use for "since start". */
	".hit .hv{display:flex;flex-direction:column;align-items:flex-end;line-height:1.2}\n"
	".hit .hl{font-size:11px;color:var(--ink3);font-family:var(--mono);\n"
	...
	/* S163: the FLEET's figures per collection, summed from the map each
	 * member gossips its block into (its own client hits and misses, so a
	 * pull served for a peer is not counted twice), joined to this node's
	 * rows by the name hash.  The cell shows the fleet total first and this
	 * node's share beside it - S93's frame rule applied to the rows. */
	"var FC=null,FCn=0,FCt=0,fk,fj;\n"
	"if(MEM&&MEM.length>1){FC={};for(fk=0;fk<MEM.length;fk++){var mc=MEM[fk].collections;FCt++;if(!mc)continue;FCn++;\n"
	"for(fj=0;fj<mc.length;fj++){var q=mc[fj],g=FC[q.hash]||(FC[q.hash]={e:0,h:0,m:0,s:0,x:0,n:0});\n"
	"g.e+=q.entries||0;g.h+=q.hits||0;g.m+=q.misses||0;g.s+=q.stores||0;g.x+=q.expired||0;g.n++;}}}\n"
	"function fv(tot,here){return '<span class=hv><span class=n>'+nf(tot)+'</span><span class=hl>'+nf(here)+' here</span></span>';}\n"
	...
	"(cl.length?(uni?\"all in \"+um+\" mode\":\"MIXED MODES\"):\"\")+(FC?' \\u00b7 fleet totals first, this node beside ('+FCn+' of '+FCt+' members reporting)':'');\n"
	...
	"ld=x.buckets?100*x.entries/x.buckets:0;\n"
	"if(pv){var dh=ch(x)-pv.h,dm=cm(x)-pv.m;\n"
	"if(dh>=0&&dm>=0){wn=dh+dm;if(wn>0)rt=100*dh/wn;}}\n"
	"NC[x.name]={h:ch(x),m:cm(x)};\n"
	"var k=rt===null?\"\":(rt<25?\"crit\":rt<60?\"low\":\"\"),FG=FC&&x.hash?FC[x.hash]:null;\n"
	"o+='<tr><td><span class=nm>'+esc(x.name)+'</span>'+\n"
	"(uni?'':'<span class=md>'+esc(x.mode||\"\")+'</span>')+\n"
	"'</td><td class=n'+(FG?' title=\"'+nf(FG.e)+' copies across '+FG.n+' members: under eager every node holds all, under spread K of P, about one otherwise\"':'')+'>'+(FG?fv(FG.e,x.entries):nf(x.entries))+'</td><td class=\"n dim\">'+nf(x.buckets)+\n"
	...
	"(FG?fv(FG.h,ch(x)):nf(ch(x)))+'</td><td class=n>'+(FG?fv(FG.m,cm(x)):nf(cm(x)))+'</td><td class=n>'+(FG?fv(FG.s,x.stores):nf(x.stores))+\n"
	"'</td><td class=\"n dim\">'+(FG?fv(FG.x,x.expired):nf(x.expired))+'</td><td class=n>'+\n"
	 ...
	 * calls and by time per call, and the slow log's tail; same poll */
	"function cmdcards(s){var cm=(s.commands||[]).slice(),sl=(s.slowlog||[]).slice(0,6),a,b;if(!cm.length&&!sl.length)return'';\n"
	"a=cm.slice().sort(function(x,y){return y.calls-x.calls;}).slice(0,8).map(function(x){return [esc(x.name),nf(x.calls)+' \\u00b7 '+(x.calls?(x.usec/x.calls).toFixed(1):'0')+' us'];});\n"
	"b=cm.filter(function(x){return x.calls>0;}).sort(function(x,y){return y.usec/y.calls-x.usec/x.calls;}).slice(0,8).map(function(x){return [esc(x.name),(x.usec/x.calls).toFixed(1)+' us \\u00b7 p99 '+nf(x.p99_us)+' us'];});\n"
	"return card('commands \\u00b7 by calls',a.length?a:[['none yet','']])+card('commands \\u00b7 slowest per call',b.length?b:[['none yet','']])+\n"
	"card('slow log \\u00b7 tail',sl.length?sl.map(function(e){return [esc((e.argv||[]).join(' ')).slice(0,48),nf(e.usec)+' us'];}):[['empty','']]);}\n"
	"function loadClients(){get('clients',function(d){if(d)CLD=d;drawClients();});}\n"
	"function drawClients(){var d=CLD||{clients:[],total:0,shown:0},f=(document.getElementById('clfilter').value||'').toLowerCase(),rows=d.clients||[],o='',i,x,n=0;\n"
	"for(i=0;i<rows.length;i++){x=rows[i];if(f&&String(x.addr).toLowerCase().indexOf(f)<0&&String(x.name||'').toLowerCase().indexOf(f)<0)continue;n++;\n"
	"o+='<tr><td class=n>'+nf(x.id)+'</td><td>'+esc(x.door)+'</td><td>'+esc(x.dialect)+'</td><td>'+(x.encrypted?'encrypted':'plain')+'</td><td class=mono>'+esc(x.addr)+'</td><td>'+esc(x.name||'')+\n"
	"'</td><td title=\"'+new Date(Date.now()-1000*(x.age_s||0)).toLocaleString()+'\">'+dur(x.age_s)+'</td><td class=n>'+dur(x.idle_s)+'</td><td class=n>'+nf(x.cmds)+'</td><td class=mono>'+esc(x.last_cmd||'')+\n"
	...
	"document.getElementById('clhint').textContent='showing '+n+' of '+nf(d.total)+(d.shown<d.total?' (the newest '+d.shown+' fetched)':'')+(f?' \\u00b7 filtered':'');}\n"
