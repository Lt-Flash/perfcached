...
const char pc_status_page[] =
	...
	"<h2>commands <span class=hint id=cmdtog role=button tabindex=0>the slowest verb - click for all</span></h2>\n"
	...
	"<th class=n title=\"the slowest single call this verb has taken - exact, not a bucket bound (S183).  The percentiles are read from a histogram of eight buckets per octave and interpolated inside the bucket, so they estimate the value rather than name a power of two above it.\">slowest call</th></tr></thead>\n"
	...
	"(st!==\"ready\"&&d.reason?'<div class=why>'+esc(d.reason)+'</div>':\"\")+\n"
	...
	"function wmax(x){if(x.max_us>0)return lat(x.max_us);var k=worst(x),n=(x.hist||[]).length;return k<0?'\\u2014':k==n-1?'> '+lat(Math.pow(2,k-1)):'\\u2264 '+lat(Math.pow(2,k));}\n"
	"function wspan(x){if(x.max_us>0)return 'exactly '+lat(x.max_us);var k=worst(x),n=(x.hist||[]).length;return k<0?'none':k==n-1?'over '+lat(Math.pow(2,k-1)):k?lat(Math.pow(2,k-1))+' to '+lat(Math.pow(2,k)):'up to 1 us';}\n"
	...
	"o+='<tr'+(i?' class=cx':'')+'><td>'+esc(x.name)+'</td><td class=n>'+nf(x.calls)+'</td><td class=n>'+lat1(x.usec/x.calls)+\n"
	...
	"if(t&&w)t.textContent=cm.length<2?'one verb called so far':w.classList.contains('open')?'showing all '+cm.length+' - click to fold':'the slowest of '+cm.length+' verbs - click for all';}\n"
	...
	"var n=document.querySelectorAll('#cmds tr').length;w.classList.toggle('open');\n"
	"t.textContent=n<2?'one verb called so far':w.classList.contains('open')?'showing all '+n+' - click to fold':'the slowest of '+n+' verbs - click for all';}\n"
