...
/* the tokenizer takes any bare word as a primitive (jsmn's rule); JSON
 * allows exactly four: true, false, null, and a number */
static int jp_prim_ok(const char *s, int n)
{
	int i = 0, digits = 0;
...
static int jp_valid_value(const char *val, size_t vlen)
{
	...
	if (ntok < 0)
		return 0;
	for (k = 0; k < ntok; k++)
		if (t[k].type == PC_J_PRIM &&
		        !jp_prim_ok(val + t[k].start, t[k].end - t[k].start)) {
			free(t);
			return 0;
		}
	e = ext_end(&t[0]);
...
int pc_jp_valid_value(const char *val, size_t vlen)
{
	return jp_valid_value(val, vlen);
}
