...
struct ps_wq {
	...
	int n;
	size_t bytes;
	uint64_t waiters[PS_OWNER_WORDS];  /* workers with paused publishers */
};
...
static unsigned long long ps_published, ps_slow_kills, ps_queued;
/* DELIVERED, per worker.  One shared counter bumped for every delivery by
 * every worker was 24% of the server's CPU delivering 12M messages a
 * second to 64 subscribers: the line it sat on bounced between the four
 * workers.  Each worker now adds a message's deliveries to a counter on
 * a cache line of its own, and /stats sums them. */
static struct ps_wcount {
	unsigned long long delivered;
} __attribute__((aligned(64))) ps_wcount[PS_MAX_WORKERS];
...
/* PRESSURE.  A drop at the cap came after PUBLISH had counted the
 * receivers, so a publisher that outran a worker was told its messages
 * were delivered while most were not (66% of 1 KB publishes lost on the
 * 223 rig).  A door's publish that leaves a queue at half the cap now
 * pauses its connection until the queue is below a quarter; the cap
 * itself stays for what cannot wait - relays from peers, keyspace events
 * - and for the one message per connection a pause lets through. */
static size_t ps_pause_at = (size_t)8 << 20, ps_resume_at = (size_t)4 << 20;
static unsigned long long ps_publish_paused;
__thread int pc_ps_pressed;
static __thread uint64_t ps_pressed_mask[PS_OWNER_WORDS];
_Static_assert(PC_PS_MASK_WORDS == PS_OWNER_WORDS, "pressure masks are owner masks");
...
void pc_pubsub_conn_closed(void *conn)
{
	...
	if (!pc)
		return;
	/* the link is read before sub_unlink frees the subscription; each
	 * one is the head of its connection's list by then, so the unlink's
	 * own walk still ends at its first step */
	for (s = pc->subs; s; s = next) {
		next = s->next_in_conn;
		sub_unlink(sh, s, worker);
	}
	conn_drop_if_empty(sh, pc);
 ...
 * Answers 1 for a delivery, which the caller counts on its worker's line */
static int account(void *conn, int rc)
{
	if (rc == 0)
		return 1;
	if (rc == PC_PS_PUSH_NOMEM)
...
void pc_pubsub_push_failed(void *conn, int rc)
{
	if (rc != 0)
		(void)account(conn, rc);
}
...
struct delivery {
	struct ps_msg *m;
	unsigned long long ok;             /* deliveries, for the worker's line */
};

static void deliver_pattern(struct psi_node *node, void *arg)
{
	struct delivery *dv = arg;
	struct ps_msg *m = dv->m;
	struct ps_local *le = (struct ps_local *)node;
	struct pc_ps_note note = { NULL, 0, 0 };   /* this message, this pattern */
	struct ps_sub *s;
	size_t flen;
	...
	for (s = le->subs; s; s = s->next_in_entry)
		dv->ok += (unsigned long long)account(s->owner->conn, f
			? pc_conn_pubsub_push(s->owner->conn, le->node.name,
			        le->node.nlen, m->chan, m->clen, m->data, m->dlen,
			        f, flen, &note)
			: PC_PS_PUSH_NOMEM);
	free(note.buf);
	free(f);
...
{
	struct ps_shard *sh = shard_of(worker);
	struct delivery dv = { m, 0 };
	struct pc_ps_note note = { NULL, 0, 0 };    /* this message, no pattern */
	struct psi_node *x;
	struct ps_sub *s;
	...
	if (x)
		for (s = ((struct ps_local *)x)->subs; s; s = s->next_in_entry)
			dv.ok += (unsigned long long)account(s->owner->conn,
				pc_conn_pubsub_push(s->owner->conn, NULL, 0,
				        m->chan, m->clen, m->data, m->dlen,
				        m->frame, m->flen, &note));
	free(note.buf);
	if (sh->pat.n)
		psi_match(&sh->pat, m->chan, m->clen, deliver_pattern, &dv);
	if (dv.ok)
		__atomic_add_fetch(&ps_wcount[worker].delivered, dv.ok,
			__ATOMIC_RELAXED);
}
...
/* A worker defers its wakes to the end of its turn: one eventfd write
 * per target worker however many messages the turn queued there, and the
 * target drains them as one batch - a wake per message had the target
 * draining, and flushing, one or two at a time.  Once a deferred wake's
 * queue holds a drain batch or PS_WAKE_EARLY bytes it is sent at once:
 * one long turn - a pipelined burst - must not fill the queue's cap
 * before the target starts draining, or messages a subscriber could
 * have taken are dropped (measured: 9,950 of 12,000 4 KB publishes, with
 * the waiting wake).  Any other thread (the relay receivers, the cluster
 * thread) wakes at once: nothing there would flush a deferred wake. */
#define PS_WAKE_EARLY (256u << 10)
static __thread int ps_wake_defer;
static __thread uint64_t ps_wake_pending[PS_OWNER_WORDS];

void pc_pubsub_defer_wakes(void)
{
	ps_wake_defer = 1;
}

void pc_pubsub_flush_wakes(void)
{
	uint64_t one = 1, v;
	int i, w;
	...
	for (i = 0; i < PS_OWNER_WORDS; i++) {
		...
		while (v) {
			...
			v &= v - 1;
			if (w < ps_nworkers && ps_wq[w].efd >= 0 &&
			    write(ps_wq[w].efd, &one, sizeof one) < 0)
				{ /* best effort: the next event drains it */ }
		}
	}
}

static void enqueue(int w, struct ps_msg *m, int door)
{
	struct ps_qnode *q = malloc(sizeof *q);
	struct ps_wq *wq = &ps_wq[w];
	uint64_t one = 1, bit = 1ull << (w % 64);
	size_t queued_bytes;
	int wake, queued_n, press;
	...
	wq->n++;
	queued_n = wq->n;
	queued_bytes = wq->bytes;
	press = door && ps_pause_at && queued_bytes >= ps_pause_at;
	pthread_mutex_unlock(&wq->lk);
	__atomic_fetch_add(&ps_queued, 1, __ATOMIC_RELAXED);
	if (press) {
		pc_ps_pressed = 1;
		ps_pressed_mask[w / 64] |= bit;
	}
	if (ps_wake_defer) {
		if (wake)
			ps_wake_pending[w / 64] |= bit;
		if (!(ps_wake_pending[w / 64] & bit) ||
		    (queued_n < PS_DRAIN_BATCH && queued_bytes < PS_WAKE_EARLY))
			return;
		ps_wake_pending[w / 64] &= ~bit;
		wake = 1;
	}
...
void pc_pubsub_drain(int worker)
{
	...
	uint64_t wake[PS_OWNER_WORDS], one = 1, v;
	int more = 0, i, waiters = 0;
	...
	/* below the resume mark: every worker with a publisher paused on
	 * this queue is told, before this batch is even delivered */
	for (i = 0; i < PS_OWNER_WORDS; i++) {
		wake[i] = wq->bytes < ps_resume_at ? wq->waiters[i] : 0;
		if (wake[i]) {
			wq->waiters[i] = 0;
			waiters = 1;
		}
	...
	pthread_mutex_unlock(&wq->lk);
	for (i = 0; waiters && i < PS_OWNER_WORDS; i++)
		for (v = wake[i]; v; v &= v - 1) {
			int w = i * 64 + __builtin_ctzll(v);

			if (w < ps_nworkers && ps_wq[w].efd >= 0 &&
			    write(ps_wq[w].efd, &one, sizeof one) < 0)
				{ /* the waiter's own timer asks again */ }
		}
	for (; q; q = next) {
		next = q->next;
...
{
	ps_queue_cap = bytes;
	ps_pause_at = bytes / 2;           /* 0: no cap, nothing presses */
	ps_resume_at = bytes / 4;
}

int pc_pubsub_pressure_take(uint64_t *mask)
{
	int i, was = pc_ps_pressed;

	for (i = 0; i < PS_OWNER_WORDS; i++) {
		if (mask)
			mask[i] = ps_pressed_mask[i];
		ps_pressed_mask[i] = 0;
	}
	pc_ps_pressed = 0;
	return was;
...
int pc_pubsub_pressure_wait(const uint64_t *mask)
{
	int me = pc_worker_id(), i, w, clear = 1;
	uint64_t v;

	for (i = 0; i < PS_OWNER_WORDS; i++)
		for (v = mask[i]; v; v &= v - 1) {
			struct ps_wq *wq;
			...
			if (w >= ps_nworkers)
				continue;
			wq = &ps_wq[w];
			/* registered under the lock the drain reads it under:
			 * a drain that passes the mark after this check wakes us */
			pthread_mutex_lock(&wq->lk);
			if (wq->bytes >= ps_resume_at) {
				clear = 0;
				if (me >= 0 && me < PS_OWNER_WORDS * 64)
					wq->waiters[me / 64] |= 1ull << (me % 64);
			}
			pthread_mutex_unlock(&wq->lk);
		}
	return clear;
...
void pc_pubsub_pause_note(void)
{
	__atomic_fetch_add(&ps_publish_paused, 1, __ATOMIC_RELAXED);
}
...
void pc_pubsub_stats(struct pc_pubsub_stats *out)
{
	...
	out->published = __atomic_load_n(&ps_published, __ATOMIC_RELAXED);
	{
		...
		for (w = 0; w < ps_nworkers; w++)
			d += __atomic_load_n(&ps_wcount[w].delivered, __ATOMIC_RELAXED);
		out->delivered = d;
	}
	...
	out->publish_paused = __atomic_load_n(&ps_publish_paused, __ATOMIC_RELAXED);
	{
