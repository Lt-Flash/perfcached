...
/* S127: `spread` holds each record on a deterministic SUBSET of the
 * fleet - K holders chosen by the same rendezvous placement shard
 * uses.  Bounds: 1 is REFUSED rather than aliased to shard (two names
 * for one behaviour is how config drift starts), and the ceiling is
 * the map's own member ceiling. */
#define PC_REPLICAS_MIN 2
#define PC_REPLICAS_MAX 257
...
/* S239: a collection's autoscaling mode ([daemon] / [collection x]
 * autoscale, and the autoscale verb) - see store.h */
#define PC_AUTOSCALE_AUTO 0
#define PC_AUTOSCALE_WARN 1
#define PC_AUTOSCALE_OFF  2
int pc_autoscale_parse(const char *s);      /* the mode, or -1 */
const char *pc_autoscale_name(int m);

struct pc_collection {
	char *name;
	int   buckets_log2;         /* the exponent convention: 16 = 65536 */
	unsigned int log_events;    /* S153: PC_EV_* mask, if has_log_events */
	int   has_log_events;
	unsigned int notify_events; /* PS8: PC_EV_* mask published as keyspace
	                             * notifications, if has_notify_events */
	int   has_notify_events;
	...
	int   autoscale;            /* S239: PC_AUTOSCALE_*, if has_autoscale */
	int   has_autoscale;
};
...
struct pc_config {
	...
	int autoscale;              /* S239: [daemon] autoscale, the default */
	int grow_at_pct;            /* S150: split once a table passes this
	                             * percent of its slot capacity; default 75 */
	int shrink_at_pct;          /* S150 D: resize down once a table sits
	                             * below this percent of its slot capacity
	                             * past the cool-off; 0 = grow_at_pct / 4 */
	int grow_floor_pct;         /* S175: how far a table may be widened
	                             * when its overflow leg says the load
	                             * factor is the reason lookups are slow;
	                             * 0 = never, default 50 */
	int leg_stuck_pct;          /* S175: the leg is "a tax" once a whole
	                             * drain pass leaves this percent of the
	                             * entries unable to return; default 1 */
	int shrink_cooloff_s;       /* S150 D: no shrink within this of a split,
	                             * a resize or the start; default 60 */
	int pubsub_queue_mb;       /* pub/sub: the most messages queued for one
	                            * worker, in MB; past it new ones are
	                            * dropped and counted.  0 = no cap */
	int max_clients;           /* S161: client connections across the
	                             * data doors; 0 = derived from the
	                             * descriptor limit minus the reserve */
	...
	/* S153: per-key event logging - the fleet default, a collection may
	 * override with its own log_events.  PC_EV_* bits (events.h). */
	unsigned int log_events;
	unsigned int notify_events; /* PS8: the [daemon] default mask, 0 = off */
	int log_events_rate;        /* lines per second per collection per kind */
	int log_events_hash;        /* log a hash of the key, not the key */
	/* [memory] */
	int arena_mb;
	...
	/* S167: pinning is attempted always and degrades to a warning; a node
	 * that MUST be resident says so and is refused a start it cannot
	 * honour.  1 = require. */
	int pin_require;
	/* S167: force the plain-4K tier - no MAP_HUGETLB, no MADV_HUGEPAGE,
	 * and THP disabled for this process (PR_SET_THP_DISABLE), so a 2 MB
	 * group costs 512 cheap faults instead of one that may compact. */
	int huge_pages_off;
	int shrink_step_mb;            /* S100: give-back per tick; 0 = sized to the arena */
	/* [secrets] */
	char *client_secret[PC_MAX_CLIENT_SECRETS];
	int   n_client_secrets;
	/* S129: the PRIVILEGE secret.  Distinct from `client` on purpose -
	 * apps and ops share the client value today, so gating DDL on it
	 * would hand create/drop/resize/rename to every application link.
	 * A rotation list like client's.  Fail-closed: none configured
	 * means `enable` always fails and DDL is unreachable from the
	 * client door however allow_create is set. */
	char *enable_secret[PC_MAX_CLIENT_SECRETS];
	int   n_enable_secrets;
	char *cluster_secret;
	/* [listen] */
	...
	int   pubsub_udp;           /* PS5: native doors may push over UDP */
	int   keepalive_s;          /* PS2: TCP keepalive on accepted data-door
	                             * sockets; 0 = off (redis: tcp-keepalive) */
	...
	int   resp_redirect;          /* RV-10: 0 forward (default), 1 moved */
	...
	char *state_dir;
	int   allow_create;         /* S69: a client may create and drop
	...
	char *cl_advertise;
	char *cl_site_salt;            /* [cluster] site_salt: mixed into a
	                                * platform-derived identity */            /* own unicast ip; NULL = auto-detect */
	int   cl_pull_timeout_ms, cl_negative_ms, cl_tombstone_ms;
	int   cl_apply_stall_ms;            /* S216; 0 = off */
	int   cl_apply_threads;             /* S217; 1 = on the receiver */
	...
	int   cl_max_pending;
	/* PS11: pub/sub relays arrive on a port of their own, read by this many
	 * threads, off the cluster socket that carries membership, pulls and
	 * replication.  0 threads = relays stay on the cluster socket.  Port 0
	 * = the cluster port + 1. */
	int   cl_ps_rx_threads;
	int   cl_ps_port;
	int   cl_ps_interested;      /* PS12: pubsub_relay = interested */
	...
	int   cl_replicas;             /* mode=spread: K holders per key.  0
	                                * when the mode is not spread, which
	                                * is what keeps it out of the digest
	                                * for every existing fleet. */
