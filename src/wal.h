...
/* S69: the collection set itself, so a replay lands records in the
 * collections that existed when they were written.  The record's KEY is
 * a one-byte placeholder (the format has no keyless shape) and its VALUE
 * carries buckets_log2 for a create, nothing for a drop. */
#define PC_WAL_T_COL_CREATE 6
#define PC_WAL_T_COL_DROP   7
#define PC_WAL_T_COL_RENAME 8   /* col = the old name, value = the new one */
/* the types whose header carries a version (32 bytes, not 24) */
#define PC_WAL_T_HASVER(t) ((t) == PC_WAL_T_UPSERT_V || \
	(t) == PC_WAL_T_TOUCH_V)
/* S240: a DELETE carries its version as its VALUE - 8 bytes, little-
 * endian - the version the remove stamped under the bucket lock, so
 * replay can order it against the upserts of the same key.  Not a new
 * type: a build that does not know it would read the header wrong, call
 * the log torn and stop replaying there - a ROLLBACK would lose every
 * record after the first delete.  An older build's delete replay ignores
 * the value, so a rollback replays these as the plain deletes they were.
 * A delete with no value (vlen 0) is a pre-S240 one: file order. */
#define PC_WAL_DEL_VERLEN 8

struct pc_wal_stats {
	int enabled;
	const char *fsync_mode;
	unsigned long long appended, bytes, dropped, late, recycles;
	/* S215: the device refused an append, a rotation or a sync (sticky;
	 * failed_errno says which way), and CONTROL writes/syncs that failed */
	int failed, failed_errno;
	unsigned long long ctrl_errors;
	unsigned long long last_seq, synced_seq;
	/* S145: the append point.  last_seq is where the ACK happens (a
	 * producer ring), pending_hi is what the pump has written to the file,
	 * synced_seq is what fdatasync has covered.  So last_seq - pending_hi
	 * is STAGED - lost on kill -9, DROPPED outright if a ring fills - and
	 * pending_hi - synced_seq is UNSYNCED - lost only to power loss, never
	 * dropped.  One number spanning both misled two rc diagnoses. */
	unsigned long long pending_hi;
	...
	int free_segments;
	/* S224: how fast the ring is filling (MB/s, an EWMA of the rotation
	 * interval, decaying while idle) and how long its free segments last
	 * at that rate (seconds, -1 = not measured yet).  full_in_s against
	 * rdb.last_dur_ms is what says whether a snapshot can keep up. */
	double fill_mb_s;
	double full_in_s;
	/* RV-13: how long the WHOLE ring takes to fill at that rate (-1 = not
	 * measured).  Below the last snapshot's duration, no trigger can keep
	 * a snapshot ahead of it: that is sizing. */
	double ring_fill_s;
	/* S229: a lost record puts the node in HEALING until a snapshot that
	 * started after the loss has completed; heals counts them, healing
	 * is 1 while one runs, last_heal_ms is the last one's length */
	unsigned long long heals;
	int healing;
	long long last_heal_ms;
};
 ...
 *
 * S215: the callback is told WHY - acknowledged writes that never
 * reached the log (a ring drop, a segment overrun), or a device that
 * refused a write or a sync - so the node can say which. */
#define PC_WAL_SHED_LOSS 1
#define PC_WAL_SHED_IO   2
void pc_wal_on_shed(void (*fn)(int why));
/* S229: told when a heal starts (1) and when it completes (0).  Without
 * one installed, or with no snapshot directory to heal from, a loss
...
/* S69: a collection created or dropped while the daemon serves */
...
int pc_wal_enabled(void);   /* S69: is a WAL running at all? */
 ...
 * so far is fdatasync'd.  0 synced, 1 WAL off, -1 timeout, -2 the
 * device refused a write or a sync and this process syncs no more. */
