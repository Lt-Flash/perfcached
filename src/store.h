...
/* S239: how a collection's table is sized, after Ceph's pg_autoscale_mode.
 * AUTO grows and shrinks it by its key count (the splitter and S150's
 * shrink - the default, so nothing changes on an upgrade); WARN changes
 * nothing but says what AUTO would do; OFF changes nothing and says
 * nothing.  Under WARN and OFF growth is blocked too: keys past the slots
 * go to the overflow leg, which the page shows red.  The modes and their
 * names live in config.h. */
extern int pc_autoscale_default;            /* [daemon] autoscale */
int pc_store_autoscale(int i);
void pc_store_set_autoscale_idx(int i, int m);
/* the verb: set it by name and persist it (it outranks the config's from
 * then on).  0 ok, -1 no such collection */
int pc_store_autoscale_set(const char *name, size_t nlen, int m);
/* what AUTO would resize it to, as log2 buckets - 0 for "leave it" */
...
/* S150 B: the retirement queue (maint, after the resize tick), its
 * figures, and the publish generation a resumable walk checks against */
...
/* the table and its publish generation as one consistent pair (dump) */
pcache_htable_t *pc_store_find_pub(const char *name, size_t nlen,
		unsigned long long *pub);
/* the slot publishing @ht and its generation, one pair; -1 if none does */
...
int pc_store_slot_of(pcache_htable_t *ht);      /* S153 */
