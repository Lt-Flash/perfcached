...
static int seg_rotate(void)
{
	...
	/* fsync what the old generation holds before moving on */
	pc_wal_fsync_start_us = wall_now_us();
	fdatasync(W.fd);
	...
	if (W.gen + 1 >= (uint32_t)W.nseg) {
		...
		if (have_rdb && seg_hot(idx, marker)) {
			...
			W.overruns++;
			LM_ERR("wal: FULL - no free segment: overwriting "
				"segment %d (seqs %llu..%llu, NEWER than the "
				"last completed snapshot marker %llu); those "
				"records are no longer replayable.  Increase "
		...
		int segments, int ring_kb, int nthreads)
{
	...
	(void)start_idx;
	if (seg_activate((int)((best_gen + 1) % (uint32_t)segments),
	        best_gen + 1) != 0) {
