		...
		pcache_iter_meta_cb cb, void *ctx)
{
	pcache_bucket_t *b;
	unsigned long lo, hi;
	unsigned int idx, i, klen, vlen, exp, wt, oidx;
	uint64_t tw;
	unsigned long long ver;
	unsigned char fl;
	char *kbuf, *vbuf;
	...
	for (idx = *cursor; idx < ht->nbuckets; idx++) {
		...
		tw = bkt_tag_word(b);
		if (!tw)
			continue;             /* empty: no slot read at all */
		for (i = 0; i < PCACHE_SLOTS; i++) {
			...
			/* S198: the callback may have REMOVED the record it was
			 * handed - a proxy migration stubs every victim from
			 * inside this walk - and pcache_ht_remove compacts the
			 * bucket, moving its LAST record into this slot.  That
			 * record has not been visited; leaving the slot skips
			 * it.  Half of every full bucket went that way (48 of 96
			 * in legwalkremovetest; proxytest left 164 of 400 small
			 * records on the donor for good).  A slot whose cell
			 * changed under the callback is looked at again; a slot
			 * that emptied, or kept its record, is left.  Bounded:
			 * every revisit follows a removal, and a bucket has six
			 * records to remove. */
			if (rc >= 0) {
				pcache_rec_t *now = __atomic_load_n(&b->slot[i],
					__ATOMIC_RELAXED);
			...
			if (rc < 0) {
				 ...
				 * callback stopped mid-bucket, so its
				 * remaining slots have not been emitted and
				 * skipping them loses records (measured: two
				 * keys of 20000 never replicated).  Re-emitting
				 * the earlier slots of this bucket on resume is
				 * harmless - every consumer of this walk is an
				 * idempotent upsert - and a single bucket can
				 * never exceed one budget, so progress is
				 * guaranteed. */
				*cursor = idx;
				goto out;
			}
...
leg:
	...
	rc = iter_overflow_from(ht, cb, ctx, kbuf, vbuf, &oidx, 0);
	if (rc < 0) {
		...
		void *ctx)
{
	pcache_bucket_t *b;
	unsigned long lo, hi;
	unsigned int idx, i, klen, vlen, exp, wt;
	uint64_t tw;
	unsigned long long ver;
	unsigned char fl;
	char *kbuf, *vbuf;
		...
		pcache_iter_cb cb, void *ctx)
{
	pcache_bucket_t *b;
	unsigned long lo, hi;
	unsigned int idx, end, i, klen, vlen, exp, wt, nb, oidx;
	uint64_t tw;
	unsigned long long ver;
	unsigned char fl;
	char *kbuf, *vbuf;
	...
	for (; idx < end; idx++) {
		...
		for (i = 0; i < PCACHE_SLOTS; i++) {
			if (!(tw & (0xFFULL << (i * 8))))
				continue;     /* free slot */
			/* records live at scattered arena addresses, so the
			 * copy-out below is a cold miss per key; start the
			 * NEXT occupied slot's record on its way while this
			 * one is being snapshotted */
			{
				unsigned int pf = i + 1;

				while (pf < PCACHE_SLOTS &&
				        !(tw & (0xFFULL << (pf * 8))))
					pf++;
			...
			if (!snapshot_slot(b, i, kbuf, vbuf, &klen, &vlen,
			        &exp, &wt, &fl, &ver, lo, hi))
				continue;
			/* NOVAL: hand emit_entry the scratch's value half
			 * (it NUL-terminates val in place, so a literal ""
			 * is a crash) with vlen 0 - the callback sees an
			 * empty value and the record's bytes stay cold */
			{
				struct rec_meta mt = { .exp = exp, .wt = wt,
					.fl = fl, .ver = ver };

				rc = emit_entry(iter_thunk_cb, &thunk, kbuf,
					klen, vbuf ? vbuf : kbuf + PCACHE_CELL_MAX,
					vbuf ? vlen : 0, &mt);
			}
			if (rc < 0)
				goto out;
