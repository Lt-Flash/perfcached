...
static int emit_entry(pcache_iter_meta_cb cb, void *ctx, char *kbuf,
		...
		const struct rec_meta *m)
{
	unsigned int exp = m->exp, wt = m->wt;
	unsigned char fl = m->fl;
	unsigned long long ver = m->ver;
	...
	return cb(&key, &val, exp, wt, fl, ver, ctx);
}
...
static int iter_thunk_cb(const str *key, const str *val, unsigned int exp,
...
again:
		...
		if (!snapshot_slot(b, i, kbuf, vbuf, &klen, &vlen, &exp, &wt,
			...
			continue;
		{
			struct rec_meta mt = { .exp = exp, .wt = wt, .fl = fl,
				.ver = ver };
	...
	for (idx = *oidx; idx < PCACHE_OVF_BUCKETS; idx++) {
		...
		for (;;) {
			...
			lock_release(&ht->ovf_lock);
			{
				...
				struct rec_meta mt = { .exp = exp, .wt = wt,
					.fl = fl, .ver = ver };
