...
static int emit_entry(pcache_iter_meta_cb cb, void *ctx, char *kbuf,
		...
		unsigned int exp, unsigned int wt, unsigned char fl,
		unsigned long long ver)
{
	str key, val;
	long long ll;
		...
		unsigned int budget)
{
	...
	for (idx = *oidx; idx < PCACHE_OVF_BUCKETS; idx++) {
		...
		for (;;) {
			...
			seen = n;
			lock_release(&ht->ovf_lock);
			rc = emit_entry(cb, ctx, kbuf, klen, vbuf, vlen, exp,
					ver,
				wt, fl);
			if (rc < 0)
				break;
		...
		pcache_iter_cb cb, void *ctx)
{
	...
	for (; idx < end; idx++) {
		...
		for (i = 0; i < PCACHE_SLOTS; i++) {
			...
			rc = emit_entry(iter_thunk_cb, &thunk, kbuf, klen,
				vbuf ? vbuf : kbuf + PCACHE_CELL_MAX,
				vbuf ? vlen : 0, exp, wt, fl, ver);
			if (rc < 0)
				goto out;
		}
	}
