 ...
 * 2^4 and 2^12 cost exactly the same - and about 200 bytes a bucket above
 * it.  README's sizing section says so; the number comes from here. */
unsigned long pcache_htable_index_bytes(unsigned int size_log2)
{
	...
	total += (unsigned long)n * (
		pcache_arena_region_cost((unsigned long)PCACHE_SEG_SIZE *
			sizeof(pcache_bucket_t)) +
		pcache_arena_region_cost((unsigned long)PCACHE_SEG_SIZE *
			sizeof(unsigned int)));
