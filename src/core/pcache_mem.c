...
int pcache_mem_no_huge;                /* S167: [memory] huge_pages = off */
...
void pcache_mem_probe(void)
{
	...
	if (pcache_mem_no_huge)
		return;                        /* S167: 4K by configuration */
	...
	p = pcache_mem_no_huge ? MAP_FAILED :
	    mmap(NULL, pc_hugetlb_sz, PROT_READ|PROT_WRITE,
...
#define MADV_POPULATE_WRITE 23        /* Linux 5.14; older headers lack it, older kernels answer EINVAL */
#endif

/* S97b: the populate call is not evidence that the pages are there.
 *
 * Under qemu-user - the build matrix's arm64 leg - madvise is emulated
 * and answers SUCCESS without backing a single page, so the first real
 * write to a MAP_NORESERVE hugetlb range takes SIGBUS.  rc27's arm64
 * build died exactly that way, at startup, one line after the daemon
 * announced "64 MB committed".  Before S97 the mapping carried no
 * MAP_NORESERVE and the kernel had charged the pool at mmap time, so a
 * memset was safe; with the lazy mapping the commit has to be CHECKED.
 *
 * mincore answers the question directly: is this page resident.  Every
 * huge page of the initial commit must be, and anything we cannot verify
 * counts as unbacked - a daemon on a slower page tier is better than one
 * that dies on its first write.  See the rule this belongs to:
 * assert the EFFECT, not the call. */
static int hugetlb_backed(char *p, size_t len)
{
	size_t off;

	for (off = 0; off < len; off += pc_hugetlb_sz) {
		unsigned char v = 0;

		if (mincore(p + off, 1, &v) != 0 || !(v & 1))
			return 0;
	}
	return 1;
}

/* S97: the hugetlb pool's free bytes now, 0 when unreadable */
size_t pcache_mem_hugetlb_free(void)
{
	...
	buf[r] = 0;
	n = atol(buf);
	return n > 0 ? (size_t)n * pc_hugetlb_sz : 0;
}

/* S97: populate @len bytes at @p - MADV_POPULATE_WRITE answers ENOMEM or
 * EFAULT where a blind touch would SIGBUS on an empty hugetlb pool; a
 * kernel without it (EINVAL) is touched, as before.  @pin adds mlock.
 * 0 ok, 1 = committed but the pin failed, -1 = the memory is not there. */
int pcache_mem_commit(void *p, size_t len, int pin)
{
	...
	if (pin && mlock(p, len) < 0)
		return 1;
	return 0;
}

void *pcache_mem_reserve(size_t size, size_t commit, enum pcache_mem_tier *tier,
		unsigned long *locked_mb)
{
	...
	*tier = PCACHE_MEM_4K;
	...
	if (!pcache_mem_no_huge &&                     /* S167 */
	    (hsize - size <= hsize / 2 || hsize - size <= 64UL << 20)) {
		/* S97: MAP_NORESERVE - the pool is charged for what is
		 * committed, group by group, never for the whole reservation
		 * (HG's tier 1 charges the cap at mmap time; that is the one
		 * thing not ported).  The initial commit is populated with the
		 * kernel's answer, not a touch: a short pool is ENOMEM here,
		 * not a SIGBUS later. */
		p = mmap(NULL, hsize, PROT_READ|PROT_WRITE,
		         MAP_PRIVATE|MAP_ANONYMOUS|MAP_HUGETLB|MAP_NORESERVE, -1, 0);
		if (p != MAP_FAILED && commit) {
			size_t c = (commit + pc_hugetlb_sz - 1) & ~(pc_hugetlb_sz - 1);
			int secured = 0;

			/* never a blind touch on hugetlb: a kernel with
			 * MADV_POPULATE_WRITE answers for the pool; without it
			 * the pool's own free count is consulted first - a race
			 * against another consumer rather than a SIGBUS */
			if (madvise(p, c, MADV_POPULATE_WRITE) == 0) {
				secured = 1;
			} else if (errno == EINVAL &&
			           pcache_mem_hugetlb_free() >= c) {
				memset(p, 0, c);
				secured = 1;
			}
			if (secured && !hugetlb_backed(p, c)) {
				LM_WARN("the huge-page pool reported the %zu MB "
					"initial commit as populated but the pages "
					"are not resident - the tier is refused "
					"rather than trusted, because the first "
					"write to an unbacked huge page is a "
					"SIGBUS, not an error return\n", c >> 20);
				secured = 0;
			}
			if (!secured) {
				munmap(p, hsize);
		...
		if (p != MAP_FAILED) {
			*tier = PCACHE_MEM_HUGETLB;
			 ...
			 */
			*locked_mb = ((commit + pc_hugetlb_sz - 1) & ~(pc_hugetlb_sz - 1)) >> 20;
			return p;
		}
	}
	...
	/* advise huge before first touch (tier 2), then pin+populate the
	 * INITIAL COMMIT: a cold mlock populates to pin, so it doubles as
	 * the pre-fault (DESIGN 2.6.2).  The rest of the reservation is
	 * untouched VA until a carve reaches it (S97). */
	if (!pcache_mem_no_huge)               /* S167 */
		madvise(base, asize, MADV_HUGEPAGE);
	commit = (commit + pc_thp_sz - 1) & ~(pc_thp_sz - 1);
	if (commit > asize)
		commit = asize;
	if (!commit) {
		*locked_mb = 0;
	} else if (mlock(base, commit) == 0) {
		*locked_mb = commit >> 20;
	} else {
		LM_WARN("mlock of the %zu MB arena failed (%s): continuing "
			...
			commit >> 20, strerror(errno));
		memset(base, 0, commit);       /* still pre-fault */
	}

	if (pcache_mem_no_huge) {
		*tier = PCACHE_MEM_4K;         /* S167: asked for, not fallen to */
	} else if (range_is_huge((unsigned long)base, pc_thp_sz)) {
