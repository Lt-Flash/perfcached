...
/* S128: what a region of `size` actually CHARGES the arena - the header,
 * the alignment slack and the whole-slot rounding the reservation carve
 * pays.  An index sizes itself with this, so a preflight and the carve
 * that follows it agree on the figure. */
