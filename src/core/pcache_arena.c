...
static pcache_chunk_t *slot_take(void)
{
	...
	/* NOLINTNEXTLINE(bugprone-assignment-in-if-condition) */
	           ...
	           (i = bit_first(arena->rcold, arena->rslots)) >= 0) { /* NOLINT(bugprone-assignment-in-if-condition) */
...
static void giveback_tick(void)
{
	...
	/* S47 shrink floor: never release below the floor.  Checked before
	 * every unit of give-back below (a group punch, a page free) via
	 * floor_allows(), so a workload that drains completely still leaves
	 * the operator's configured resident minimum in place.
	 *
	 * S47 also un-scoped the giveback_off latch from this whole
	 * function: it is set when MADV_DONTNEED is REFUSED on the
	 * RESERVATION, which on a pinned arena is permanent and expected
	 * (mlock and MADV_DONTNEED are mutually exclusive - EINVAL).  The
	 * page phase below returns malloc'd memory with free(), a
	 * completely different mechanism that keeps working; latching it
	 * off alongside the reservation meant a pinned node that had grown
	 * past its reservation could never return the overflow either. */

	/* the reservation: whole 2 MB groups of free resident slots, punched
	 * out with MADV_DONTNEED - the mapping stays, the pages go (hugetlb
	 * ones back to the pool).  Bookkeeping first: the punch zeroes the
	 * slot headers, so nothing may still be read from them afterwards. */
	if (arena->hbase && !arena->giveback_off) {
		for (g = 0; ((unsigned long)g + 1) * PCACHE_GROUP_SLOTS <= arena->hoff / PCACHE_SLOT; g++) {
			...
			if (madvise(p, PCACHE_HPS, MADV_DONTNEED) < 0) {
				...
				arena->nfree_warm += PCACHE_GROUP_SLOTS;
				arena->nfree_cold -= PCACHE_GROUP_SLOTS;
				break;   /* the pages below still come back */
			}
	...
	for (ppg = &arena->pages; (pg = *ppg) != NULL; ) {
		...
		if (!floor_allows((PCACHE_PAGE_SLOTS + 1) * PCACHE_SLOT)) {
