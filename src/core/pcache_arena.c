...
static void commit_ahead(void)
{
	...
	/* S168: the index zone grows DOWN from rtop, and its groups are
	 * committed by whoever splits a table - the maintenance thread,
	 * inside the slice the expiry sweep and this very tick wait behind.
	 * Same window, same tick, the other direction. */
	if (!arena->rtop)
		return;
	/* the group holding the next byte a region would take: rtop is the
	 * frontier and regions are carved BELOW it, so rtop itself may sit
	 * on a boundary and belong to the group above */
	off = (arena->rtop - 1) / PCACHE_HPS * PCACHE_HPS;
	for (n = 0; n <= PCACHE_AHEAD_GROUPS; n++) {
		if (off < arena->hoff + PCACHE_HPS)
			return;                /* the chunk frontier's side */
		g = (unsigned int)(off / PCACHE_HPS);
		if (!bit_get(arena->hcommit, g)) {
			if (group_commit(g) != 0)
				return;
...
void *pcache_region_alloc(size_t size)
{
	...
	if (pcache_arena_room_for(need) != 0)
		return NULL;
...
static void giveback_tick(void)
{
	...
	if (arena->hbase && !arena->giveback_off && !arena->tail_done) {
		 ...
		 * punching it here would hand back what the tick just committed
		 * and put the populate back on the write path */
		unsigned long keep = arena->hoff / PCACHE_HPS * PCACHE_HPS +
			(unsigned long)(PCACHE_AHEAD_GROUPS + 1) * PCACHE_HPS;

		if (lo < keep)
			lo = keep;
		/* S168: and the same window below the region frontier, which
		 * the index zone grows into */
		if (arena->rtop) {
			unsigned long rk = (arena->rtop - 1) / PCACHE_HPS * PCACHE_HPS;
			unsigned long win = (unsigned long)PCACHE_AHEAD_GROUPS * PCACHE_HPS;
