...
static pcache_chunk_t *slot_take(void)
{
	...
	if (!ch && arena->hbase && arena->hoff + PCACHE_SLOT <= arena->rtop &&
	    group_commit((unsigned int)(arena->hoff / PCACHE_HPS)) == 0) {   /* S97 */
 ...
 * that is the number a preflight must use.  The shm fallback charges the
 * unrounded `need` - less - so this over-states there rather than under,
 * which is the safe direction for a ceiling test. */
unsigned long pcache_arena_region_cost(unsigned long size)
{
	unsigned long need = size + sizeof(pcache_region_t) + 64;

	return ((need + PCACHE_SLOT - 1) >> PCACHE_SLOT_SHIFT) * PCACHE_SLOT;
}
...
int pcache_arena_room_for(unsigned long bytes)
{
	 ...
	 *
	 * KB below a megabyte: a bucket segment is 272 KB, and "an index of
...
static void giveback_tick(void)
{
	...
	if (arena->hbase && !arena->giveback_off) {
		...
		for (g = 0; g < ngroups && nruns < PUNCH_RUNS_MAX; g++) {
			...
			planned += PCACHE_HPS;
			if (nruns && runs[nruns - 1].g0 + runs[nruns - 1].n == g) {
...
void pcache_arena_pressure(struct pcache_arena_pressure *out)
{
	...
	out->warm_free_bytes = (unsigned long)arena->nfree_warm * PCACHE_SLOT;
