...
typedef struct pcache_chunk {
	...
	/* S190: two flags as BYTES, not words.  The debug build (RV-3) adds
	 * dbg_state to this header and it came to exactly 64 - adding a
	 * tenth word for `sparse` broke the static assert below, which is
	 * the header doing its job.  Neither is CAS'd: `cold` is written
	 * under the arena lock, `sparse` with a relaxed store. */
	unsigned char cold;          /* memory punched out; a carve re-faults it */
	unsigned char sparse;        /* a handful of survivors pin it - set by
	                              * the reclaim tick, read by the evacuator */
...
typedef struct pcache_arena {
	...
	/* live CELL bytes, own backing (bytes is chunk-granular and
	 * peak-sticky until reclaim - the rebalancer levels on THIS,
	 * perfcached addition).  S235: STRIPED.  One counter took an atomic
	 * add on every cell alloc and a subtract on every free, from every
	 * worker - one cache line bouncing between all of them, 21% of a
	 * saturated SET at 12 workers once the clock stopped holding the
	 * queue (DESIGN 12ga).  Each thread adds to its own padded stripe;
	 * readers (stats, /metrics, the ALIVE beat, the debug checker) sum
	 * them.  In the arena, not static: the arena is shared memory, and
	 * a forked host (cachedb_perf) counts across processes. */
	struct {
		long long v;
		/* 128 apart, not 64 with an alignment attribute: shm_malloc
		 * promises no 64-byte alignment, and at 128 no two counters
		 * can share a line wherever the arena lands */
		char pad[128 - sizeof(long long)];
	} live[PCACHE_LIVE_STRIPES];
	unsigned int live_next;                 /* stripe handout */
	...
	unsigned int          sparse_chunks;   /* S190: as of the last reclaim tick */
	unsigned long         evacuated;       /* S190: cells handed out for a move */
} pcache_arena_t;
...
/* S235: this thread's live-bytes stripe, handed out round-robin on first
 * use.  Threads past PCACHE_LIVE_STRIPES share one, which is still an
 * atomic add - only no longer every thread's */
static __thread int my_live_stripe = -1;

static inline void live_add(long long delta)
{
	int s = my_live_stripe;

	if (s < 0) {
		s = (int)(__atomic_fetch_add(&arena->live_next, 1,
			__ATOMIC_RELAXED) % PCACHE_LIVE_STRIPES);
		my_live_stripe = s;
	}
	__atomic_fetch_add(&arena->live[s].v, delta, __ATOMIC_RELAXED);
}

static unsigned long long live_sum(void)
{
	long long sum = 0;
	int s;

	for (s = 0; s < PCACHE_LIVE_STRIPES; s++)
		sum += __atomic_load_n(&arena->live[s].v, __ATOMIC_RELAXED);
	/* a free can be counted on one stripe before its alloc is on
	 * another is read: a transient negative total is 0, not 2^64 */
	return sum > 0 ? (unsigned long long)sum : 0;
}
...
void pcache_cell_free(void *cell)
{
	...
	live_add(-(long long)cell_sizes[c]);
...
/* ---- S190: evacuating the survivors of a mass expiry ------------------ */
int pcache_cell_evacuable(const void *cell)
{
	if (backing != PCACHE_BACKING_OWN)
		return 0;
	return __atomic_load_n(&cell_chunk(cell)->sparse, __ATOMIC_RELAXED);
}

/*
 * A cell for a record leaving a sparse chunk: from the DENSEST chunk of
 * the class that still has a free cell, never a sparse one.  The private
 * stacks are bypassed - they are fed from the class's current chunk,
 * which is often the emptiest, and a survivor moved into another sparse
 * chunk has not moved.  Falls back to the ordinary allocator (a carve
 * gives a fresh chunk, which is dense-to-be); the caller checks the
 * result with pcache_cell_evacuable() and gives it back if the fallback
 * handed it a sparse cell after all.
 */
void *pcache_cell_alloc_dense(unsigned int size)
{
	pcache_chunk_t *ch, *best = NULL;
	unsigned int best_free = ~0u;
	void *cell = NULL;
	int c;

	if (size > PCACHE_CELL_MAX || backing != PCACHE_BACKING_OWN)
		return pcache_cell_alloc(size);
	c = size2class[(size + 31) >> 5];
	lock_get(&arena->lock);
	/* pops happen only under this lock; pushes prepend, so the walk from
	 * a loaded head sees a consistent suffix */
	for (ch = __atomic_load_n(&arena->avail[c], __ATOMIC_ACQUIRE); ch;
	     ch = ch->link) {
		unsigned int nf = __atomic_load_n(&ch->nfree, __ATOMIC_ACQUIRE);

		if (nf && nf < best_free && !ch->sparse) {
			best = ch;
			best_free = nf;
		}
	}
	if (!best) {
		/* Every chunk of the class is sparse - the soak's own shape,
		 * where the survivors ARE the keyspace.  Consolidate: the
		 * fullest sparse chunk becomes the target, and stops being
		 * sparse so its own records stay put and the evacuator
		 * accepts cells from it.  When it fills, the next fullest. */
		for (ch = __atomic_load_n(&arena->avail[c], __ATOMIC_ACQUIRE); ch;
		     ch = ch->link) {
			unsigned int nf = __atomic_load_n(&ch->nfree, __ATOMIC_ACQUIRE);
		...
		if (best)
			__atomic_store_n(&best->sparse, (unsigned char)0,
				__ATOMIC_RELAXED);
	}
	if (best)
		cell = chunk_pop(best);        /* HOME -> PRIVATE in the ledger */
	lock_release(&arena->lock);
	if (!cell)
		return pcache_cell_alloc(size);
	dbg_move(cell, DBG_M(DBG_PRIVATE), DBG_OUT, "allocated dense");
	live_add(cell_sizes[c]);
	__atomic_fetch_add(&arena->evacuated, 1, __ATOMIC_RELAXED);
	return cell;
}

unsigned int pcache_arena_sparse_chunks(void)
{
	return arena ? __atomic_load_n(&arena->sparse_chunks, __ATOMIC_RELAXED) : 0;
}
...
void pcache_arena_reclaim_tick(void)
{
	...
	unsigned int kept, nfree, lingering = 0, sparse_now = 0;
	int c;
	...
	for (c = 0; c < PCACHE_NCLASSES; c++) {
		...
		for (ch = head; ch; ch = next) {
			...
			if (nfree && nfree < ch->cells &&
				...
				lingering++;
			/* S190: SPARSE - at most an eighth of it live, and quiet
			 * for the window: a mass expiry has left a handful of
			 * survivors pinning 256 KB each.  The evacuator on the
			 * maintenance thread moves them out; the chunk then
			 * drains fully and retires here on a later tick. */
			__atomic_store_n(&ch->sparse, (unsigned char)
				(nfree < ch->cells && nfree >= ch->cells - ch->cells / 8 &&
				 arena->tick - ch->home_since >= (unsigned int)pcache_reclaim_quiet_s),
				__ATOMIC_RELAXED);
			if (ch->sparse)
				sparse_now++;
	...
	arena->sparse_chunks = sparse_now;                  /* S190 */
...
void pcache_arena_pressure(struct pcache_arena_pressure *out)
{
	...
	out->sparse_chunks = arena->sparse_chunks;          /* S190 */
	out->evacuated = __atomic_load_n(&arena->evacuated, __ATOMIC_RELAXED);
