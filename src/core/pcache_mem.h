...
/* S167: set before pcache_mem_probe() to refuse every huge-page tier -
 * no MAP_HUGETLB, no MADV_HUGEPAGE.  The caller also disables THP for
 * the process (PR_SET_THP_DISABLE), or the kernel may still fold the
 * arena's pages behind our back on `enabled = always`. */
extern int pcache_mem_no_huge;
...
/* S97: @size is the VA reserved (MAP_NORESERVE on every tier - nothing is
 * charged for it); @commit, at most @size, is what is populated and pinned
 * now.  The rest commits by 2 MB group as the arena carves into it
 * (pcache_mem_commit). */
int pcache_mem_commit(void *p, size_t len, int pin);
size_t pcache_mem_hugetlb_free(void);   /* the pool's free bytes now */
