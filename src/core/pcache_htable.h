...
typedef struct pcache_htable {
	...
	unsigned int            evac_next;   /* S190: the evacuator's bucket cursor */
	unsigned long           evacuated;   /* S190: records moved out of sparse chunks */
...
/* S190: walk up to @buckets buckets from the evacuator's cursor and move
 * at most @budget records that sit in a SPARSE chunk (the reclaim tick's
 * verdict) into a dense one - a copy into a fresh cell, the slot swapped
 * under the bucket lock with the version bumped around it, the old cell
 * sent home.  Returns the records moved. */
...
/* S240: the version the last remove on THIS thread stamped (under the
 * bucket lock, found or not) - for the WAL's versioned delete record,
 * read immediately after the remove, as pcache_last_ver is */
extern __thread unsigned long long pcache_last_del_ver;
...
/* as pcache_ht_remove, plus *@ver = the delete's version, stamped under
 * the bucket lock whether or not the key was there (S234).  The version
 * a tombstone carries has to be taken where the delete happened: ticked
 * after the lock was dropped - as the tombstone used to be - it could
 * land above a set that reached the bucket in between, and every peer
 * then deleted a write this node kept.  Never below the removed record's
 * version. */
...
/* RV-8: what a metadata walk says about a record - a struct, because
 * the callback used to take these four as positional arguments, and
 * every pair of them converts into the next silently (u64 -> u32,
 * u32 -> u8, u8 -> u64).  S173's leg walk passed them in the record's
 * order rather than the callback's, compiled without a murmur, and
 * mis-reported every record it emitted for a year of releases.  A field
 * cannot be handed over in the wrong position, and a callback still on
 * the old shape does not compile. */
struct pcache_rec_meta {
	unsigned int expires;          /* absolute ticks, 0 = never */
	unsigned int wtick;            /* the tick of the last write */
	unsigned char rflags;          /* PCACHE_F_* */
	unsigned long long ver;        /* the record's version (A1) */
};
typedef int (*pcache_iter_meta_cb)(const str *key, const str *val,
		const struct pcache_rec_meta *meta, void *ctx);
