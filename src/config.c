...
static int handle_kv(struct pstate *ps, char *key, char *val)
{
	...
	case S_COLLECTION:
		...
		if (!strcasecmp(key, "mode")) {
			 ...
			 * spread would have no copy factor at all */
			if (m == PC_MODE_SPREAD) {
				PERR(ps, "collection mode = spread is not a "
					"per-collection setting: the copy factor "
					"is cluster state.  Declare [cluster] "
					"mode = spread with replicas = N\n");
				return -1;
			}
	...
	for (i = 0; i < cfg->n_col; i++) {
		...
		if (cfg->col[i].eager &&
		        (cfg->col[i].mode != PC_MODE_STORE ||
		         !cfg->cl_enabled)) {
			PERR(ps, "collection '%s': mode = eager needs a "
				"[cluster] section\n", cfg->col[i].name);
			return -1;
		}
