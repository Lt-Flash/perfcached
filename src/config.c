...
/* one "a.b.c.d/len" (or a bare address = /32) into the allow-list */
static int add_resp_allow(struct pstate *ps, const char *v)
{
	...
	if (cfg->n_resp_allow >= PC_MAX_RESP_ALLOW) {
		PERR(ps, "more than %d resp_allow entries\n", PC_MAX_RESP_ALLOW);
		return -1;
	}
	if (strlen(v) >= sizeof buf) {
		PERR(ps, "resp_allow entry too long\n");
		return -1;
	}
	strcpy(buf, v);
	slash = strchr(buf, '/');
	...
	if (!inet_aton(buf, &ia)) {
		PERR(ps, "resp_allow: '%s' is not an IPv4 address or CIDR\n", buf);
		return -1;
	}
	cfg->resp_allow[cfg->n_resp_allow].mask =
		len ? htonl(0xFFFFFFFFu << (32 - len)) : 0;
	cfg->resp_allow[cfg->n_resp_allow].net =
		ia.s_addr & cfg->resp_allow[cfg->n_resp_allow].mask;
	cfg->n_resp_allow++;
	return 0;
}
