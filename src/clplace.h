...
/* Is @node_id one of the top @k placeable nodes for @kh?  This is the
 * predicate the copy-factor write path asks once per record per peer,
 * so it counts how many nodes BEAT @node_id and stops caring at @k -
 * no ranked array is materialised and nothing is sorted.  It returns
 * exactly what pc_clplace_rank() would put in its first @k entries;
 * the test asserts that agreement rather than trusting this comment.
 *
 * False for a node that is not in the map or not placeable.  @k <= 0 is
 * false; @k at or above the placeable count puts every placeable node
 * in the set.
 *
 * RANK 0 IS THE PRIMARY, AND ONLY FOR REPAIR OWNERSHIP (S127).  Any
 * member of the set may accept a write: eager has no primary at all
 * today, so primary-only admission would be a NEW restriction rather
 * than a simplification, and the store already refuses an older version
 * under the bucket lock.
 */
int pc_clplace_in_set(const struct pc_clmap *m, uint64_t kh,
		uint16_t node_id, int k);

/* Does @m mention @node_id at all, placeable or not?
 *
 * THE REASON THIS EXISTS.  pc_clplace_in_set() answers 0 for two
 * completely different situations: "this node is not in the top K" and
 * "this map has never heard of this node".  A caller that only ADDS
 * copies may treat them alike - both mean "do not send here".  A caller
 * that DELETES may not: the first says the record is not ours, the
 * second says the map cannot tell us anything about ourselves, and a
 * node that has just rejoined can hold an id an older published map
 * predates.  Acting on the second cost a returning node 39 of its 58
 * records on 2026-09-13.
 *
 * So ask this first if a 0 from in_set() is going to remove data. */
int pc_clplace_has(const struct pc_clmap *m, uint16_t node_id);
