...
/* 4 is FAILED.  A peer built before it exists stores the byte and
 * compares it against READY, so it reads an unknown state as "not
 * usable" - the safe direction, and the reason this could be added
 * without a format bump. */
#define PC_CLMAP_ST_FAILED     4
/* 5 is STALLED (S216), added the same way and safe for the same reason. */
#define PC_CLMAP_ST_STALLED    5
/* 6 is HEALING (S229): the same again - an older peer reads it as not
 * usable, which is what it is for writes while it lasts. */
#define PC_CLMAP_ST_HEALING    6
