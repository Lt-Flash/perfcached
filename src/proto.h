...
/* PS4: pub/sub on the binary dialect.  PUBLISH: [clen2][chan][data...];
 * the four subscription verbs: [clen2][name], clen 0 on an unsubscribe
 * meaning "all of this kind".  Replies are small JSON objects like every
 * non-value reply; deliveries are NOTIFY frames carrying the same
 * id-less JSON the text dialect gets. */
#define PC_VERB_PUBLISH      10
#define PC_VERB_SUBSCRIBE    11
#define PC_VERB_UNSUBSCRIBE  12
#define PC_VERB_PSUBSCRIBE   13
#define PC_VERB_PUNSUBSCRIBE 14
 ...
 *   PC_LK_NATIVE  the JSON-RPC / binary dialects (sniffed per S7)
...
/* S107: the same payload to every native-dialect client on a worker's
 * list (RESP and HTTP have no notification frame); on that worker */
