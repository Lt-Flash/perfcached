...
static int term_store(uint32_t t)
{
	...
	if (!T.dir[0])
		return 0;
	snprintf(tmp, sizeof tmp, "%s/" TERM_FILE ".tmp.%d", T.dir, (int)getpid());
	snprintf(fin, sizeof fin, "%s/" TERM_FILE, T.dir);
...
int pc_term_init(const char *state_dir)
{
	...
	snprintf(path, sizeof path, "%s/" TERM_FILE, state_dir);
