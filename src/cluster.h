...
struct pc_cl_stats {
	...
	unsigned long long neg_displaced, tomb_displaced;       /* RV-15 */
	...
	unsigned long long map_unadmitted;  /* S241: maps/syncs from refused senders */
...
void pc_tombstone_send(const char *col, size_t collen, const char *key,
		size_t klen);
/* S234: the same, carrying the version the delete was stamped with under
 * its bucket lock (pcache_ht_remove_ver) - a tombstone that removed a
 * record here must not tick a fresh version after the lock is gone, or a
 * set landing in between sits below it on every peer and above it here.
 * pc_tombstone_send, which ticks, is left for a delete that removed
 ...
 * waits up to @timeout_ms for them to leave, fills @gone, @still and
 * @lost (node ids, at most @cap each), and schedules THIS node's own
 * clean stop to follow - it goes last, after it has reported.  Worker
 * thread.  Returns the number of peers it was sent to, -1 if not
 * clustered.  S233: @gone is a peer that said GOODBYE - a clean stop
 * ends in one; @lost left the live set WITHOUT one (it crashed, was cut
 * off, or stopped following us), so nothing says it stopped. */
