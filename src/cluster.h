 ...
 * block and every live peer's gossiped one (S163).  A HELD record counts
 * once per member that holds it, so entries and expired follow the mode's
 * copy rule (@basis); client events - hits, misses, stores, removes -
 * happen once, on the node the client reached, and sum.  @store is the
