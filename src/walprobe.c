...
{
	char path[512], dev[64];
	FILE *f;
	int n;

	memset(pr, 0, sizeof *pr);
	if (pc_wal_dev_id(dir, dev, sizeof dev) != 0)
		return -1;
	snprintf(path, sizeof path, "%s/.pc-walprobe", dir);
	f = fopen(path, "r");
	if (!f)
		return -1;
	n = fscanf(f, "v1 %63s %lf %lld %lld %lld %d", pr->dev_id,
		&pr->seq_mb_s, &pr->fsync_p50_us, &pr->fsync_p99_us,
		&pr->sync_iops, &pr->probed_secs);
	fclose(f);
	if (n != 6)
		return -1;
	if (strcmp(pr->dev_id, dev) != 0)
		return -1;                     /* the device changed: re-probe */
	pr->valid = 1;
	pr->cached = 1;
	return 0;
}
...
int pc_wal_probe_cache_store(const char *dir, const struct pc_wal_probe *pr)
{
	...
	snprintf(tmp, sizeof tmp, "%s/.pc-walprobe.new", dir);
	snprintf(path, sizeof path, "%s/.pc-walprobe", dir);
	f = fopen(tmp, "w");
	if (!f)
		return -1;
	fprintf(f, "v1 %s %.1f %lld %lld %lld %d\n", pr->dev_id,
		pr->seq_mb_s, pr->fsync_p50_us, pr->fsync_p99_us,
		pr->sync_iops, pr->probed_secs);
	fclose(f);
		...
		const struct pc_wal_policy *pol, char *buf, size_t cap)
{
	...
	if (pol && n < cap)
		n += (size_t)snprintf(buf + n, cap - n,
			"wal: policy: fsync %s recommended, sustainable ~%lld "
			"synced writes/s, segments %d MB (%s)\n",
			pol->fsync_recommend, pol->max_durable_wps,
			pol->segment_mb, pol->note ? pol->note : "");
