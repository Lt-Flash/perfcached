...
/* The group header.  @count is 0 when records are still being gathered -
 * clmig_record_done patches it as they land - and the final count when
 * the group is sealed in one go, which the write-path push does.  Both
 * shapes exist; only one place spells the layout. */
size_t clmig_group_hdr(unsigned char *buf, size_t cap, unsigned char type,
		uint32_t req, int node, unsigned int count);
