		...
		const struct clmemb_alive *in)
{
	...
	pc_w32(&c, in->cl_open);                /* S160 */
	pc_w32(&c, in->cl_resp);
	pc_w32(&c, in->cl_bin);
	pc_w32(&c, in->cl_json);
	pc_w32(&c, in->cl_nresp);
	{
		int i, n = in->ncols < 0 ? 0 : in->ncols > CLMEMB_COLS_MAX
			? CLMEMB_COLS_MAX : in->ncols;      /* S163 */

		pc_w8(&c, (unsigned)n);
		pc_w8(&c, (unsigned)(in->ncols_total > 255 ? 255 : in->ncols_total));
		for (i = 0; i < n; i++) {
			pc_w64(&c, in->cols[i].hash);
			pc_w64(&c, in->cols[i].entries);
			pc_w64(&c, in->cols[i].hits);
			pc_w64(&c, in->cols[i].misses);
			pc_w64(&c, in->cols[i].stores);
			pc_w64(&c, in->cols[i].removes);
			pc_w64(&c, in->cols[i].expired);
		}
	}
	pc_w16(&c, (uint16_t)in->ps_port);           /* PS11: after the block */
	pc_w64(&c, in->ps_interest);
	pc_w8(&c, (unsigned)in->nreason);       /* S186 */
	pc_w8(&c, (unsigned)in->epoch);         /* S221, last: additive */
...
int clmemb_alive_patch_state(unsigned char *buf, size_t len, int nstate,
		int nreason)
{
	/* the shortest frame the writer makes: everything up to the
	 * collection block's two counts, no collections, then ps_port(2),
	 * ps_interest(8) and the reason byte */
	if (len < 92 + 11)                      /* = CLMEMB_ALIVE_LEN with none */
		return -1;
	buf[CLMEMB_ALIVE_NSTATE_OFF] = (unsigned char)nstate;
	/* the reason is the SECOND-to-last byte since S221 put the epoch
	 * behind it - clmembtest holds this equal to a fresh encode, which
	 * is what caught the off-by-one when the epoch was appended */
	buf[len - 2] = (unsigned char)nreason;
	return 0;
}
		...
		struct clmemb_alive *out)
{
	...
	if (!pc_rfits(&c, 5 * 4))
		return 1;
	out->cl_open = pc_ru32(&c);            /* S160 */
	out->cl_resp = pc_ru32(&c);
	out->cl_bin = pc_ru32(&c);
	out->cl_json = pc_ru32(&c);
	out->cl_nresp = pc_ru32(&c);
	out->have |= CLMEMB_A_CLIENTS;
	{
		int i, n;                              /* S163: all or nothing */

		if (!pc_rfits(&c, 2))
			return 1;
		n = pc_ru8(&c);
		out->ncols_total = pc_ru8(&c);
		if (n > CLMEMB_COLS_MAX || !pc_rfits(&c, (size_t)n * 56))
			return 1;
		for (i = 0; i < n; i++) {
			out->cols[i].hash = pc_ru64(&c);
			out->cols[i].entries = pc_ru64(&c);
			out->cols[i].hits = pc_ru64(&c);
			out->cols[i].misses = pc_ru64(&c);
			out->cols[i].stores = pc_ru64(&c);
			out->cols[i].removes = pc_ru64(&c);
			out->cols[i].expired = pc_ru64(&c);
		}
		out->ncols = n;
		out->have |= CLMEMB_A_COLS;
	}
	if (!pc_rfits(&c, 2 + 8))
		return 1;
	out->ps_port = pc_ru16(&c);                   /* PS11 */
	out->ps_interest = pc_ru64(&c);
	out->have |= CLMEMB_A_PSRELAY;

	if (!pc_rfits(&c, 1))
		return 1;
	out->nreason = pc_ru8(&c);
	out->have |= CLMEMB_A_REASON;

	if (!pc_rfits(&c, 1))
		return 1;
	out->epoch = pc_ru8(&c);                /* S221 */
	out->have |= CLMEMB_A_EPOCH;
	return 1;
}
