...
struct pending {
	...
	unsigned long long first_ver;  /* and the VERSION it answered with.
	                                * Two holders is the NORMAL state in
	                                * eager mode - every key is on every
	                                * node - so "two positives" cannot by
	                                * itself mean a conflict.  The same
	                                * record at the same version is the
	                                * same record; only a DIFFERENT
	                                * version is a real birth race. */
	...
	unsigned char probe_op;        /* 0 none, 1 set, 2 add, 3 touch, 4 del */
...
struct clpend {
	...
	int peak;
	/* S212: occupancy BY KIND (PC_DONE_*, 0..15), the same accounting -
	 * "reconcile probes issued and not yet answered" is this for kind
	 * PC_DONE_RECONCILE, whatever the answer turns out to be */
	int used_kind[16];
...
int clpend_used_kind(const struct clpend *p, int kind);   /* S212 */
...
enum clpend_ans {
	...
	CLPEND_ANS_RACE,        /* a second positive at a DIFFERENT version:
	                         * the holders genuinely disagree */
	CLPEND_ANS_SAME,        /* a second positive at the SAME version:
	                         * the same record, which is the normal
	                         * state in eager mode - nothing to do */
	...
	CLPEND_ANS_POSSESS,     /* S146: a designated holder CONFIRMED it has
	                         * the record, so our surplus copy may go.
	                         * The mirror of RECONCILE - that one deletes
	                         * on a unanimous no, this one only on a
	                         * yes. */
};
...
/* @ver is the version the peer answered with, and it is what separates
 * "two nodes hold the same record" from "two nodes disagree about it".
 * See first_ver above; 0 means the caller has no version to offer, and
 * is treated as "cannot prove they are the same". */
