...
/* S159/S183: the command rows merged by name across the workers, with
 * the latency histogram behind each.  RESP commands sit under their
 * bare Redis name, the native doors as json.<verb> and bin.<verb> - the
 * two dialects cost differently and a merged row would hide it.
 *
 * S183: EIGHT buckets per octave, and the percentile is INTERPOLATED
 * inside the bucket it lands in.
 *
 * It was one bucket per power of two and a percentile answered with
 * that bucket's upper bound, so every figure the page showed was a
 * power of two - 512 us, 128 us, 16 us.  The operator put it exactly
 * right: "if the value is 262 us then obviously it would be less than
 * 512 but more than 256" - a bound that wide tells a reader nothing
 * they did not already know, and only the mean looked alive.
 *
 * Eight sub-buckets make a bucket 12.5% of its value wide, and the
 * percentile then reads the position of the wanted rank INSIDE that
 * bucket and interpolates, which is what histogram_quantile does over
 * Prometheus buckets.  The answer is an estimate of the VALUE rather
 * than a ceiling above it, and it is no longer a power of two unless
 * the data is.  Below PC_OBS_SUB microseconds each value keeps its own
 * slot - that is where the fast paths live, and it makes p50 exact for
 * a GET that takes 2 us.
 *
 * Cost: 129 x 8 bytes per command per worker, 66 KB a worker for the
 * full 64 command slots.
 *
 * The exact MAX is kept beside the histogram: "the slowest call" is a
 * real number, not the bound of whichever bucket it fell in. */
#define PC_OBS_NAME 24
/*
 * S194: the LAST FIVE MINUTES, beside the totals since start.
 *
 * Every figure on the commands card was cumulative, and the operator
 * put the problem precisely: the dashboard is read as "what is
 * happening now", while Prometheus is the thing that wants counters.
 * A cumulative mean drifts toward the past, a cumulative p99 is diluted
 * by every quiet second, and a cumulative MAX is stickiest of all - one
 * 8.3 ms call during a bulk load owns that column for the life of the
 * process.
 *
 * Five one-minute slots, rolled by the 1 Hz tick: the maintenance
 * thread snapshots the cumulative counters into the slot it is leaving,
 * so a window is the difference between now and the oldest snapshot -
 * four to five minutes, with NO extra work on the write path.  The max
 * is the exception, since a maximum cannot be differenced: each slot
 * keeps its own, which costs the hot path one compare, and the window's
 * max is the largest of the five.
 *
 * /metrics is untouched - it keeps the cumulative form Prometheus needs.
 */
#define PC_OBS_WIN  5
#define PC_OBS_SUB  8                   /* buckets per octave */
#define PC_OBS_HIST 129                 /* octaves 1..16 x SUB, + overflow */
struct pc_obs_cmdsum {
	char name[PC_OBS_NAME];
	unsigned int nlen;
	unsigned long long calls, usec, max;
	unsigned long long hist[PC_OBS_HIST];
	/* S194: the same three over the last PC_OBS_WIN minutes */
	unsigned long long w_calls, w_usec, w_max;
	unsigned long long w_hist[PC_OBS_HIST];
	unsigned int w_secs;               /* how long that window is */
};

/* S183: the inclusive upper bound of bucket @k, in microseconds - the
 * one place that knows the layout, so /metrics and the percentiles
