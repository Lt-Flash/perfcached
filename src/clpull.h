...
/* M9b: this node's PULL_REQ as a whole frame - the request filled from
 * the arguments and written under M_PULL_REQ (clmsg.h).  @msg must hold
 * clpull_req_size(collen, klen).  What cluster.c's build_pull_req() did,
 * with the node id passed in instead of read from `C`. */
size_t clpull_req_frame(unsigned char *msg, uint32_t req, int node,
