...
/* ---- the wire envelope --------------------------------------------
 *
 * M_CLSYNC carries two length-prefixed blobs:
 *
 *     [type1][maplen4][map bytes][histlen4][hist bytes]
 *
 * and the payloads are encoded IN PLACE by pc_clmap_encode and
 * pc_clhist_encode, which write into the buffer and return their
 * length.  So the framing is reserve-then-patch rather than a straight
 * append, and that is why this is offsets-and-lengths rather than a
 * cursor walk: the caller fills the middle.
 *
 * The parse side is the half that matters.  Its three bounds checks -
 * the header, the map's declared length against the datagram, and the
 * history's length landing EXACTLY on the end - lived inline in
 * cluster.c where nothing could test them.  A control-state sync is
 * what a promoted standby rebuilds the fleet from, so a frame that
 * parses loosely is worse here than almost anywhere.
 */
#define CLSYNC_ENV_HDR      5      /* type + maplen */
#define CLSYNC_ENV_MAP_AT   CLSYNC_ENV_HDR
#define CLSYNC_ACK_LEN      9      /* type + term4 + seq4 */

/* where the history payload goes, once the map's length is known */
size_t clsync_env_hist_at(uint32_t maplen);

/* Write the type and both length prefixes around payloads already
 * encoded in place.  Returns the total frame length, or 0 if the
...
/* Split a received envelope.  Returns 0 - and touches nothing - unless
 * the frame is whole: header present, the map wholly inside it, and the
 * history ending EXACTLY at the end.  A trailing byte is a refusal, not
 * something to ignore. */
