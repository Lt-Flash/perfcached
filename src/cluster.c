		...
		unsigned int *cursor, int incl_passive)
{
	...
	if (C.bulkb_len) {
		if (clbulk_offer(&C.bk, C.bulkb, C.bulkb_len, C.bulk_recs,
			 ...
			 * is a cycle that must not advance the mark.  The
			 * next sweep offers the same records again. */
	...
	for (c = 0; c < pc_store_count(); c++) {
		...
		for (i = 0; i < C.pt.n_peers; i++) {
			...
			if (pr->repl_cursor[c] == 0) {
				...
				/* S174: oversized copies leave on the bulk
				 * plane, whose outcome lands after this
				 * slice returns - so the cycle remembers
				 * where the failure count stood when it
				 * began, and refuses to advance the mark if
				 * it moved.  Global rather than per peer: a
				 * failure to one peer costs another an
				 * extra walk, which is the safe direction. */
				pr->repl_bulk0[c] = (unsigned int)
					__atomic_load_n(&C.px.repl_bulk_lost,
						__ATOMIC_RELAXED);
			}
...
out:
	...
	if (!ok || stored < recs) {
		...
		if (kind == CLBULK_KIND_REPLICA) {
			 ...
			 * the mark must not advance past them */
			__atomic_fetch_add(&C.px.repl_bulk_lost, 1,
				__ATOMIC_RELAXED);
