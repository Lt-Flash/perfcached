...
#define M_JOIN_WAIT     23   /* ucast: [tok8] - the master is HOLDING this
...
#define BOOT_WAIT_MS    3000   /* how long a fresh joiner waits for a
                                * ready peer's heartbeat before giving
                                * up on the pull */
#define BOOT_PICK_MS    1200   /* one beat and a margin after the join
                                * before the candidates are picked, so
                                * every live peer's heartbeat is in and
                                * the lowest holder is the choice */
 ...
 * Bounded (RESERVE_MAX, oldest evicted) and expiring (RESERVE_MS), so ids
 * return to the pool. */
#define RESERVE_MAX 64
#define RESERVE_MS  3600000LL
struct pc_reservation {
	struct in_addr addr;
...
static struct pc_reservation reserved[RESERVE_MAX];
...
static struct {
	...
	struct {
		uint32_t req;
		uint16_t recs;
		size_t off;                /* S85: its datagram, still in migb */
		long long sent_ms;         /* S85: last (re)send */
		unsigned char retx;        /* S85: re-sends so far */
	} mig_out[MIG_WINDOW * 2];
	...
	int boot_pending;
	int boot_ncand;
	struct sockaddr_in boot_cand[BOOT_MAX_CAND];
	int boot_cand_node[BOOT_MAX_CAND];
	long long boot_deadline_ms;    /* give up waiting for a ready peer */
	long long boot_pick_ms;        /* no pick before this */
	int repl_open;                 /* S105: groups open across all peers */
...
static void nonce_next(unsigned char out[12])
{
...
static int peers_live(void);           /* the counter, defined below */

static inline int peer_live(const struct peer *p, long long now)
{
...
static void broadcast_live(const unsigned char *msg, size_t n, long long now)
{
	...
	for (i = 0; i < C.n_peers; i++)
		if (peer_live(&C.peers[i], now))
			seal_send(&C.peers[i].addr, msg, n);
}

/* the live peer holding this node id, or NULL */
static struct peer *peer_by_id_live(int node, long long now)
{
	int i;
...
static unsigned char beat_alive[70];
...
static uint64_t config_digest(void)
{
	...
	DIG(C.cfg_mode);
	DIG(C.cfg_eager);
	/* S73b: a fleet is UNIFORM - every node logs a WAL or none does.  A
	 * node that disagrees is refused at the join like a mode mismatch,
	 * which settles every question the WAL-posture section raised at
	 * once: recovery stays before the join (a refused node never
	 * replicates a stale replay outward), a node without [wal] in a
	 * logging fleet is refused with a message that says so, and nothing
	 * is adopted from a master, so no term or epoch is needed.  Folded
	 * only when ON: a fleet without a WAL keeps the digest it has today,
	 * so a rolling upgrade across this change does not split it (a
	 * logging fleet upgrades with a coordinated restart - none exists
	 * yet).  fsync policy stays per node: strength, not posture. */
	if (C.cfg_wal) {
		DIG('W');
		DIG(1);
	}
...
int pc_shard_owner_slot(unsigned slot)
{
	...
	best = pc_node_state() == PC_NST_READY
		? pc_hrw_mix(kh, C.self_addr.sin_addr.s_addr,
...
static void reserve_note(const struct peer *p);   /* S90: defined with id_taken() */
...
static struct peer *peer_by_node(int node)
{
	return peer_by_id_live(node, now_ms());
}
...
struct wgroup {
	unsigned char *q;                  /* MIG_GHDR + group + one record */
	size_t qlen;
	unsigned int qcount;
	long long qopen_ms;
...
static struct wgroup *wgroups[PC_TIDX_MAX];   /* [thread slot] -> per peer */

static void wgroup_send(struct wgroup *g)
{
	if (!g->qcount)
		return;
	g->q[0] = M_REPL_MANY;
	...
	__atomic_fetch_sub(&C.repl_open, 1, __ATOMIC_RELAXED);
	g->qlen = MIG_GHDR;
	g->qcount = 0;
	g->qopen_ms = 0;
}
...
{
	int me = pc_worker_id(), i;
	struct wgroup *gs;

	if (me < 0 || me >= PC_TIDX_MAX || !(gs = wgroups[me]))
		return;
	for (i = 0; i < PC_CL_MAXPEER; i++)
		if (gs[i].qcount && now - gs[i].qopen_ms >= REPL_FLUSH_MS)
			wgroup_send(&gs[i]);
}
...
static void repl_flush_due(long long now)
{
	...
	if (!__atomic_load_n(&C.repl_open, __ATOMIC_RELAXED))
		return;
		...
		int drop)
{
	...
	if (!C.enabled || nlen == 0 || nlen >= PC_COL_NAME_MAX)
		return;
	msg[0] = M_COL_SET;
	msg[1] = (unsigned char)(drop ? 1 : 0);
	msg[2] = (unsigned char)buckets_log2;
	memcpy(msg + 3, &gen, 8);
	msg[11] = (unsigned char)nlen;
	memcpy(msg + 12, name, nlen);
	broadcast_live(msg, 12 + nlen, now);
}
		...
		size_t tlen)
{
	...
	broadcast_live(msg, 13 + flen + tlen, now);
}
		...
		unsigned long long ver)
{
	...
	p32(rec, ttl_left);
	rec[4] = (unsigned char)collen;
	p16(rec + 5, (uint16_t)klen);
	p32(rec + 7, (uint32_t)vlen);
	p64(rec + 11, ver);
	memcpy(rec + MIG_RHDR, col, collen);
	memcpy(rec + MIG_RHDR + collen, key, klen);
	memcpy(rec + MIG_RHDR + collen + klen, val, vlen);
	n = MIG_RHDR + collen + klen + vlen;
	nowms = now_ms();
	{
		int me = pc_worker_id();
		struct wgroup *gs;

		if (me < 0 || me >= PC_TIDX_MAX)
			return;                    /* not a slot thread: never pushes */
		if (!(gs = wgroups[me])) {
			gs = calloc(PC_CL_MAXPEER, sizeof *gs);
			if (!gs)
				return;
			wgroups[me] = gs;          /* this thread's alone, forever */
		}
		for (i = 0; i < C.n_peers; i++) {
			struct peer *p = &C.peers[i];
			struct wgroup *g = &gs[i];

			if (!peer_live(p, nowms))
				continue;
			if (!g->q) {
				g->q = malloc(MIG_GHDR + REPL_GROUP_FLUSH_BYTES + sizeof rec);
				if (!g->q)
					continue;
				g->qlen = MIG_GHDR;
				g->qcount = 0;
			}
			if (g->qcount && !addr_eq(&g->to, &p->addr)) {
				/* the slot changed hands: what was gathered for the
				 * old peer is dropped - it is gone, and the sweep owes
				 * the newcomer everything anyway */
				g->qlen = MIG_GHDR;
				g->qcount = 0;
				__atomic_fetch_sub(&C.repl_open, 1, __ATOMIC_RELAXED);
			}
			if (g->qcount && (g->qlen + n > MIG_GATHER_CAP ||
			        nowms - g->qopen_ms >= REPL_FLUSH_MS))
				wgroup_send(g);
			if (!g->qcount) {
				g->qopen_ms = nowms;
				g->to = p->addr;
				__atomic_fetch_add(&C.repl_open, 1, __ATOMIC_RELAXED);
			}
			memcpy(g->q + g->qlen, rec, n);
			g->qlen += n;
			g->qcount++;
			if (g->qlen >= REPL_GROUP_FLUSH_BYTES || n > MIG_GATHER_CAP)
				wgroup_send(g);
		}
	}
}
...
static void clients_membership(const char *event, struct peer *p, int node)
{
	...
	if (n > 0 && (size_t)n < sizeof buf) {
		...
		LM_INFO("cluster: telling this node's clients that node %d %s "
			"(%s:%d, %d member(s) now)\n", node, event,
			inet_ntoa(p->addr.sin_addr), p->client_port, peers_live() + 1);
	}
}
...
static int hex2bin(const char *s, unsigned char *out, size_t n)
{
	size_t i;
		...
		unsigned char *id, char *why, size_t wlen)
{
	...
	if (len < hl + 1 || memcmp(s, IDENT_HEADER, hl) != 0 ||
	        s[hl] != '\n') {
		snprintf(why, wlen, "the first line is not \"" IDENT_HEADER
			"\"");
		return -1;
	}
	s += hl + 1;
...
static void send_join_req(const struct sockaddr_in *to)
{
	...
	msg[41] = (unsigned char)(C.cfg_wal ? 2 : 1);
	seal_send(to ? to : &C.mcast_dst, msg, 42);
...
static int self_start_kind(void)
{
	...
	if (__atomic_load_n(&C.boot_done, __ATOMIC_RELAXED) &&
...
static void send_alive(void)
{
	...
	/* S81: how this node started, same additive tail - PC_START_* */
	msg[63] = (unsigned char)self_start_kind();
	/* S78: the HTTP door and uptime, same additive tail.  A peer built
	 * before them reads its 64-byte prefix and shows them as unknown. */
	p16(msg + 64, (uint16_t)C.cfg_http_port);
	p32(msg + 66, (uint32_t)pc_metrics_uptime());
	seal_send(&C.mcast_dst, msg, 70);
	C.st.hb_sent++;
	/* publish for the watchdog; a non-master's stale MASTER_ALIVE
	 * must not outlive the role, so clear it here */
	beat_store(beat_alive, &beat_alive_len, msg, 70);
...
static int pick_backup(void)
{
	...
	for (i = 0; i < (unsigned int)C.n_peers; i++) {
		...
		if (!id || id == C.node_id || !peer_live(p, now))
			continue;
...
static void send_master_alive(void)
{
	 ...
	 * the config digest. */
	p32(msg + 35, pc_term_current());
	seal_send(&C.mcast_dst, msg, 39);
	beat_store(beat_malive, &beat_malive_len, msg, 39);
}
...
/* S90: remember a departing member's binding for RESERVE_MS */
static void reserve_note(const struct peer *p)
{
	long long now = now_ms(), oldest = 0;
	int i, slot = -1;

	if (!p->node)
		return;
	for (i = 0; i < RESERVE_MAX; i++) {
		if (reserved[i].id &&
		    reserved[i].addr.s_addr == p->addr.sin_addr.s_addr) {
			slot = i;              /* the same address: refresh */
			break;
		}
		if (!reserved[i].id || reserved[i].until_ms <= now) {
			if (slot < 0)
				slot = i;
		} else if (slot < 0 && (!oldest || reserved[i].until_ms < oldest))
			oldest = reserved[i].until_ms;
	}
	if (slot < 0)                      /* full: evict the oldest */
		for (i = 0; i < RESERVE_MAX; i++)
			if (reserved[i].until_ms == oldest) {
				slot = i;
				break;
			}
	if (slot < 0)
		slot = 0;
	reserved[slot].addr = p->addr.sin_addr;
	reserved[slot].has_ident = p->has_ident;
	memcpy(reserved[slot].ident, p->ident, 16);
	reserved[slot].id = p->node;
	reserved[slot].until_ms = now + RESERVE_MS;
	LM_NOTICE("cluster: node %d (%s) left - its id is reserved for it "
		"for %lld s\n", p->node, inet_ntoa(p->addr.sin_addr),
		RESERVE_MS / 1000);
}

/* S90: does a live reservation hold @id (for someone else)? */
static int reserve_holds(int id)
{
	long long now = now_ms();
	int i;

	for (i = 0; i < RESERVE_MAX; i++)
		if (reserved[i].id == id && reserved[i].until_ms > now)
			return 1;
	return 0;
}

/* S90: a joiner claims its reservation - identity first, then address.
 * The entry is consumed; *how names which matched. */
static int reserve_take(const unsigned char *ident, const struct in_addr *addr,
		const char **how)
{
	long long now = now_ms();
	int i, id = 0;

	for (i = 0; i < RESERVE_MAX && !id; i++)
		if (reserved[i].id && reserved[i].until_ms > now &&
		    reserved[i].has_ident && ident &&
		    memcmp(reserved[i].ident, ident, 16) == 0) {
			id = reserved[i].id;
			*how = "identity";
			reserved[i].id = 0;
		}
	for (i = 0; i < RESERVE_MAX && !id; i++)
		if (reserved[i].id && reserved[i].until_ms > now &&
		    reserved[i].addr.s_addr == addr->s_addr) {
			id = reserved[i].id;
			*how = "address";
			reserved[i].id = 0;
		}
...
static int id_taken(int id)
{
	...
	if (id == C.node_id)
		return 1;
	if (reserve_holds(id))             /* S90: reserved for a departed member */
		return 1;
	for (i = 0; i < C.n_peers; i++)
...
static void become_master(int founded)
{
	...
	if (founded && any_eager() && !self_entries() &&
	        ...
	        !__atomic_load_n(&C.boot_done, __ATOMIC_RELAXED) &&
	        peers_live() > 0) {
		...
		C.boot_deadline_ms = now_ms() + BOOT_WAIT_MS;
		C.boot_pick_ms = now_ms() + BOOT_PICK_MS;
...
static int dup_seen_recent(const struct in_addr *addr)
{
	long long now = now_ms();
	int i;

	for (i = 0; i < PC_CL_MAXPEER; i++)
		if (C.dup_seen[i].addr.s_addr == addr->s_addr &&
		        now - C.dup_seen[i].first_ms < DUP_GRACE_MS)
			return 1;
	return 0;
}
...
static int reserve_home(const unsigned char *ident, struct in_addr *home)
{
	long long now = now_ms();
	int i;
	...
	for (i = 0; i < RESERVE_MAX; i++)
		if (reserved[i].id && reserved[i].until_ms > now &&
		    reserved[i].has_ident &&
		    memcmp(reserved[i].ident, ident, 16) == 0) {
			*home = reserved[i].addr;
			return 1;
		}
	return 0;
}
		...
		const struct sockaddr_in *from)
{
	...
	if (C.role == PC_ROLE_MASTER) {
		...
		if (!id) {
			 ...
			 * reserved id. */
			if (n >= 35 && reserve_home(pt + 19, &home) &&
			        home.s_addr != from->sin_addr.s_addr &&
			        dup_seen_recent(&home)) {
				char hx[33], hs[24];
			...
			rid = reserve_take(n >= 35 ? pt + 19 : NULL,
				&from->sin_addr, &how);
		...
		const struct sockaddr_in *from)
{
	...
	if (n >= 25) {
		...
		if (theirs != config_digest() && C.cfg_authoritative) {
			...
			shout_mismatch(from, pt[15], pt[16], theirs, 0);
			C.cfg_refused++;
			peer_forget(from);
			return;
		}
	...
	if (p) {
		...
		p->http_port = n >= 70 ? g16(pt + 64) : 0;   /* S78 */
		p->boot_ms = n >= 70
			? p->last_seen_ms - (long long)g32(pt + 66) * 1000 : 0;
		...
		const struct sockaddr_in *from)
{
	...
	if (n >= 35) {
		...
		if (theirs != config_digest() && C.cfg_authoritative) {
			...
			shout_mismatch(from, pt[25], pt[26], theirs, 0);
			C.cfg_refused++;
			peer_forget(from);
			return;
		}
	...
	if (p)
		peer_publish(p, their_id, g32(pt + 13),
			n >= 25 ? g32(pt + 17) : 0,
			n >= 25 ? g32(pt + 21) : 0, pd_noident, 0, 0, PC_START_UNKNOWN);
	if (C.role == PC_ROLE_JOINING)
...
static int boot_request(void)
{
	...
	for (i = 0; i < n; i++) {
		C.boot_cand[i] = addr[i];
		C.boot_cand_node[i] = id[i];
	}
	C.boot_ncand = n;
	C.boot_pending = 1;
 ...
 * alike - a founder with live peers holding records pulls too */
static void boot_tick(long long now)
{
	...
	if (pc_node_state() == PC_NST_RECOVERING &&
		...
		pthread_mutex_lock(&C.bmx);
		pending = C.boot_pending;
		pthread_mutex_unlock(&C.bmx);
		int failed_round;

		pthread_mutex_lock(&C.bmx);
		failed_round = C.boot_round_failed;
		C.boot_round_failed = 0;
		pthread_mutex_unlock(&C.bmx);
		if (failed_round && ++C.boot_rounds >= 3) {
			C.boot_deadline_ms = 0;
			LM_WARN("cluster: no peer served a bootstrap in %d "
				...
				"sweeps)\n", C.boot_rounds);
			pc_node_state_set(PC_NST_READY);
		} else if (!pending && now >= C.boot_pick_ms) {
			int br = boot_request();

			if (failed_round && !br) {
				/* nobody to re-pick yet: wait a beat more */
				C.boot_deadline_ms = now + BOOT_WAIT_MS;
				br = 1;
			}
			if (br == 2) {
				/* a fleet that is itself empty - its birth,
				 * or a keyspace that expired - has nothing
				 * to hand over; the few records a beat may
				 * hide arrive by the push.  Waiting would
				 * only hold writers off an empty node. */
				C.boot_deadline_ms = 0;
				__atomic_store_n(&C.boot_done, 1, __ATOMIC_RELAXED);
				LM_NOTICE("cluster: every ready peer reports "
					"holding nothing - nothing to pull; ready\n");
				pc_node_state_set(PC_NST_READY);
			} else if (br) {
				C.boot_deadline_ms = 0;
			} else if (now >= C.boot_deadline_ms) {
				C.boot_deadline_ms = 0;
...
static void col_sync_peer(struct peer *p)
{
	...
	for (i = 0; i < pc_store_count(); i++) {
		...
		if (nl == 0 || nl >= PC_COL_NAME_MAX)
			continue;
		msg[0] = M_COL_SET;
		msg[1] = (unsigned char)(live ? 0 : 1);
		msg[2] = (unsigned char)bl;
		memcpy(msg + 3, &gen, 8);
		msg[11] = (unsigned char)nl;
		memcpy(msg + 12, name, nl);
		seal_send(&p->addr, msg, 12 + nl);
	}
}
...
static void membership_tick(void)
{
	 ...
	 * peer per interval, and idempotent by generation at the receiver. */
	{
		...
		for (i = 0; i < C.n_peers; i++)
			if (peer_live(&C.peers[i], now) &&
			        (resync || !C.peers[i].col_synced)) {
				C.peers[i].col_synced = 1;
				col_sync_peer(&C.peers[i]);
			}
	}
...
static void handle_col_set(const unsigned char *pt, size_t n)
{
	...
	memcpy(&gen, pt + 3, 8);
	nl = pt[11];
	if (nl == 0 || nl >= PC_COL_NAME_MAX || n < 12 + nl)
		return;
	memcpy(name, pt + 12, nl);
	name[nl] = 0;
	pc_lamport_observe(gen);
	if (pt[1] == 2) {                  /* S69: a rename, two names */
		...
		if (n < 13 + nl)
			return;
		tl = pt[12 + nl];
		if (tl == 0 || tl >= PC_COL_NAME_MAX || n < 13 + nl + tl)
			return;
		memcpy(to, pt + 13 + nl, tl);
		to[tl] = 0;
	...
	} else if (pt[1] == 3) {           /* S69: resize to bl */
		if (pc_store_find(name, nl))
		...
		case M_MIGRATE_ACK: {
			...
			for (j = 0; j < sizeof C.mig_out / sizeof C.mig_out[0];
				...
				if (C.mig_out[j].req == mreq) {
					C.mig_out[j].req = 0;
					C.mig_out[j].recs = 0;
					/* S85: only a matching ack frees a
					 * window slot; the ack of a re-sent
					 * batch whose first ack arrived after
					 * all is a duplicate and frees nothing */
					if (C.mig_inflight > 0)
						C.mig_inflight--;
					break;
				}
...
void pc_cluster_thread(volatile int *stop)
{
	...
	while (!*stop) {
		...
		if (poll(pf, 2, __atomic_load_n(&C.repl_open, __ATOMIC_RELAXED)
		        ? REPL_FLUSH_MS : 100) <= 0) {
...
static void mig_retransmit(const struct sockaddr_in *to)
{
	...
	for (j = 0; j < sizeof C.mig_out / sizeof C.mig_out[0]; j++) {
		if (!C.mig_out[j].req || C.mig_out[j].retx >= MIG_RETX ||
		    now - C.mig_out[j].sent_ms < MIG_RETX_MS)
			continue;
		seal_send(to, C.migb + C.mig_out[j].off + 2,
			g16(C.migb + C.mig_out[j].off));
		C.mig_out[j].sent_ms = now;
		C.mig_out[j].retx++;
		C.px.migrate_retx++;
	}
}
...
void pc_rebalance_tick(void)
{
	...
	for (i = 0; i < C.n_peers; i++) {
		...
		if (!peer_live(&C.peers[i], now_ms()) || !C.peers[i].total_mb)
			continue;
		u = live_per_mille(C.peers[i].live_kb, C.peers[i].total_mb);
		sum += u;
		n++;
...
{
	if (bc->len && bulk_send_stream(bc->fd, bc->cs, bc->buf, bc->len) != 0) {
		bc->err = 1;
		return -1;
	}
	bc->len = 0;
		...
		void *arg)
{
	...
	rp = bc->buf + bc->len;
	p32(rp, ttl_left);
	rp[4] = (unsigned char)bc->cn;
	p16(rp + 5, (uint16_t)key->len);
	p32(rp + 7, (uint32_t)val->len);
	p64(rp + 11, ver);
	memcpy(rp + MIG_RHDR, bc->col, bc->cn);
	memcpy(rp + MIG_RHDR + bc->cn, key->s, (size_t)key->len);
	memcpy(rp + MIG_RHDR + bc->cn + key->len, val->s, (size_t)val->len);
	bc->len += n;
	bc->streamed++;
	return 0;
}
		...
		size_t scap)
{
	...
	if (bulk_recv_exact(cfd, cs_r, stash, slen, scap, req, 4) != 0)
		return;
	...
	if (bulk_send_stream(cfd, cs_s, end, sizeof end) != 0)
		goto out;
	__atomic_fetch_add(&C.px.boot_out, bc.streamed, __ATOMIC_RELAXED);
	if (bulk_recv_exact(cfd, cs_r, stash, slen, scap, cnt4, 4) == 0)
		...
		unsigned int *streamed, unsigned int *stored, unsigned int *older)
{
	...
	*streamed = *stored = *older = 0;
	...
	if (bulk_io(fd, 1, hdr, 3 + mlen) != 0 || bulk_io(fd, 0, hdr, 2) != 0)
		goto out;
	cl = (size_t)hdr[0] | ((size_t)hdr[1] << 8);
	if (cl == 0 || cl > sizeof m2 || bulk_io(fd, 0, m2, cl) != 0 ||
		...
		goto out;
	p32(cnt4, BOOT_MAGIC);
	req[0] = BOOT_KIND_PULL;
	p16(req + 1, (uint16_t)C.node_id);
	req[3] = 0;
	if (bulk_send_stream(fd, &cs_s, cnt4, 4) != 0 ||
	        bulk_send_stream(fd, &cs_s, req, 4) != 0)
		goto out;
	...
	for (;;) {
		...
		if (bulk_recv_exact(fd, &cs_r, stash, &slen, sizeof stash,
		        rhdr, MIG_RHDR) != 0)
	...
	p32(cnt4, *stored);
	bulk_send_stream(fd, &cs_s, cnt4, 4);   /* so the peer can say what landed */
	rc = 0;
out:
	free(rec);
	if (fd >= 0)
...
static void bulk_pull(void)
{
	...
	pthread_mutex_lock(&C.bmx);
	n = C.boot_ncand;
	memcpy(cand, C.boot_cand, sizeof cand);
	memcpy(node, C.boot_cand_node, sizeof node);
	pthread_mutex_unlock(&C.bmx);
	for (k = 0; k < n; k++) {
...
void pc_cluster_get_stats(struct pc_cl_stats *out)
{
	...
	*out = C.st;
	...
	out->peers_up = 0;
	out->reserved_ids = reserve_count();   /* S90 */
	out->peers_up = peers_live();
}
