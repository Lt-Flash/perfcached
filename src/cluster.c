		...
		const struct sockaddr_in *from, int passive)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		/* S127: DO NOT add a "refuse a replica we are not a holder
		 * for" test here.  It was tried on 2026-09-13 and reverted:
		 * a receiver whose membership view has not converged refuses
		 * copies it SHOULD hold, and the fleet measured 212 where
		 * placement owed 240 - under-replicated, which is worse than
		 * the surplus it was meant to prevent.  Surplus is a
		 * RECLAMATION problem and belongs in a settled-only local
		 * pass, not in the receive path where no node can know
		 * whether its view or the sender's is the stale one. */
		if (ht) {
			k.s = (char *)r.key;
			k.len = (int)kn;
			v.s = (char *)r.val;
			...
			/* S209: a tombstone for this key that is NEWER than the
			 * copy means the copy is the set the delete superseded,
			 * arriving after it.  Refuse it; count it apart from
			 * "older", which is the ordinary duplicate. */
			{
				unsigned long long tv;

				if (pc_neg_ver(colz, cn, k.s, kn, &tv) && tv &&
				        ver <= tv) {
					C.px.recv_tombstoned++;
					continue;
				}
			}
			...
			if (rc == PCACHE_E_OLDER) {
				...
				/* PS8: an eager replica applied here notifies here;
				 * a migrated record (a placement move) does not */
				if (passive)
					pc_ev_applied(ht, &k);
				if (!passive) {
