...
static unsigned long long pubsub_seq;
		...
		size_t dlen)
{
	...
	seq = __atomic_add_fetch(&pubsub_seq, 1, __ATOMIC_RELAXED);
	frame[0] = M_PUBLISH;
	...
	for (i = 0; i < C.pt.n_peers; i++) {
		...
		if (!__atomic_load_n(&p->node, __ATOMIC_ACQUIRE) ||
		    !clpeers_live(p, now))
			continue;
		if (seal_send(&p->addr, frame, n) == 0)
			sent++;
	}
	return sent;
}

/* the receiver: gap accounting by sender id, then a relayed publish */
static struct { int node; unsigned long long seq; } ps_seen[PC_CL_MAXMEMBERS];

static void handle_publish(const unsigned char *pt, size_t n)
{
	unsigned node, clen;
	unsigned long dlen;
	unsigned long long seq, lost = 0;
	int i, free_i = -1;
	...
	for (i = 0; i < PC_CL_MAXMEMBERS; i++) {
		if (ps_seen[i].node == (int)node) {
			if (seq > ps_seen[i].seq + 1)
				lost = seq - ps_seen[i].seq - 1;
			if (seq > ps_seen[i].seq)
				ps_seen[i].seq = seq;
			break;
		}
		if (!ps_seen[i].node && free_i < 0)
			free_i = i;
	}
	if (i == PC_CL_MAXMEMBERS && free_i >= 0) {
		ps_seen[free_i].node = (int)node;  /* first sighting: no gap yet */
		ps_seen[free_i].seq = seq;
	}
	pc_pubsub_relay_account(0, lost, 0);
