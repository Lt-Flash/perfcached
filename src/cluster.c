...
static void spread_reclaim_tick(void)
{
	...
	for (c = 0; c < pc_store_count(); c++) {
		...
		for (i = 0; i < r.n; i++) {
			...
			kk.s = r.hit[i].key;
			kk.len = r.hit[i].klen;
			/* a plain remove: a TOMBSTONE here would delete the
			 * record from the holders, which is the opposite of
			 * what reclaiming a surplus copy means */
			if (pcache_ht_remove(ht, &kk) == 1) {
				pc_wal_del(nm, kk.s, kk.len);
