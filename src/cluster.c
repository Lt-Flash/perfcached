 ...
 * fresh always qualifies.  What the holdoff does not cover is a
 * keyspace whose fill takes longer than it, with restarts spaced
 * between the two; that needs the joining node to pull its own
 * bootstrap and know when it is done, which is filed, not built. */
