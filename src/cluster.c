...
void pc_repl_push(const char *col, size_t collen, const char *key,
		...
		unsigned long long ver)
{
	unsigned char msg[MIG_GHDR + MIG_RHDR + 255 + 4096 + MAX_FWD_VAL + 8];
	unsigned char *rp = msg + MIG_GHDR;
	unsigned int now = get_ticks(), ttl_left = 0, pushed = 0;
	long long nowms;
	size_t n;
	int i;
	...
	msg[0] = M_REPL_MANY;
	p32(msg + 1, 0);                   /* req 0: no ack, see above */
	p16(msg + 5, (uint16_t)C.node_id);
	p16(msg + 7, 1);
	p32(rp, ttl_left);
	rp[4] = (unsigned char)collen;
	p16(rp + 5, (uint16_t)klen);
	p32(rp + 7, (uint32_t)vlen);
	p64(rp + 11, ver);
	memcpy(rp + MIG_RHDR, col, collen);
	memcpy(rp + MIG_RHDR + collen, key, klen);
	memcpy(rp + MIG_RHDR + collen + klen, val, vlen);
	n = MIG_GHDR + MIG_RHDR + collen + klen + vlen;
	nowms = now_ms();
	for (i = 0; i < C.n_peers; i++) {
		if (!__atomic_load_n(&C.peers[i].node, __ATOMIC_ACQUIRE) ||
		        nowms - C.peers[i].last_seen_ms >= PEER_UP_MS)
			continue;
		seal_send(&C.peers[i].addr, msg, n);
		pushed++;
	}
	if (pushed)
		__atomic_fetch_add(&C.px.repl_pushed, pushed, __ATOMIC_RELAXED);
}
...
static void identity_init(const char *state_dir, const char *legacy_dir)
{
	char path[512];
	int fd, got = 0;
	...
	if (state_dir && *state_dir) {
		...
		if (legacy_dir)
			carry_file(legacy_dir, state_dir, "node-identity");
		...
		const struct sockaddr_in *from)
{
	...
	if (C.role == PC_ROLE_MASTER) {
		...
		if (!id) {
			...
			const char *how = "";
			int rid = reserve_take(n >= 39 ? pt + 21 : NULL,
				&from->sin_addr, &how);
...
static void eager_repl_tick(void)
{
	...
	for (c = 0; c < pc_store_count(); c++) {
		...
		for (i = 0; i < C.n_peers; i++) {
			...
			bf = pr->backfill[c] && lowest_live_sender(pr->node, c);
		...
		const char *state_dir, const char *legacy_dir, int max_pending)
{
	...
	identity_init(state_dir, legacy_dir);
