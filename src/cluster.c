...
static int apply_rec(const struct clapply_rec *r, void *ctx);
...
static int apply_rec(const struct clapply_rec *r, void *ctx)
{
	...
	(void)ctx;
	if (r->collen >= sizeof colz)
		return 0;
	memcpy(colz, r->col, r->collen);
	colz[r->collen] = 0;
	...
	ht = pc_store_find(colz, r->collen);
	 ...
	 * duplicate. */
	{
		...
		if (pc_neg_ver(colz, r->collen, k.s, r->klen, &tv) && tv &&
		        r->ver <= tv) {
	...
	if (r->passive)
		pc_ev_applied(ht, &k);
	if (!r->passive) {
		pc_wal_upsert(colz, k.s, k.len, v.s, v.len, exp, pcache_last_ver);
		pc_loc_clear(colz, r->collen, k.s, r->klen);
		if (pc_store_shard_enabled(ht))
			shard_note_change();
	}
