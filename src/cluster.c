...
/* S105: send a peer's open group; the caller holds qmx */
static void repl_group_send(struct peer *p)
{
	if (!p->qcount)
		return;
	p->q[0] = M_REPL_MANY;
	p32(p->q + 1, 0);                  /* req 0: no ack, see above */
	p16(p->q + 5, (uint16_t)C.node_id);
	p16(p->q + 7, (uint16_t)p->qcount);
	seal_send(&p->addr, p->q, p->qlen);
	__atomic_fetch_add(&C.px.repl_pushed, p->qcount, __ATOMIC_RELAXED);
...
static void repl_flush_due(long long now)
{
	...
	for (i = 0; i < C.n_peers; i++) {
		...
		if (!p->q)
			continue;
		pthread_mutex_lock(&p->qmx);
		if (p->qcount && now - p->qopen_ms >= REPL_FLUSH_MS)
			repl_group_send(p);
		pthread_mutex_unlock(&p->qmx);
	}
}
 ...
 * syscall per peer.  The group is sent by whoever fills it, by the
 * next append that finds it older than REPL_FLUSH_MS, or by the
 * cluster loop's timer.  A record too large for a group goes alone. */
		...
		unsigned long long ver)
{
	...
	for (i = 0; i < C.n_peers; i++) {
		...
		pthread_mutex_lock(&p->qmx);
		if (!p->q) {
			p->q = malloc(MIG_GHDR + REPL_GROUP_FLUSH_BYTES + sizeof rec);
			if (!p->q) {
				pthread_mutex_unlock(&p->qmx);
				continue;
			}
		...
		if (p->qcount && (p->qlen + n > MIG_GATHER_CAP ||
		        nowms - p->qopen_ms >= REPL_FLUSH_MS))
			repl_group_send(p);
		if (!p->qcount) {
			p->qopen_ms = nowms;
			__atomic_fetch_add(&C.repl_open, 1, __ATOMIC_RELAXED);
		}
		memcpy(p->q + p->qlen, rec, n);
		p->qlen += n;
		p->qcount++;
		if (p->qlen >= REPL_GROUP_FLUSH_BYTES || n > MIG_GATHER_CAP)
			repl_group_send(p);
		pthread_mutex_unlock(&p->qmx);
	}
}
		...
		const char *state_dir, const char *legacy_dir, int max_pending)
{
	...
	for (i = 0; i < PC_CL_MAXPEER; i++)
		pthread_mutex_init(&C.peers[i].qmx, NULL);   /* S105 */
