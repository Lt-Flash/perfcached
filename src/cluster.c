...
/* S163: this node's per-collection block - its OWN client hits and
 * misses (pcache_ht_totals_client), entries, stores, removes, expired,
 * the first CLMEMB_COLS_MAX live collections, named by hash */
static void self_cols(struct clmemb_col *cols, int *ncols, int *ntotal)
{
	...
	for (i = 0; i < pc_store_count(); i++) {
		...
		cols[n].hash = clmemb_col_hash(pc_store_name(i));
		cols[n].entries = t.entries;
		cols[n].hits = ct.hits;
		cols[n].misses = ct.misses;
		cols[n].stores = t.stores;
		cols[n].removes = t.removes;
		cols[n].expired = t.expired;
		n++;
	}
	*ncols = n;
	*ntotal = tot;
