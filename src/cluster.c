...
static int ps_skip_mask(const char *chan, size_t clen, uint64_t *skip)
{
	...
	for (tries = 0; tries < 16; tries++) {
		...
		n = __atomic_load_n(&C.pt.n_peers, __ATOMIC_ACQUIRE);
		for (i = 0; i < n; i++) {
		...
		size_t dlen)
{
	...
	frame[0] = M_PUBLISH;
	ps_put16(frame + 1, (unsigned)C.node_id);
	...
	for (i = 0; i < C.pt.n_peers; i++) {
		...
		if (__atomic_load_n(&p->pv_mode, __ATOMIC_ACQUIRE) == PV_ALL) {
			...
			if (!slen) {
				ps_put64(frame + 3, seq);
				if (!(slen = clwire_seal(C.psk, frame, n, sealed)))
					return sent;
			}
			out = sealed;
			olen = slen;
			...
			if (!(olen = clwire_seal(C.psk, frame, n, ps_peer_seal)))
				continue;
			out = ps_peer_seal;
		}
		...
		if (sendto(C.fd, out, olen, 0, (const struct sockaddr *)&p->addr,
		        sizeof p->addr) >= 0) {
			sent++;
...
static void handle_publish(const unsigned char *pt, size_t n)
{
	unsigned node, clen;
	unsigned long dlen;
	unsigned long long seq;
...
static int open_dgram(unsigned char *buf, size_t n, unsigned char *pt,
		unsigned long long *ptlen);   /* defined with the receive loop */
...
int pc_cluster_pubsub_rx_open(int port, int threads)
{
	...
	if (!C.enabled || threads <= 0 || port <= 0 || port > 65535)
		return 0;
	if (threads > PS_RX_MAX)
		threads = PS_RX_MAX;
	if (ps_port_taken(C.self_addr.sin_addr, port)) {
	...
	sa.sin_family = AF_INET;
	sa.sin_port = htons((uint16_t)port);
	sa.sin_addr = C.self_addr.sin_addr;
	for (i = 0; i < threads; i++) {
	...
	LM_NOTICE("cluster: pub/sub relays on %s:%d, %d receive thread(s)\n",
		inet_ntoa(sa.sin_addr), port, ps_rx_n);
	return ps_rx_n;
fail:
	for (i = 0; i < ps_rx_n; i++)
		close(ps_rx_fd[i]);
		...
		const struct sockaddr_in *from)
{
	...
	if (n < 11)
		return;
	msg[0] = M_PSPROBE_ACK;
	ps_put16(msg + 1, (unsigned)C.node_id);
	memcpy(msg + 3, pt + 3, 8);            /* the nonce, echoed */
	len = clwire_seal(C.psk, msg, sizeof msg, buf);
	if (len)
...
static void handle_psprobe_ack(const unsigned char *pt, size_t n,
		const struct sockaddr_in *from)
{
	uint64_t nonce;
	int node, i;
	...
	for (i = 0; i < C.pt.n_peers; i++) {
		...
		if (__atomic_load_n(&p->node, __ATOMIC_ACQUIRE) == node &&
			...
			psr_path_answer(&p->ps, nonce, now_ms());
			return;
		}
	}
}
		...
		unsigned long long *rx_dgrams)
{
	...
	for (i = 0; C.enabled && i < C.pt.n_peers; i++)
		if (__atomic_load_n(&C.pt.peers[i].node, __ATOMIC_ACQUIRE) &&
		    psr_path_direct(&C.pt.peers[i].ps, now))
			d++;
	*port = ps_rx_port;
	*threads = ps_rx_n;
	*peers_direct = d;
...
static void pv_forget(struct peer *p)
{
	int slot = (int)(p - C.pt.peers);
...
static void pv_maybe_request(struct peer *p, long long now)
{
	...
	msg[0] = M_PSINT_REQ;
	ps_put16(msg + 1, (unsigned)C.node_id);
	if (seal_send(&p->addr, msg, sizeof msg) == 0)
...
static struct peer *ps_peer_from(unsigned node, const struct sockaddr_in *from)
{
	int i;
	...
	for (i = 0; i < C.pt.n_peers; i++) {
		...
		if (__atomic_load_n(&p->node, __ATOMIC_ACQUIRE) == (int)node &&
		    p->addr.sin_addr.s_addr == from->sin_addr.s_addr)
			return p;
	}
	return NULL;
		...
		const char *name, size_t nlen, uint32_t h1, uint32_t h2)
{
	...
	if (!(slen = clwire_seal(C.psk, msg, n, sealed)))
		return;
	now = now_ms();
	for (i = 0; i < C.pt.n_peers; i++) {
		struct peer *p = &C.pt.peers[i];
		...
		if (__atomic_load_n(&p->node, __ATOMIC_ACQUIRE) &&
		    clpeers_live(p, now) &&
		    sendto(C.fd, sealed, slen, 0, (const struct sockaddr *)&p->addr,
		        sizeof p->addr) >= 0)
			__atomic_add_fetch(&ps_int_upd_sent, 1, __ATOMIC_RELAXED);
	}
}

static void handle_psint_add(const unsigned char *pt, size_t n,
		const struct sockaddr_in *from)
{
	struct peer *p;
	uint64_t ver;
	...
	if (n < 12 || !(p = ps_peer_from(ps_get16(pt + 1), from)))
		return;
	ver = ps_get64(pt + 3);
	kind = pt[11];
	...
	pv_learn(p, ver);
	pv_evaluate(p, now_ms());
	pv_maybe_request(p, now_ms());
}

static void handle_psint_req(const unsigned char *pt, size_t n,
		const struct sockaddr_in *from)
{
	...
	if (n < 3 || !pc_pubsub_interest_version() ||
	    !ps_peer_from(ps_get16(pt + 1), from))
		return;
	...
	if (len && seal_send(from, msg, 3 + len) == 0)
		__atomic_add_fetch(&ps_int_full_sent, 1, __ATOMIC_RELAXED);
}

static void handle_psint_full(const unsigned char *pt, size_t n,
		const struct sockaddr_in *from)
{
	...
	if (n < 3 + PSF_HDR || !(p = ps_peer_from(ps_get16(pt + 1), from)))
		return;
	bn = n - 3;
	ver = ps_get64(b);
...
static void ps_interest_tick(long long now)
{
	int i;
...
void pc_cluster_pubsub_interest_figures(struct pc_psint_figures *f)
{
	...
	for (i = 0; C.enabled && i < C.pt.n_peers; i++) {
		const struct peer *p = &C.pt.peers[i];
		unsigned char m;
...
int pc_cluster_members(struct pc_member *out, int max)
{
	...
	out[n].relay_port = ps_rx_port;                  /* PS11 */
 ...
 */
static int replicas_short_now;
#define REPLICAS_SHORT_HOLD 3

static void replicas_short_tick(long long now)
{
	static int last = -1, held;
	int k = pc_cluster_replicas(), live, s;

	if (k <= 0)
		return;
	if (pc_node_state() < PC_NST_READY || pc_shard_grace())
		return;                        /* not yet: hold the reading */
	...
	if (++held < REPLICAS_SHORT_HOLD || s == replicas_short_now)
		return;
	if (s)
		...
		LM_NOTICE("cluster: replicas = %d met again by %d live "
			"members\n", k, live);
	__atomic_store_n(&replicas_short_now, s, __ATOMIC_RELAXED);
}

int pc_cluster_replicas_short(void)
{
	return __atomic_load_n(&replicas_short_now, __ATOMIC_RELAXED);
}
...
int pc_cluster_col_fleet(int store, int basis, struct pc_col_fleet *out)
{
	...
	col_fill(store, &own);
	out->basis = basis;
	out->members = out->reporting = 1;
	out->copies = emax = own.entries;
	xsum = xmax = own.expired;
	out->hits = own.hits;
	out->misses = own.misses;
	out->stores = own.stores;
	out->removes = own.removes;
	for (i = 0; i < C.pt.n_peers; i++) {
		struct peer *p = &C.pt.peers[i];
		...
		if (!__atomic_load_n(&p->node, __ATOMIC_ACQUIRE) ||
			...
			continue;
		out->members++;
		nc = p->cols_reported ? p->ncols : 0;
		for (k = 0; k < nc && k < CLMEMB_COLS_MAX; k++) {
			if (p->cols[k].hash != own.hash)
				continue;
			c = p->cols[k];
			out->reporting++;
			out->copies += c.entries;
			if (c.entries > emax)
				emax = c.entries;
			xsum += c.expired;
			if (c.expired > xmax)
	...
	case PC_FLEET_PER_K:
		...
		d = pc_cluster_replicas();
		if (d > out->reporting)
			d = out->reporting;
		if (d < 1)
			d = 1;
		out->entries = (out->copies + (uint64_t)d / 2) / (uint64_t)d;
		out->expired = (xsum + (uint64_t)d / 2) / (uint64_t)d;
		break;
	default:
...
static void wgroup_send_cb(struct wgroup *g, void *ctx)
{
	...
	clmig_group_hdr(g->q, CLMIG_GHDR, M_REPL_MANY, 0, C.node_id,
		g->qcount);
...
static void peer_gone(struct peer *p)
{
	...
	p->gone_ms = now_ms();
	pv_forget(p);                          /* PS12: the slot may be reused */
		...
		const struct sockaddr_in *from)
{
	...
	if (p) {
			...
			memcpy(p->cols, a.cols, (size_t)a.ncols * sizeof p->cols[0]);
		{
			...
			pv_heartbeat(p, (a.have & CLMEMB_A_PSRELAY) ? a.ps_interest : 0,
				now_ms());
		}
...
static int drain_one_fd(int fd, int slot)
{
	...
	while (seen++ < DRAIN_BATCH) {
		...
		case M_PUBLISH:   handle_publish(pt, ptlen); break;   /* PS3 */
		...
		case M_PSPROBE_ACK:  handle_psprobe_ack(pt, ptlen, &from); break;   /* PS11 */
		case M_PSINT_ADD:    handle_psint_add(pt, ptlen, &from); break;     /* PS12 */
		case M_PSINT_REQ:    handle_psint_req(pt, ptlen, &from); break;
		case M_PSINT_FULL:   handle_psint_full(pt, ptlen, &from); break;
		default:
			break;
		}
	}
...
static void spread_note_set_change(void)
{
	...
	if (!pc_cluster_replicas())
		return;
	for (i = 0; i < C.pt.n_peers; i++)
		if (clpeers_holds_slots(&C.pt.peers[i], now))
			...
			fp ^= fnv1a64_u64(FNV1A64_BASIS,
				(unsigned long long)
				C.pt.peers[i].addr.sin_addr.s_addr << 16 |
				(unsigned long long)
				C.pt.peers[i].addr.sin_port);
	if (fp == last_fp)
		return;
	if (last_fp) {
...
#define PC_RECLAIM_KEYMAX        256   /* keys longer than this are left;
                                        * the buffer is per-tick stack */
...
static int spread_settled(int c)
{
	 ...
	 */
	{
		const struct pc_clmap *m = cl_map_usable() ? cl_map() : NULL;
		int seen = 1, j;

		for (j = 0; j < C.pt.n_peers; j++)
			if (clpeers_holds_slots(&C.pt.peers[j], now))
				seen++;
		 ...
		 * 207 of 240 owed.
		 */
		if (!pc_clplace_has(m, (uint16_t)C.node_id))
			return 0;
	}
	for (i = 0; i < C.pt.n_peers; i++) {
		struct peer *pr = &C.pt.peers[i];
...
struct reclaim_ctx {
	...
	struct { char key[PC_RECLAIM_KEYMAX]; int klen; } hit[SPREAD_RECLAIM_PER_TICK];
};
		...
		void *arg)
{
	...
	if (r->n >= SPREAD_RECLAIM_PER_TICK)
		return -1;
	if (key->len <= 0 || key->len > PC_RECLAIM_KEYMAX)
		return 0;
	slot = pc_key_slot(key->s, (size_t)key->len);
	if (pc_spread_in_set(slot, 0, r->k))
		return 0;                      /* we ARE a holder: keep it */
	if (!pc_spread_set_live(slot, r->k))
...
static void spread_reclaim_tick(void)
{
	...
	for (c = 0; c < pc_store_count(); c++) {
		...
		if (!spread_settled(c)) {
			settled_for[c] = 0;
			cursor[c] = 0;         /* restart the sweep when it is */
			continue;
		}
		if (++settled_for[c] < SPREAD_SETTLE_TICKS)
			continue;              /* settled, but not for long enough */
		ht = pc_store_ht(c);
		nm = pc_store_name(c);
		r.k = k;
		r.n = 0;
...
void pc_cluster_get_stats(struct pc_cl_stats *out)
{
	...
	*out = C.st;
	...
	out->replicas_short = replicas_short_now;      /* S157 */
}
