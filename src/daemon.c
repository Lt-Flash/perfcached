...
/* S176: the leg drain's budget is a TIME slice, like the splitter's.
 *
 * It was a flat 1,024 per tick, and named for chains - but the drain's
 * loop does one RECORD per iteration, so the real bound was 1,024
 * records a second.  Measured on the fleet 2026-09-20: a production
 * keyspace loaded into a small table put 32,651 records in the leg and
 * they came back at ~630/s, fifty seconds of slow lookups; and because
 * the growth signal only publishes when a whole pass ENDS, the table
 * did not widen until the drain had chewed through the leg either.
 *
 * S131 learned this for the splitter and the same sentence applies:
 * a count is the wrong bound, what must be bounded is the time this
 * thread spends here.  So: chunks until the slice is spent or a whole
 * pass over the leg is done, whichever comes first.
 *
 * Stopping at "a chunk moved nothing" was tried first and is wrong: the
 * records a widening has just made placeable sit wherever the cursor is
 * NOT, so the drain crawled a chunk a tick until it reached them
 * (measured: three ticks stuck at 3,060 records).  A pass is the honest
 * bound - past it the drain would be re-examining what it just saw -
 * and a barren pass is cheap, since a chain with nothing in it costs a
 * lock and a pointer read. */
#define PC_DRAIN_CHUNK    1024
#define PC_EVAC_BUCKETS   4096         /* S190: buckets per second */
#define PC_EVAC_BUDGET    512          /* records moved per second, at most */
#define PC_DRAIN_SLICE_US 10000
...
/*
 * S239: what AUTO would do with table @i, as a log2 bucket count - 0 when
 * it would leave it.  Grow: entries past grow_at_pct of the slots.  Shrink:
 * below shrink_at_pct, and more than one segment.  Either way the target
 * is the size midway between the two thresholds, as shrink_consider picks
 * it.  Published for /stats, /metrics and the page, every mode.  Under
 * WARN it is also said in the log - the first time at once (the boot
 * clock starts at 0, S242's lesson), then at most every ten minutes, and
 * again whenever the target changes.
 */
static unsigned int as_warned_at[PC_MAX_COLLECTIONS];
static int as_warned_t[PC_MAX_COLLECTIONS];

static void autoscale_advise(int i, pcache_htable_t *cht, unsigned int now)
{
	pcache_ht_totals_t tot;
	unsigned int nb = pcache_ht_nbuckets(cht), cur = 0, t = 0;
	unsigned long long mid = (unsigned long long)(pc_grow_at_pct + pc_shrink_at_pct) / 2;
	int grow;
	const char *name;
	...
	while ((1U << cur) < nb)
		cur++;
	grow = tot.entries * 100 > (unsigned long long)pc_grow_at_pct * PCACHE_SLOTS * nb;
	if (grow) {
		for (t = cur + 1; t < 30; t++)
			if (tot.entries * 100 <= mid * PCACHE_SLOTS * (1ULL << t))
				break;
	} else if (pc_shrink_at_pct > 0 && nb > PCACHE_SEG_SIZE &&
	           tot.entries * 100 < (unsigned long long)pc_shrink_at_pct * PCACHE_SLOTS * nb) {
		for (t = PCACHE_SEG_BITS; t < cur; t++)
			if (tot.entries * 100 <= mid * PCACHE_SLOTS * (1ULL << t))
				break;
		if (t >= cur || (1ULL << (t + 1)) > nb)
			t = 0;             /* less than a halving: not worth a copy */
	}
	pc_store_set_autoscale_target(i, (int)t);
	if (!t || pc_store_autoscale(i) != PC_AUTOSCALE_WARN)
		return;
	if (as_warned_at[i] && as_warned_t[i] == (int)t &&
	    now - as_warned_at[i] < 600)
		return;
	as_warned_at[i] = now ? now : 1;
	as_warned_t[i] = (int)t;
	name = pc_store_name(i);
	LM_WARN("collection '%s' (autoscale = warn): %llu records in %u buckets "
		"(%llu%% of the slots%s) - auto would %s it to 2^%u.  Resize it "
		"(`perfcli resize %s %u`) or let it size itself (`perfcli "
		"autoscale %s auto`)\n", name,
		(unsigned long long)tot.entries, nb,
		(unsigned long long)tot.entries * 100 /
			((unsigned long long)PCACHE_SLOTS * nb),
		grow ? ", growth blocked: the rest is in the overflow leg" : "",
		grow ? "grow" : "shrink", t, name, t, name);
}
...
static void *maint_main(void *p)
{
	...
	while (!stop_flag) {
		...
		pc_qs_enter();
		rpc_drain(self_slot);
		/* S188: the commit-ahead window at THIS loop's cadence, not
		 * the 1 Hz one below.  A bulk load crosses a 2 MB group
		 * boundary every ~50 ms and a once-a-second refill cannot
		 * follow it, so the crossings fell through to the worker
		 * holding the client's write - the millisecond tail S166
		 * exists to remove.  Everything else here stays on the
		 * second; an idle node pays one predicate ten times a
		 * second and takes nothing. */
		pcache_arena_ahead_tick();
		now = get_ticks();
		if (now == last)
			continue;
		last = now;
		...
		for (i = 0; i < pc_store_count(); i++) {
			 ...
			 * 0, which is what it cost before. */
			{
				...
				/* S239: only AUTO sizes the table; under WARN
				 * and OFF the keys past its slots go to the leg */
				if (pc_store_autoscale(i) == PC_AUTOSCALE_AUTO)
				do {
			 ...
			 * looks in the leg, so nothing else would.  S176: for
			 * as long as it keeps yielding, inside a slice. */
			{
				struct timespec d0, d1;
				unsigned long p0 = pcache_ht_drain_passes(cht);
				long long dus = 0;

				clock_gettime(CLOCK_MONOTONIC, &d0);
				do {
					pcache_ht_drain_leg(cht,
						PC_DRAIN_CHUNK);
					clock_gettime(CLOCK_MONOTONIC, &d1);
					dus = (long long)(d1.tv_sec - d0.tv_sec)
						* 1000000LL +
						(d1.tv_nsec - d0.tv_nsec) / 1000;
				} while (dus < PC_DRAIN_SLICE_US &&
					 pcache_ht_drain_passes(cht) == p0 &&
					 pcache_ht_overflow(cht));
			}
			/* S190: move the survivors of a mass expiry out of the
			 * chunks they pin, so those chunks can retire.  The
			 * arena says which chunks are sparse (its reclaim
			 * tick); this walks a slice of buckets per second and
			 * moves at most a few hundred records, and does nothing
			 * when no chunk is sparse - the ordinary case. */
			if (pcache_arena_sparse_chunks())
				pcache_ht_evacuate(cht, PC_EVAC_BUCKETS, PC_EVAC_BUDGET);
			...
			if (pc_store_autoscale(i) == PC_AUTOSCALE_AUTO)
				shrink_consider(i, cht, now,
					pcache_arena_regions_bytes() > rg0);
			autoscale_advise(i, cht, now);          /* S239 */
		}
	}
	return NULL;
}
...
/* S217: a replica-apply thread */
static void *apply_main(void *p)
{
	struct psrx_arg *a = p;

	slot_attach(a->slot);
	thread_name("pc-apply");
	pc_cluster_apply_thread((volatile int *)&stop_flag, a->idx);
	return NULL;
}
...
int pc_daemon_run(struct pc_config *cfg)
{
	...
	struct psrx_arg apth[16];               /* S217 */
	int npsrx = 0, napply = 0;
	struct sigaction sa;
	...
	pc_autoscale_default = cfg->autoscale;                 /* S239 */
	...
	for (i = 0; i < cfg->n_col; i++) {
		...
		if (cfg->col[i].has_autoscale)                               /* S239 */
			pc_store_set_autoscale_idx(i, cfg->col[i].autoscale);
	...
	pc_verb_set_resp_redirect(cfg->resp_redirect);      /* RV-10 */
	...
	if (cfg->wal_dir) {
		...
		if (pc_wal_init(cfg->wal_dir,
		        ...
		        /* S217: the apply threads sit at nw+7.. and a
		         * migration they apply lands in THEIR ring, so the
		         * WAL needs one per thread; 1 (the default) adds none */
		        cfg->workers + 6 + (cfg->cl_apply_threads >= 2
		            ? 1 + cfg->cl_apply_threads : 0)) != 0) {
	...
	if (!cfg->cl_enabled) {
		...
		/* S215: pc_cluster_init() is where this was installed, so a
		 * daemon with no cluster never had it - a ring drop, an
		 * overrun or a refused sync logged its CRIT and the node went
		 * on taking writes.  After READY, so a failure the WAL found
		 * at startup (delivered by this call) is what stands. */
	...
	if (cfg->cl_enabled) {
		...
		pc_cluster_set_apply_stall_ms(cfg->cl_apply_stall_ms);   /* S216 */
		pc_cluster_set_apply_threads(cfg->cl_apply_threads);     /* S217 */
		...
		napply = pc_cluster_apply_threads();                     /* S217 */
	...
	if (cfg->cl_enabled) {
		...
		/* S217: the apply threads take nw+7.., the relay threads
		 * follow them - the WAL ring count above depends on it */
		for (i = 0; i < napply; i++) {
			apth[i].slot = slot_init(nw + 7 + i);
			apth[i].idx = i;
			pthread_create(&apth[i].slot->tid, &tattr, apply_main, &apth[i]);
		}
		for (i = 0; i < npsrx; i++) {
			psrx[i].slot = slot_init(nw + 7 + napply + i);
			psrx[i].idx = i;
	...
	/* S217: after the cluster thread (ct) - nothing pushes any more,
	 * and a thread drains what it holds before it sees the flag */
	for (i = 0; i < napply; i++)
		pthread_join(apth[i].slot->tid, NULL);
	for (i = 0; i < npsrx; i++)
	...
	/*
	 * S222: A CLEAN STOP LEAVES A SNAPSHOT.
	 *
	 * Until this, a node stopped on purpose came back off its WAL alone -
	 * correct, and in eager mode exactly the problem: an eager replica
	 * never logs the copies it receives, so the WAL holds only what this
	 * node WROTE.  After the whole fleet was stopped, every node returned
	 * with its own share and waited on the repair sweep for the rest;
	 * none of them could have served the whole keyspace alone.  The
	 * snapshot walk stores the copies too, so a node that stopped cleanly
	 * now comes back complete on its own.
	 *
	 * Here, after every join: nothing writes, the WAL has had its final
	 * flush, the RDB thread is gone, and the arena is still intact.  Any
	 * mode with a WAL directory - a store node's pulled copies and a
	 * spread node's placed ones are lost from its WAL in the same way.
	 * A failure is logged and changes nothing else: the WAL is intact.
	 */
	if (rt) {
		LM_NOTICE("perfcached: stopping - taking the shutdown snapshot\n");
		if (pc_rdb_save_on_stop() != 0)
			LM_ERR("perfcached: the shutdown snapshot FAILED - the WAL "
				"still holds everything this node wrote, but it will "
				"come back without the copies it held for its "
				"peers\n");
	}
