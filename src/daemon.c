...
static void *worker_main(void *p)
{
	...
	int ep, i, j, n, nlfd = 0, http_open = 0;
	...
	while (!stop_flag) {
		...
		n = epoll_wait(ep, evs, 32,
			pc_enum_pending() ? 0 : (http_open ? 1000 : -1));
		http_open = pc_conn_sweep_http(&conns,
			CFG->http_timeout_s);
		if (n < 0) {
			if (errno == EINTR)
				continue;
...
static void *maint_main(void *p)
{
	...
	while (!stop_flag) {
		...
		for (i = 0; i < pc_store_count(); i++) {
			...
			cht = pc_store_ht(i);
			pcache_ht_sweep(cht, now, NULL, NULL);
