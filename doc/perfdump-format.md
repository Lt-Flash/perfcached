...
again. That is the cursor's at-least-once contract under growth, the
same one Redis `SCAN` has, and `perfload` upserts by version, so a
repeated record costs bytes, never correctness.

A resize is not a split. An operator's `resize`, or the automatic
shrink after mass deletes, builds a new table of another shape and swaps
it in, and a bucket cursor from the old table means nothing in the new
one: carried across, the walk repeats records after a grow and silently
MISSES them after a shrink. So every `dump` reply names the table it
walked (`table`, with its `buckets`); a caller that passes `table` back
on its next call is refused ("table resized during the dump") once the
collection has been swapped, and must restart that collection from
cursor 0. perfdump (0.4.0-rc33 and later) pins every walker of a
collection to one table, plans their bucket ranges over that table's
`buckets` rather than the count in `stats`, and on a refusal discards
the collection's files and dumps it again - up to five times, then it
fails rather than write a dump it cannot vouch for. The manifest's
`buckets` and each file's `bucket_lo`/`bucket_hi` are the pinned
table's. Records expiring during
