 ...
 * routing OFF, the daemon forwards, correctness costs a hop.  A daemon
 * too old to report the field at all says 0, which is the grandfather
 * case and stays acceptable only while we are at epoch 1. */
#define PERFD_WIRE_EPOCH 1
...
static int parse_members(perfd_t *p, const char *res)
{
	 ...
	 * routing off - the client still works, it just pays the forward. */
	{
		...
		if (tr >= 0) {
			...
			int tep = pc_json_get(res, t, ntok, tr, "epoch");
			int their_epoch = tep >= 0 ?
				atoi(res + t[tep].start) : 0;
			int epoch_ok = their_epoch == PERFD_WIRE_EPOCH ||
				(their_epoch == 0 && PERFD_WIRE_EPOCH == 1);
			int algo_ok = ta >= 0 &&
			...
			p->route_epoch = their_epoch;
			p->route_ok = p->route_want && algo_ok && mode_ok &&
				epoch_ok && p->route_cport > 0;
		}
	}
	free(t);
	p->nmem = n;
