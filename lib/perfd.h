...
#define PERFD_VERSION_PATCH 11
#define PERFD_VERSION "0.2.11"
 ...
 * an id-less {"notify":"membership","event":"joined"|"expelled",...}
 * to every client when a node joins or is expelled; the library dials a
 * newcomer and drops an expelled member - connection, standby, routing
 * weight - before the application's perfd_set_notify() hook sees the
 * message.  A handle with no traffic acts on nothing: call
 ...
 * (perfd_set_notify) as id-less JSON, {"jsonrpc":"2.0","method":"message",
 * "params":{"channel":"...","payload":"..."[,"enc":"b64"]}} and "pmessage"
 * with a "pattern" member - the connection stays multiplexed, a blocking
 * call keeps waiting for its own id.  They are dispatched whenever the
 ...
 * They reach the same notify hook with the same JSON as a delivery on the
 * connection.  The daemon sends only to this connection's own address,
 ...
 * connection.  That drop arrives as the notification
 * {"method":"pubsub_udp_pruned",...}: the handle closes its socket and
 * does NOT re-subscribe - the application decides.  A failover to
