...
**0.4.0** line is at release candidate 27 (2026-09-22), which is what
...
CHANGELOG says so against each.  Start from rc27, a capability step: **Redis pub/sub on every door and
...
everything, and the hand-off itself costs about 0.7 us a record against
the ~1 us the apply takes, so raise this only when the receiving thread
is demonstrably the bottleneck - and **use 2 at most**: measured on an
idle 16-core receiver, 4 threads apply only 47-83% of the offered load
and drop tens of thousands of datagrams at the socket (S220).  A hot key
pins one thread.  `stats.cluster` shows `apply_threads`,
