...
**Status.**  The last tagged release is **0.3.7** (2026-09-15),
source-identical to its sixteenth candidate, which the staging fleet
runs.  What the line added, in order: `spread` mode (K copies of P, routed by libperfd
...
traffic; the staging fleet runs `eager`.  One thing found while
measuring is filed rather than fixed: a spread node that restarts
empty holds the whole keyspace until the reclaim pass trims it back
(S156 in the changelog).
...
[Clients](#clients) (libperfd, PHP, cachedb_perfd) ·
...
- **`resp = <addr:port>`** adds a dedicated listener for Redis clients
  ...
  included), `CLIENT LIST`/`SETNAME`, `SLOWLOG`, and the `CLUSTER`
  family (`SLOTS`/`SHARDS`/`KEYSLOT`/`NODES`) - so Grafana's
...
1. **Index.**  Each collection's table is carved at creation.
   ...
   has moved on.  A create or a resize whose index would not fit under
...
verb answers, as JSON), `/metrics` for Prometheus, and `/members`.
...
Every table names the build it came from and the date; the harness
stamps the binary's revision into the results and refuses to measure a
build that cannot name itself.  Read a figure with what bounded it:
the SET cells at capacity pin the server's cores, most GET cells are
bounded by the client or the wire, and a loopback read is a best case a
1500-byte network does not reproduce.  The method behind each table,
how to reproduce it, and the readings that changed as the harnesses
improved are in [bench/RESULTS.md](bench/RESULTS.md); raw rows are in
[bench/results/](bench/results/).  Numbers rot: quote from a run, not
from here.

### One server against one server

`redis-benchmark` — Redis's own tool, unmodified — against one
perfcached, no cluster, and against Redis, both on the same 16-vCPU
Debian 13 host, on loopback.  `build=2db7baf` (v0.3.0-rc3 + 13 commits),
2026-09-03; 20k keys x 200 B, cells sized to run for over a second,
median of 3 (`bench/respbench.sh`,
[bench/results/respbench.tsv](bench/results/respbench.tsv)).
...
Parity at depth 1 is the expected result: with one request in flight
per connection both servers wait on the round trip.  At depth 64 the
difference is real - **+120% on SET at a 2.5x tighter p99** - because
Redis is single-threaded and perfcached runs four workers.  (`tail` is
how many times tighter the SET p99 is.)

### Across a real network, client on another host

The same comparison run properly: `redis-benchmark` on one host, both
servers on another, a real NIC between them at MTU 9000 (verified end
to end).  Same build (`2db7baf`), median of 3, arms alternated
(`bench/xhostbench.sh`,
...
This is the number a deployment gets.  Loopback and the wire agree on
SET (x2.20 there, x2.22 here); the wire costs perfcached a little more
than Redis on GET; and loopback understated depth 1, where sharing a
host was costing perfcached real work.
...
the same rig.  0.3.7's daemon (`4d5b412`, source identical to
`v0.3.7-rc16`), 2026-09-15, 16-vCPU Ubuntu 24.04 under podman, 20k
keys x 200 B, 8 workers a node, 50 clients, median of 3, every cell
sized to run for at least 10 seconds (`bench/containerbench.sh`,
[bench/results/containerbench-rc16.tsv](bench/results/containerbench-rc16.tsv)).
One node on its own, same rig, same client: 2,210,222 / 2,390,590 at pipeline 16,
2,463,984 / 2,699,632 at 64.

**50 clients, pipeline 16** (redis: 491,205 SET / 544,500 GET):

| mode | SET/s | GET/s | vs redis | SET p99 |
|---|---|---|---|---|
| store | 2,254,387 | 2,441,798 | x4.59 / x4.48 | 0.94 ms |
| eager | 1,411,025 | 2,440,377 | x2.87 / x4.48 | 1.74 ms |
| proxy | 2,056,531 | 2,494,154 | x4.19 / x4.58 | 1.09 ms |
| **shard** | **235,045** | **210,327** | **x0.48 / x0.39** | 1.55 ms |
| **spread K=2** | **1,434,194** | **428,545** | **x2.92 / x0.79** | 1.81 ms |

**50 clients, pipeline 64** (redis: 785,162 SET / 857,965 GET):

| mode | SET/s | GET/s | vs redis | SET p99 |
|---|---|---|---|---|
| store | 2,543,424 | 2,598,972 | x3.24 / x3.03 | 3.10 ms |
| eager | 1,694,195 | 2,656,439 | x2.16 / x3.10 | 4.61 ms |
| proxy | 2,192,954 | 2,646,851 | x2.79 / x3.09 | 3.24 ms |
| **shard** | **146,515** | **135,350** | **x0.19 / x0.16** | 2.51 ms |
| **spread K=2** | **1,658,636** | **352,709** | **x2.11 / x0.41** | 4.80 ms |
...
Redis client pays for the mode.  Both are the
floor; the next two tables are the same fleets through clients that
can route.
<!-- /containerbench:rc16 -->

### The same fleet through a cluster-aware client

Same fleet, same daemon, same moment — `natbench` over `libperfd` with
`opts.route_keys`, binary dialect.  `build=76fc29b`, 2026-09-12.
...
The mode a Redis client drives at x0.19 comes within 2% of store once
the client can route: shard was never the slow mode, it was the mode
whose client could not compute an owner.  Routing buys two things - the
removed hop and connections spread over the fleet - decomposed in
[bench/RESULTS.md](bench/RESULTS.md).

**The same two fleets through a Redis client that routes.**  Run 2 on
the same rig and daemon, 2026-09-15: `redis-benchmark` given one hash
tag per owner from `CLUSTER NODES`, so three drivers each talk to the
node that owns their keys - what a cluster-aware Redis client does
with `CLUSTER SLOTS`, without one.  The plain rows are the same fleets
through the one-node client of the table above
([bench/results/containerbench-rc16-routed.tsv](bench/results/containerbench-rc16-routed.tsv)).

**50 clients, pipeline 16** (redis: 463,987 SET / 509,471 GET):

| mode | client | SET/s | GET/s | vs redis | SET p99 | GET p99 |
|---|---|---|---|---|---|---|
| shard | plain | 245,942 | 207,566 | x0.53 / x0.41 | 1.86 ms | 1.28 ms |
| shard | **routed** | **2,571,413** | **2,908,750** | **x5.54 / x5.71** | 0.88 ms | 0.71 ms |
| spread K=2 | plain | 1,420,432 | 428,977 | x3.06 / x0.84 | 1.42 ms | 1.47 ms |
| spread K=2 | **routed** | **1,909,546** | **2,851,709** | **x4.12 / x5.60** | 1.22 ms | 0.82 ms |

**50 clients, pipeline 64** (redis: 778,805 SET / 811,250 GET):

| mode | client | SET/s | GET/s | vs redis | SET p99 | GET p99 |
|---|---|---|---|---|---|---|
| shard | plain | 155,063 | 143,885 | x0.20 / x0.18 | 3.14 ms | 2.13 ms |
| shard | **routed** | **4,365,587** | **5,425,932** | **x5.61 / x6.69** | 1.75 ms | 1.44 ms |
| spread K=2 | plain | 1,719,350 | 350,820 | x2.21 / x0.43 | 5.25 ms | 1.27 ms |
| spread K=2 | **routed** | **2,938,075** | **5,756,621** | **x3.77 / x7.10** | 4.85 ms | 1.47 ms |

Routing turns shard from the slowest arm into the fastest, and it
takes spread's reads with it: every read lands on a holder, so the
pull path is never taken.  Spread's SET stays at its copy cost - the
holder that takes the write still pushes the second copy - which is
the price of the mode and the only cost routing cannot remove.  Three
drivers beside three nodes on a 16-vCPU host make these cells
client-bound; the off-box farm in the next section is the ceiling.

**Read through a node holding nothing** (50 clients, pipeline 64, the
RESP client by construction; same run as the tables above - node 2 is
restarted empty before every rep and read through):

| mode | GET/s | GET p99 |
|---|---|---|
| store | 3,047,663 | 0.96 ms |
| eager | 2,862,855 | 1.01 ms |
| spread K=2 | 2,922,175 | 1.09 ms |
| shard | 149,237 | 2.09 ms |
| proxy | 74,278 | 50.98 ms |

Store pulls the key and keeps it, eager already replicated it, and a
spread node that comes back empty is backfilled with the whole
keyspace before its surplus is trimmed away, 64 keys a tick, so inside
the bench window it reads everything locally at eager's rate.  The
node that took the writes, two tables up, is the steady state: a
spread node never keeps what it pulls, so one read in three pulls
every time.  The two modes that have to go and get the key differ in
how: shard unicasts to the one owner, proxy consults a locator and
broadcasts on a miss.

### `spread` at capacity, from an off-box client

`bench/spreadbench.sh`, rc16 (`286eb22`), 2026-09-15.  P daemons on one
16-core host; natbench (this tree's, over libperfd, binary dialect, 64
connections, 8 threads, depth 32, 200,000 keys x 200 B, 30 s cells) on
separate hosts: one 12-core host for the SET and un-routed cells, a
20-core farm (12 + 8) for the routed and eager GET cells, which one host
could not saturate.  *Un-routed* is one natbench per node with the
connections split - a dumb load balancer; *routed* is one natbench
letting libperfd 0.2.8 send each key to a holder.  Server cores are the
fleet's CPU-seconds over the cell: the three-node fleets run 5 workers
a node (15 threads), the six-node fleets 2 (12), so a cell at the
thread count is a server ceiling.  Eager is the yardstick on the same
fleet.

| fleet | SET, un-routed | SET, routed | GET, un-routed | GET, routed / eager GET |
|---|---|---|---|---|
| eager, 3 nodes | 1.84M (15.0 cores, p99 2.3 ms) | - | - | **4.10M** (14.3, p99 5.3 ms) |
| spread K=2, 3 nodes | 1.92M (15.1) | **2.08M** (15.3, p99 2.4) | **0.33M** (11.6, p99 42 ms) | **4.25M** (14.4, p99 2.9) |
| spread K=3, 3 nodes | 1.79M (15.5) | 1.82M (15.4) | 2.77M (= eager, one host) | **4.30M** (14.6, p99 3.1) |
| eager, 6 nodes | **1.11M** (16.0, p99 4.7 ms) | - | - | 3.51M (11.9, p99 4.4) |
| spread K=2, 6 nodes | 1.58M (15.1) | **1.76M** (14.3, p99 3.0) | **0.31M** (13.6, p99 34 ms) | 3.60M (12.1, p99 3.8) |
| spread K=3, 6 nodes | 1.38M (15.9) | 1.48M (15.6, p99 3.8) | 0.28M (12.1, p99 36 ms) | 3.69M (12.1, p99 3.5) |

Writes are a server ceiling in every cell, and eager's falls with the
fleet.  Un-routed spread reads are bound by the pull path.  Routed
spread reads are eager's, within 5%, at the fleet's worker ceiling -
14.4 of 15 threads on three nodes, 12.1 of 12 on six - with a lower p99
than eager's even split of connections, and zero pulls: ~285k GET/s a
worker thread either way.  Rows, with p50, pulls and forward counts per
cell: [bench/results/spreadbench.tsv](bench/results/spreadbench.tsv)
(`clients` = 1 or 2 says which farm drove the row).

### What bounds a read

Same host, same binary, same container, only the MTU changed:
...
Writes barely move; reads nearly triple.  A loopback GET figure is a
best case a 1500-byte network does not reproduce, and a deployment that
can raise its MTU should.  The rest of that story - including the GET
allocation that hid behind it - is in
[bench/RESULTS.md](bench/RESULTS.md).
