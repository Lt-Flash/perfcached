...
**0.4.0** line is at release candidate 39 (2026-09-26), which is what
the test fleet runs.  Candidates 10, 13, 14, 30-34 and 36-38 of that
line are RED - their pipelines failed `tailtest`, `growtest`, `pintest`,
`sweepclocktest` (30, 31), `loadtest` (32), `keysretiretest` and
`outagetest` (33), `applythreadstest` (34), `healtest` (36, 38) and
`rdbkilltest` (37), each a suite that runs only on a tag - and 11, 12
and 35 are superseded; the CHANGELOG says so against each.  Start from
rc39, a capability step: **Redis pub/sub on every door and
