...
**0.4.0** line is at release candidate 32 (2026-09-25), which is what
the test fleet runs.  Candidates 10, 13, 14, 30 and 31 of that line are
RED - their pipelines failed `tailtest`, `growtest`, `pintest` and (30,
31) `sweepclocktest`, each a suite that runs only on a tag - and 11 and
12 are superseded; the CHANGELOG says so against each.  Start from rc32, a capability step: **Redis pub/sub on every door and
