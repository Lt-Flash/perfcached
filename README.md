...
**0.4.0** line is at release candidate 14 (2026-09-20), which is what
the test fleet runs.  Candidates 10 and 13 of that line are RED - their
pipelines failed `tailtest` and `growtest` - and 11 and 12 are
superseded; the CHANGELOG says so against each.  Start from rc14, a capability step: **Redis pub/sub on every door and
