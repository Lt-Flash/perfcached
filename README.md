...
**0.4.0** line is at release candidate 40 (2026-09-26), which is what
...
only on a tag - and 11, 12 and 35 are superseded; the CHANGELOG says so
against each.  Start from rc40, a capability step: **Redis pub/sub on every door and
...
every node stops that way, and the one you asked reports which peers
went and which did not before it stops too.  It needs the `[secrets]
