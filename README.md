...
1. **Index.**  Each collection's table is carved at creation:
   2^`buckets_log2` buckets at about 200 bytes each, never freed.
   log2 18 costs 52 MB, log2 16 costs 13 MB, log2 12 under 1 MB - before
   a single record.  A table grows by splitting once it passes four
   entries per bucket, and growth adds index.
...
5. **What the host sees.**  The reservation is populated whole at start,
   so resident memory is `arena_mb` plus about 15 MB for the daemon from
   the first second; once idle the give-back returns the never-carved
   part, and after a burst it returns the drained groups, so resident
   follows what is held to within a few 2 MB groups.  With
   `arena_cap_mb` only the floor is resident at start and growth comes
   and goes.  The arena's pages are counted in RSS when the tier is
