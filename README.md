...
**0.4.0** line is at release candidate 44 (2026-09-26), which is what
the test fleet runs.  Candidates 10, 13, 14, 30-34, 36-39 and 42 of
that line are RED - their pipelines failed `tailtest`, `growtest`, `pintest`,
`sweepclocktest` (30, 31), `loadtest` (32), `keysretiretest` and
`outagetest` (33), `applythreadstest` (34), `healtest` (36, 38),
`rdbkilltest` (37), `walsizetest` (38, 39) and `eagerparttest` (42),
each a suite that runs only on a tag - and 11, 12, 35, 40, 41 and 43 are superseded; the
CHANGELOG says so against each.  Start from rc44, a capability step: **Redis pub/sub on every door and
...
- **One number says whether two builds may share a cluster.**  Peer
  frames carry a *wire epoch*, reported as `wire_epoch` in
  `stats.cluster` and as `perfcached_cluster_wire_epoch`.  Releases
  with the same epoch federate freely, whatever their version numbers;
  a peer with a different epoch is refused at the door by both sides,
  with a message naming both numbers, and `epoch_refused` counts it.
  The epoch moves only when a frame would be **mis-executed** by an
  older build rather than ignored by it - an addition to the tail of a
  frame is not such a change and does not move it.  So a rolling
  upgrade stays the normal case, and the release notes say plainly when
  one is not available: at an epoch change, stop the fleet, upgrade
  every node, start it again.
...
master steps down when it sees a bigger fleet.  **A partition has one
known gap in eager mode:** writes made on both sides are kept when it
heals, but a DELETE made on either side while the fleet was split can
be undone, because the other side still holds the key and sends it
back.  A node that crashed and restarted does not have this problem:
on restart it asks the fleet which of its keys were deleted while it
was down.  A partition that heals without a restart asks nothing.
Until that is fixed, re-issue deletes that matter after a split.
Failure detection
...
survives, so that a key deleted while it was away is not resurrected -
but only if some peer was up throughout its absence.  After an outage
...
node).  A snapshot is what makes a single node independently
...
and the node **heals** (0.4.0-rc36): the dropped record is missing from
...
WAL thread alone tops out near 180k records/s (0.4.0-rc35; ~125k
before), and on a disk whose write-back held ~35 MB/s it fell behind at
...
1. **Index.**  Each collection's table is carved at creation.
   ...
   is the `auto` **autoscale** mode, the default.  `autoscale = warn`
   ...
   <auto|warn|off>` sets it at runtime; it is persisted and outranks the
...
3,000).  Since 0.4.0-rc33 every chunk names the table it walked, the
...
orders two requests on one key from one client: `jdel` and `jincr` did
not until S211, and a `jdel` right behind a routed `jset` could reach a
