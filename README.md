...
the staging fleet; the **0.4.0** line is at release candidate 2
(2026-09-17), a capability step: **Redis pub/sub on every door and
...
the node that applies a write.  rc2 fixes rc1's pattern matcher, which
a hostile pattern could stall or push past its buffer.  It carries the 0.3.8 candidates' four
...
(`pubsub_queue_mb`, 16 MB by default); past it they are dropped and
counted, never allowed to pin memory or delay a shutdown.  Raising it
drops less in a burst and costs memory and a slower exit under load.
