...
**Status.**  The last tagged release is **0.3.7** (2026-09-15), on
the staging fleet; the **0.4.0** line is at release candidate 1
(2026-09-16), a capability step: **Redis pub/sub on every door and
...
and in libperfd 0.2.9, a publish on any node reaching the subscribers
on every node, and Redis keyspace notifications (`notify_events`) from
the node that applies a write.  It carries the 0.3.8 candidates' four
...
landed on another.  A collection with `notify_events` publishes Redis
