...
**0.4.0** line is at release candidate 42 (2026-09-26), which is what
the test fleet runs.  Candidates 10, 13, 14, 30-34 and 36-39 of that
line are RED - their pipelines failed `tailtest`, `growtest`, `pintest`,
`sweepclocktest` (30, 31), `loadtest` (32), `keysretiretest` and
`outagetest` (33), `applythreadstest` (34), `healtest` (36, 38),
`rdbkilltest` (37) and `walsizetest` (38, 39), each a suite that runs
only on a tag - and 11, 12, 35, 40 and 41 are superseded; the CHANGELOG
says so against each.  Start from rc42, a capability step: **Redis pub/sub on every door and
