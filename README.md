...
**0.4.0** line is at release candidate 34 (2026-09-25), which is what
the test fleet runs.  Candidates 10, 13, 14 and 30-33 of that line are
RED - their pipelines failed `tailtest`, `growtest`, `pintest`,
`sweepclocktest` (30, 31), `loadtest` (32) and `keysretiretest` and
`outagetest` (33), each a suite that runs only on a tag - and 11 and 12
are superseded; the CHANGELOG says so against each.  Start from rc34, a capability step: **Redis pub/sub on every door and
