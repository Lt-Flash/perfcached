...
check-fast: selftest selftest_broken memprobe noisetest noiseboundtest wipetest perfcached pubsubcli udppushcli \
		...
		clplacetest clseltest clpeerstest clpendtest clloctest clboottest clpushtest clwiretest clbulktest clmigtest clfwdtest clinittest clrestest clmembtest clunktest clsendtest clfleettest clworktest clpstest clspreadtest clpulltest clcoltest clplattest natbench slottest jwtest walringtest asyncbintest routedpairtest movedhinttest \
		clustersim psrelaytest psindextest psqueuetest psinteresttest psudptest rpsbench legmetatest legwalkremovetest scanremovetest histtest capracetest verstamptest arenadbgtest syncfailshim.so threadfreeze clapplytest evactest
	./clustersim 500
	./clapplytest
	./evactest
	./slottest
...
check: check-wired selftest selftest_broken memprobe noisetest noiseboundtest wipetest perfcached perfcli perfdump perfload pcbench \
		...
		natbench slottest jwtest walringtest clustersim reachtest origintest indextest quiesctest regiontest legmetatest legwalkremovetest scanremovetest histtest capracetest verstamptest arenadbgtest syncfailshim.so netcutshim.so threadfreeze clapplytest evactest
	./clustersim 5000
	./clapplytest
	./evactest
	./reachtest
...
clean:
	rm -f src/*.o src/compat/*.o src/core/*.o lib/*.o perfcached \
		perfcli perfdump perfload libperfd.a libtest failovertest routedpairtest movedhinttest capracetest verstamptest arenadbgtest noiseboundtest syncfailshim.so netcutshim.so threadfreeze clapplytest evactest wedgetest selftest selftest_broken \
