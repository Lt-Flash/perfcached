...
check: check-wired selftest selftest_broken memprobe noisetest noiseboundtest wipetest perfcached perfcli perfdump perfload pcbench \
		...
		natbench slottest jwtest walringtest clustersim reachtest origintest indextest quiesctest regiontest legmetatest legwalkremovetest scanremovetest histtest capracetest verstamptest arenadbgtest syncfailshim.so threadfreeze clapplytest evactest
	./clustersim 5000
	./clapplytest
	./evactest
	./reachtest
...
clean:
	rm -f src/*.o src/compat/*.o src/core/*.o lib/*.o perfcached \
		perfcli perfdump perfload libperfd.a libtest failovertest routedpairtest movedhinttest capracetest verstamptest arenadbgtest noiseboundtest syncfailshim.so threadfreeze clapplytest evactest wedgetest selftest selftest_broken \
