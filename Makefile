...
check-fast: selftest selftest_broken memprobe noisetest noiseboundtest wipetest perfcached pubsubcli udppushcli \
		...
		clustersim psrelaytest psindextest psqueuetest psinteresttest psudptest rpsbench legmetatest legwalkremovetest scanremovetest histtest capracetest arenadbgtest syncfailshim.so
	./clustersim 500
	./slottest
	./legmetatest
	./legwalkremovetest
...
check: check-wired selftest selftest_broken memprobe noisetest noiseboundtest wipetest perfcached perfcli perfdump perfload pcbench \
		...
		natbench slottest jwtest clustersim reachtest origintest indextest quiesctest regiontest legmetatest legwalkremovetest scanremovetest histtest capracetest arenadbgtest syncfailshim.so
	./clustersim 5000
	./reachtest
	./origintest
	./indextest
...
clean:
	rm -f src/*.o src/compat/*.o src/core/*.o lib/*.o perfcached \
		perfcli perfdump perfload libperfd.a libtest failovertest routedpairtest movedhinttest capracetest arenadbgtest noiseboundtest syncfailshim.so wedgetest selftest selftest_broken \
