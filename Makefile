...
check-fast: selftest selftest_broken memprobe noisetest noiseboundtest wipetest perfcached pubsubcli udppushcli \
		...
		clustersim psrelaytest psindextest psqueuetest psinteresttest psudptest rpsbench legmetatest legwalkremovetest scanremovetest histtest capracetest arenadbgtest syncfailshim.so threadfreeze clapplytest
	./clustersim 500
	./clapplytest
	./slottest
	./legmetatest
...
check: check-wired selftest selftest_broken memprobe noisetest noiseboundtest wipetest perfcached perfcli perfdump perfload pcbench \
		...
		natbench slottest jwtest clustersim reachtest origintest indextest quiesctest regiontest legmetatest legwalkremovetest scanremovetest histtest capracetest arenadbgtest syncfailshim.so threadfreeze clapplytest
	./clustersim 5000
	./clapplytest
	./reachtest
	./origintest
...
clean:
	rm -f src/*.o src/compat/*.o src/core/*.o lib/*.o perfcached \
		perfcli perfdump perfload libperfd.a libtest failovertest routedpairtest movedhinttest capracetest arenadbgtest noiseboundtest syncfailshim.so threadfreeze clapplytest wedgetest selftest selftest_broken \
