...
perfcached: src/main.o src/config.o src/daemon.o src/proto.o src/json.o \
		...
		src/obs.o $(CORE_OBJS) src/compat/compat.o
	$(CC) $(CFLAGS) -o $@ $^ $(LDLIBS)
...
check: selftest selftest_broken memprobe noisetest perfcached perfcli pcbench \
		libtest failovertest asynctest clmaptest cltermtest clsynctest clhisttest clplacetest clseltest \
		natbench slottest jwtest
	./slottest
	./jwtest
	./clmaptest
	./cltermtest
...
install: perfcached perfcli libperfd.a
	...
	@echo "next: cp $(DESTDIR)/etc/perfcached/perfcached.conf.example" \
		"/etc/perfcached/perfcached.conf && edit the secrets"

.PHONY: all shim core check clean install
